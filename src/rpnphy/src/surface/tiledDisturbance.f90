...
contains
  ...
  subroutine tiledDisturbancePrep(ctem_on, lnduseon, PFTCompetition, timberHarvest, prescribedFire, &
    ...
    use classicParams, only : DELT, crop, dynTilingMaxDisturb, c_switch
    ...
    character(8), intent(inout), dimension(nlat,nmos) :: peatlandTyperow
    ...
    logical, intent(in) :: Ncycle_on
    character(6), intent(in) :: useTracer
    ...
    associate(prescribedFireTimestep  => c_switch%prescribedFireTimestep)   !< Switch determining the time step in the prescribed fire file.

    ! perform a check and stop the model if CTEM is off, competition is on or agricultural LUC is on and dynamic tiling is on
    if ((.not.(ctem_on)) .or. (lnduseon .or. PFTCompetition) .and. dynamicTilingOn) then
        write(6, * ) 'tiledDisturbancePrep: tiled Disturbance not available with competition, agricultural LUC or when CTEM is off'
    ...
    ! If timber harvest or prescribed fire is on and this is the first day of the year, execute the timber harvest or prescribed fire case.
    ! Also execute the prescribed fire case, if prescribed fire is on, the fire forcing file time step is monthly and it's the first day of the month.
    if ((iday == 1) .and. (ncount == nday) .and. (timberHarvest .or. prescribedFire) .or. (prescribedFire .and. (prescribedFireTimestep == 'Monthly') .and. (DOM == 1))) then

      ! decide if harvest or prescribed fire need to occur and select the appropriate case, probably the simplified dyn cases are not needed
      if (any(timharvrow(:) > 0.0) .and. dynamicTilingOn .and. timberHarvest .and. .not.(prescribedFire)) then
       ...
       title = 'cleandyn' ! run cleanTiles to pre-emptively join tiles in years with no harvest or fire prescribed
        ...
        title = 'noaction' ! The default return, nothing needs to happen
      end if 
    ...
    ! and to make it easier to create special cases
    ...
    case ('timhar') ! Timber harvest -----------------------
        ...
        do i=1,nlat 
          if(timharvrow(i) > 0.0) then
            ...
            ! if there is no disturbance we make certain timharvarearow is set to zero 
            timharvarearow(i,:) = 0.0
            prsfirearearow(i,:) = 0.0 ! because we defined timhar as having no prescribed fire
          end if 
        end do
    ...
    case ('prefire') ! Prescribed fire --------------------------------
        ...
        do i=1,nlat 
          if(prsfirerow(i) > 0.0) then
            ...
            ! because prescribed fire alone is being applied here, there's no need for further checks after FAREAROUT is determined
            fcancmxcrop(i) = 0.0
            do j = 1,icc
              if (crop(j)) then
        ...
        do i=1,nlat 
          if(prsfirerow(i) > 0.0) then
            ...
            timharvarearow(i,:) = 0.0 ! because we defined this as only a prescribed fire operation, no timber harvest allowed.
          else
            ! if there is no disturbance we make certain prsfirearearow is set to zero 
            prsfirearearow(i,:) = 0.0
            timharvarearow(i,:) = 0.0 ! because we defined this as only a prescribed fire operation, no timber harvest allowed.
          end if 
        end do
    ...
    case ('firehar')  ! Prescribed fire and timber harvest ------------------------------
        ...
        do i=1,nlat 
          if ((prsfirerow(i) + timharvrow(i)) > 1.0) then
              write(6, * ) 'tiledDisturbancePrep: disturbances exceed available tile area'
        ...
        do i=1,nlat 
          if (prsfirerow(i) > 0.0) then
            ...
            if(FAREAROUT(i) + timharvrow(i) > dynTilingMaxDisturb) then
              ...
              ! the amout to be harvested/non-vegetated/non-crop area available in the grid cell
              ! as a result fire is downscaled and priority is given to harvest because it is stand replacing

              write(6, * ) '########################################'
              write(6, * ) 'warning in tiledDisturbancePrep: disturbance was corrected as to not exceed available land area'
              ...
              if ((FAREAROUT(i) + timharvrow(i)) > dynTilingMaxDisturb) then
                  write(6, * ) 'tiledDisturbancePrep: disturbance rescaling failed'
        ...
        do i=1,nlat 
          if (timharvrow(i) > 0.0) then
            ...
            ! keep setting valid tiles as input tiles until there's enough input tile area to carry out a split
    ...
    case('notdyn')  ! Prescribed fire or timber harvest but without dynamic tiling -----------------
        ...
        do i=1,nlat 
            ...
            if((prsfirearearow(i,1) + timharvarearow(i,1)) > dynTilingMaxDisturb) then
              ...
              ! the not about to be harvested/non-vegetated/non-crop area available in the grid cell.
              ! As a result, fire is downscaled and priority is given to harvest, because it is stand replacing.
    ...
    case ('noaction') ! The default return, nothing needs to happen
  ...
  subroutine initialMassBalance(fcancmx, gleafmas, bleafmas, stemmass, rootmass, &
    ...
    ! these are important descriptors of the land surface used by the function
    real, intent(in), dimension(ilg,icc) :: fcancmx
    logical, intent(in) :: Ncycle_on
    real, intent(in), dimension(ilg) :: faregat

    ! vegetation biomass related variables used by mass balance checks
    ...
    ! soil and litter variables used by mass balance checks
    real, intent(in), dimension(ilg,iccp2,ignd) :: soilcmas
    real, intent(in), dimension(ilg,iccp2,ignd) :: litrmass

    ! N cycling related variables used by mass balance checks
  ...
  subroutine finalMassBalance(fcancmx, gleafmas, bleafmas, stemmass, rootmass, &
    ...
    ! fire flux used by mass balance checks
    ...
    ! N cycling related fire flux used by mass balance checks
  ...
  subroutine fireRank(gleafmasrow, bleafmasrow,stemmassrow,litrmassrow,fcancmxrow,bterm_veg)
    ...
    real, dimension(nlat,nmos,icc) :: biomass
    real, dimension(nlat,nmos,icc) :: bterm_veg_pp
  ...
  !! These operations allow biomass to be harvested or burned based upon inputs read in from a file. They call the subroutines within dynamicTiling to construct
  ...
  !!
  !! Constituent functions and subroutines
  !!
  !! 1. harvestTile: This subroutine harvests biomass from a tile based upon inputs read in from a file. It is called in ctemDriver.f90. It takes as its input timharvareagat,
  ...
  !! tiling is off, just a portion of the single grid cell is harvested. After the tile is harvested, it also alters its age [tileAgegat] as needed. It uses the same parameters
  !! and storage pools as the default land use change module (i.e. Arora and Boer 2010).  
  !!
  !! 2. tiledDisturbancePrep: The subroutine preps the simulation of disturbance. If dynamic tiling is off, if just reads in information from the disturbance files and 
  ...
  !!
  !! 3. initialMassBalance: The subroutine calculates the initial mass balance of the harvested tile for use later on.
  !!
