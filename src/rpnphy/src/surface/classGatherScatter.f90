...
contains
  ...
  subroutine classGather (TBARGAT, THLQGAT, THICGAT, TPNDGAT, ZPNDGAT, & ! Formerly CLASSG
                          ...
                          stddevTopographyGAT, ILMOS, JLMOS, NML, NL, NT, NM, &
                          ILG, IG, IC, ICP1, NBS, &
    ...
    real, intent(in) :: REFROT(NL,NM)    !< Snow grain size (for snoAlbedoParam='4-band' option)
    real, intent(in) :: maxAnnualActLyrROT(NL,NT)  !< Active layer depth maximum over the e-folding period specified by parameter eftime (m)
	  real, intent(in) :: FSNOROT(NL,NM)   !< Fractional snow cover
	  real, intent(in) :: maxSNOROT(NL,NM)   !< Effective maximum SWE used to reconcile accumulation and ablation curves \f$[kg m^{-2}]\f$.
    !
    ...
    real, intent(out)   :: THLQGAT(ILG,IG), THICGAT(ILG,IG),         &
                           ...
                           maxAnnualActLyrGAT(ILG), FSNOGAT(ILG), &
                           maxSNOGAT(ILG)
    !
    ...
    !! coverage is < 100% [m]
    real, intent(in) :: stddevTopographyROT(NL,NT)       !< Standard deviation of elevation [m]
    ...
    real, intent(out) :: THPGAT (ILG,IG), THRGAT (ILG,IG), THMGAT (ILG,IG), &
                         ...
                         stddevTopographyGAT(ILG), ZPLGGAT(ILG), ZPLSGAT(ILG)
    !
    ...
    real, intent(in) :: BCSNROW(NL) ! Black carbon mixing ratio in snow \f$[kg m^{-2} s^{-1}]\f$ 
    ...
    do K = 1,NML ! loop 100
      ...
      FSNOGAT(K) = FSNOROT(ILMOS(K),JLMOS(K))
      maxSNOGAT(K) = maxSNOROT(ILMOS(K),JLMOS(K))
      ...
      stddevTopographyGAT(K) = stddevTopographyROT(ILMOS(K),JLMOS(K))
  ...
  subroutine classScatter (TBARROT, THLQROT, THICROT, TSFSROT, TPNDROT, & ! Formerly CLASSS
                           ...
                           QACROT, WSNOROT, REFROT, BCSNROW, &
                           EMISROT, SALBROT, CSALROT, &
                           groundHeatFluxROT, ILMOS, JLMOS, &
                           ...
                           QACGAT, WSNOGAT, REFGAT, BCSNGAT, &
                           EMISGAT, SALBGAT, CSALGAT, groundHeatFlux)
    !
    ...
    real, intent(out)    :: FSNOROT(NL,NM) !< Fractional snow cover [ ]
    ...
    real, intent(out)    :: maxSNOROT(NL,NM)   !< Effective maximum SWE used to reconcile accumulation and ablation curves \f$[kg m^{-2}]\f$.	
    real, intent(out)    :: BCSNROW(NL)      !< Black carbon mixing ratio in snow \f$[kg m^{-2} s^{-1}]\f$	
    !
    ...
    real, intent(in)     :: FSNOGAT(ILG) !<	Fractional snow cover [ ]
    ...
    real, intent(in)     :: BCSNGAT(ILG) !< Black carbon mixing ratio in snow \f$[kg m^{-2} s^{-1}]\f$
    real, intent(in)     :: EMISGAT(ILG) !< Surface emissivity  [  ]
    real, intent(in)     :: groundHeatFlux(ILG) !< Heat flux at soil surface \f$[W m^{-2} ]\f$
    real, intent(in)     :: maxSNOGAT(ILG) !< Effective maximum SWE used to reconcile accumulation and ablation curves \f$[kg m^{-2}]\f$.	
    ...
    do K = 1,NML ! loop 100
      ...
      maxSNOROT(ILMOS(K),JLMOS(K)) = maxSNOGAT(K)
      !>
