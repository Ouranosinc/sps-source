...
module classStateVars
  ...
  ! P. Bartlett Sep 2021 - Declare variables (maxSNOROT/maxSNOGAT, stddevTopographyROT/stddevTopographyGAT) for new fractional snow
  !                        parameterization based on CLM.  
  ...
  use classicParams,                 only: ican, icp1
  ...
  type class_gather(ilg, ignd, nbs)
    ...
    real, dimension(ilg) :: BCSNGAT !< Black carbon mixing ratio in snow \f$[kg m^{-2} s^{-1}]\f$ 
    ...
    real, dimension(ilg) :: FSNOGAT !< Fractional snow coverage [ ]
    real, dimension(ilg) :: maxSNOGAT !< Effective maximum SWE to reconcile accumulation and recession curves \f$[kg m^{-2} ]\f$
    real, dimension(ilg) :: stddevTopographyGAT !< Standard deviation of elevation [m]
    ...
    integer, dimension(ilg) :: snwct !< FWI snow counter
    ...
    real, dimension(ilg) :: SFCTDGAT !< Diagnosed screen-level dew point temperature [K]
    ...
    real, dimension(ilg) :: FSNOW   !<
    real, dimension(ilg) :: SNWMAX  !<
    real, dimension(ilg) :: SNOPRE  !< Effective maximum SWE used to reconcile snow accumulation and recession curves \f$[kg m^{-2} ]\f$
    real, dimension(ilg) :: FSNOPRE !< Effective maximum SWE used to reconcile snow accumulation and recession curves \f$[kg m^{-2} ]\f$
    real, dimension(ilg) :: stddevTopography  !< standadrd deviation of elevation \f$[m]\f$ 
  ...
  ! ================================================================================
  !> Physics variables in the 'row/tile' (rot) structure
  type class_rowtile(nlat, nmos, ignd, nbs)
    ...
    real, dimension(nlat) :: BCSNROW !< Black carbon mixing ratio in snow \f$[kg m^{-2} s^{-1}]\f$
    real, dimension(nlat) :: REFROW  !< FLAG lbw please add name
    ...
    real, dimension(nlat) :: FSNOROW !< Fractional snow coverage
    real, dimension(nlat) :: maxSNOROW !< Effective maximim SWE to reconcile accumulation and recession curves \f$[kg m^{-2} ]\f$
    ...
    real, dimension(nlat) :: SFCTDROW !< Diagnosed screen-level dew point temperature [K]
    ...
    real, dimension(nlat,nmos) :: FSNOROT !< Fractional snow coverage
    real, dimension(nlat,nmos) :: maxSNOROT !< Effective maximum SWE to reconcile accumulation and recession curves
    real, dimension(nlat,nmos) :: stddevTopographyROT !< Standard deviation of elevation [m]
    ...
    real, dimension(nlat,nmos) :: SFCTDROT !< Diagnosed screen-level dew point temperature [K]
    ...
    real, dimension(nlat,nmos,ignd) :: MRSOLACC_M        !< Total water content of soil layers \f$[kg m^{-2}]\f$ (accumulated for means)
    ...
    real, dimension(nlat,nmos) :: SPCPACC_M             !< Snowfall rate \f$[kg m^{-2} s^{-1} ]\f$
    real, dimension(nlat,nmos) :: RPCPACC_M             !< Rainfall rate \f$[kg m^{-2} s^{-1} ]\f$
    ...
    real, dimension(nlat,nmos) :: BCSNACC_M             !< Black carbon mixing ratio in snow \f$[kg m^{-2} s^{-1}]\f$
    real, dimension(nlat,nmos) :: BCSNOACC_M            !< Black carbon mixing ratio \f$[kg m^{-3}]\f$
    real, dimension(nlat,nmos) :: REFACC_M              !< Snow grain size \f$[ m ]\f$ 
    ...
    real, dimension(nlat,nmos) :: FSUPACC_M             !< Daily sum of reflected solar radiation \f$[W m^{-2} ]\f$
  ...
  end type class_rowtile

  !=================================================================================
  !> CLASS's monthly and annual outputs
  type class_moyr_output(nlat, ignd)
    ...
    real, dimension(nlat) :: FSUPACC_MO   !< Upward solar radiation flux in air \f$[W m^{-2} ]\f$
    ...
    real, dimension(nlat) :: BCSNACC_MO   !< Black carbon mixing ratio in snow \f$[kg m^{-2} s^{-1}]\f$ 
    ...
    real, dimension(nlat) :: SPCPACC_MO   !< Snowfall rate \f$[kg m^{-2} s^{-1} ]\f$
    real, dimension(nlat) :: RPCPACC_MO   !< Rainfall rate \f$[kg m^{-2} s^{-1} ]\f$
    real, dimension(nlat) :: PRESACC_MO   !< Surface air pressure [Pa]
    real, dimension(nlat) :: QAACC_MO     !< Specific humidity at reference height \f$[kg kg^{-1} ]\f$ 
    real, dimension(nlat) :: UVACC_MO     !< Wind speed \f$[m s^{-1} ]\f$
    ...
    real, dimension(nlat) :: ROFBACC_YR  !< Base flow from bottom of the soil column \f$[kg m^{-2} s^{-1} ]\f$ (accumulated)
    real, dimension(nlat) :: OVRACC_YR   !< Overland flow from top of soil column \f$[kg m^{-2} s^{-1} ]\f$ (accumulated)
    ...
    real, dimension(nlat) :: SPCPACC_YR  !< Snowfall rate \f$[kg m^{-2} s^{-1} ]\f$
    real, dimension(nlat) :: RPCPACC_YR  !< Rainfall rate \f$[kg m^{-2} s^{-1} ]\f$
    real, dimension(nlat) :: PRESACC_YR  !< Surface air pressure [Pa]
    real, dimension(nlat) :: QAACC_YR    !< Specific humidity at reference height \f$[kg kg^{-1} ]\f$
    real, dimension(nlat) :: UVACC_YR    !< Wind speed \f$[m s^{-1} ]\f$
    ...
    real, dimension(nlat) :: TCANACC_YR  !< Vegetation canopy temperature [K]
    ...
    real, dimension(nlat) :: SNOACC_YR   !< Mass of snow pack \f$[kg m^{-2}]\f$
    real, dimension(nlat) :: ZSNACC_YR   !< Depth of snow pack \f$[ m ]\f$
    ...
    real, dimension(nlat,ignd) :: MRSOL_YR !< Total water content of soil layers \f$[kg m^{-2}]\f$ (accumulated for means)
    real, dimension(nlat,ignd) :: TBARACC_YR !< Temperature of soil layers [K] (accumulated for means)
...
contains
  ...
  subroutine resetClassMon(class_out, nlat, ignd)

    implicit none

    integer, intent(in) :: nlat
    integer, intent(in) :: ignd
    class(class_moyr_output(nlat, ignd)), intent(inout) :: class_out
    ...
    do i = 1, nlat
      ...
      class_out%SPCPACC_MO(I) = 0.
      class_out%RPCPACC_MO(I) = 0.
      class_out%PRESACC_MO(I) = 0.
      class_out%QAACC_MO(I) = 0.
      class_out%UVACC_MO(I) = 0.
  ...
  subroutine resetClassYr(class_out, nlat, ignd)
    ...
    do i = 1, nlat
      ...
      class_out%ROFBACC_YR(I) = 0.
      class_out%OVRACC_YR(I) = 0.
      class_out%QEVPACC_YR(I) = 0.
      class_out%ROFACC_YR(I) = 0.
      class_out%PREACC_YR(I) = 0.
      class_out%PRESACC_YR(I) = 0.
      class_out%SPCPACC_YR(I) = 0.
      class_out%RPCPACC_YR(I) = 0.
      class_out%QAACC_YR(I) = 0.
      class_out%UVACC_YR(I) = 0.
      class_out%EVAPACC_YR(I) = 0.
      class_out%TRANSPACC_YR(I) = 0.
      class_out%TAACC_YR(I) = 0.
      class_out%TCANACC_YR(I) = 0.
      ...
      class_out%SNOACC_YR(I) = 0.
      class_out%ZSNACC_YR(I) = 0.
      ...
      do J = 1,IGND
        ...
        class_out%MRSOL_YR(I,J) = 0.
        class_out%TBARACC_YR(I,J) = 0.
      end do
    end do
  ...
  subroutine resetAccVars(class_rot, nlat, nmos, ignd, nbs)
    ...
    class(class_rowtile(nlat, nmos, ignd, nbs)), intent(inout) :: class_rot

    class_rot%altotcntr_d = 0

    class_rot%PREACC_M = 0.
    class_rot%SPCPACC_M = 0.
    class_rot%RPCPACC_M = 0.
    class_rot%GTACC_M = 0.

    class_rot%QEVPACC_M = 0.
    class_rot%HFSACC_M = 0.
    class_rot%HMFNACC_M = 0.
    class_rot%ROFACC_M = 0.
    class_rot%ZSNACC_M = 0.
    class_rot%SNOACC_M = 0.
    class_rot%FSNOACC_M = 0.
    class_rot%BCSNACC_M = 0.
    class_rot%BCSNOACC_M = 0.
    class_rot%REFACC_M = 0.

    class_rot%OVRACC_M = 0.
    class_rot%ROFSACC_M = 0.
    class_rot%ROFBACC_M = 0.
    class_rot%ROFCACC_M = 0.
    class_rot%ROFNACC_M = 0.
    class_rot%wtableACC_M = 0.
    class_rot%ALVSACC_M = 0.
    class_rot%ALIRACC_M = 0.
    class_rot%RHOSACC_M = 0.
    class_rot%TSNOACC_M = 0.
    class_rot%WSNOACC_M = 0.
    class_rot%TCANACC_M = 0.
    class_rot%RCANACC_M = 0.
    class_rot%SCANACC_M = 0.
    class_rot%GROACC_M = 0.
    class_rot%FSINACC_M = 0.
    class_rot%FLINACC_M = 0.
    class_rot%TAACC_M = 0.
    class_rot%TSURFACC_M = 0.
    class_rot%UVACC_M = 0.
    class_rot%PRESACC_M = 0.
    class_rot%QAACC_M = 0.
    class_rot%ALTOTACC_M = 0.
    class_rot%FSUPACC_M = 0.
    class_rot%ALSNOACC_M = 0.
    class_rot%EVAPACC_M = 0.
    class_rot%GROUNDEVAP_M = 0.
    class_rot%CANOPYEVAP_M = 0.
    class_rot%TRANSPACC_M = 0.
    class_rot%ZPNDACC_M = 0.
    class_rot%FLUTACC_M = 0.
    class_rot%ACTLYR_M = 0.

    class_rot%TBARACC_M = 0.
    class_rot%THLQACC_M = 0.
    class_rot%THICACC_M = 0.
    class_rot%MRSOLACC_M = 0.
  ...
  subroutine initDiagnosticVars (class_gat, nml, ignd, ilg, nbs)
    ...
    integer, intent(in) :: nbs
    class(class_gather(ilg, ignd, nbs)), intent(inout) :: class_gat
  ...
  !> Initialization of diagnostic variables split out of classGather for consistency with gcm applications.
  subroutine initRowVarsPhysics(class_rot, nmos, nlat, ignd, nbs)
  ...
  subroutine classdump(class_gat, class_rot, class_out)

    use classicParams, only : nlat, nmos, ilg, ignd, nbs
    
    implicit none
    
    type(class_gather(ilg, ignd, nbs)),         intent(inout) :: class_gat
    type(class_rowtile(nlat, nmos, ignd, nbs)), intent(inout) :: class_rot
    type (class_moyr_output(nlat, ignd)),       intent(inout) :: class_out
    ...
    print *, 'SFCTDGAT ', class_gat%SFCTDGAT
    ...
    print *, 'SFCTDROW', class_rot%SFCTDROW
    ...
    print *, 'MRSOLACC_M ', class_rot%MRSOLACC_M
    ...
    print *, 'SPCPACC_M ', class_rot%SPCPACC_M
    print *, 'RPCPACC_M ', class_rot%RPCPACC_M
    print *, 'PRESACC_M ', class_rot%PRESACC_M
    ...
    print *, 'ROFBACC_YR ', class_out%ROFBACC_YR
    print *, 'OVRACC_YR ', class_out%OVRACC_YR
    print *, 'EVAPACC_YR ', class_out%EVAPACC_YR
    print *, 'TAACC_YR ', class_out%TAACC_YR
    print *, 'TCANACC_YR ', class_out%TCANACC_YR
    ...
    print *, 'SNOACC_YR ', class_out%SNOACC_YR
    print *, 'ZSNACC_YR ', class_out%ZSNACC_YR
