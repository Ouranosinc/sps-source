...
contains
  ...
  subroutine mortalty (stemmass, stemmass_ns, stemmass_s, & ! In
    ...
    !
    use classicParams, only : icc, kk, zero, mxmortge, kmort1, maxage, maxage_peat, c_switch
    ...
    real, intent(out) :: tracerStemMort(ilg,icc) !< Tracer stem litter from mortality, 14C: \f$ng ^{14}C/m^2\f$
    real, intent(out) :: tracerRootMort(ilg,icc) !< Tracer stem litter from mortality, 14C: \f$ng ^{14}C/m^2\f$
    real, intent(out) :: tracerGLeafMort(ilg,icc) !< Tracer stem litter from mortality, 14C: \f$ng ^{14}C/m^2\f$
  ...
  subroutine updatePoolsMortality (il1, il2, ilg, stemltrm, stemltrm_ns, & ! In
    ...
    use classicParams, only : ican, nol2pfts, classpfts, ignd, icc, reindexPFTs, c_switch
    ...
    real, intent(in) :: tracerStemMort(ilg,icc) !< Tracer stem litter from mortality14C: \f$ng ^{14}C/m^2\f$
    real, intent(in) :: tracerRootMort(ilg,icc) !< Tracer stem litter from mortality14C: \f$ng ^{14}C/m^2\f$
    character(6), intent(in) :: useTracer !< character: Switch for use of a model tracer. If useTracer is 'None' then the
                                      !! tracer code is not used. useTracer = 'Simple' turns on a simple tracer that tracks
                                      !! pools and fluxes. The simple tracer then requires that the tracer values in
                                      !! the init_file and the tracerCO2file are set to meaningful values for the experiment being run.
    ...
    real, intent(inout) :: tracerGLeafMort(ilg,icc) !< Tracer stem litter from mortality14C: \f$ng ^{14}C/m^2\f$
    ...
    do j = 1,ican
      do m = reindexPFTs(j,1),reindexPFTs(j,2)
        do i = il1,il2
          ...
          if (useTracer /= 'None') then ! Update the tracer pools if being used.
          ...
          case ('NdlTr' , 'BdlTr', 'Crops', 'BdlSh')
            ...
            if (useTracer /= 'None') tracerGLeafMass(i,m) = tracerGLeafMass(i,m) - tracerGLeafMort(i,m)
          ...
          do k = 1,ignd
            if (k == 1) then
              ...
              if (useTracer /= 'None') tracerLitrMass(i,m,k) = tracerLitrMass(i,m,k) + tracerStemMort(i,m) &
              ...
              if (useTracer /= 'None') tracerLitrMass(i,m,k) = tracerLitrMass(i,m,k) + tracerRootMort(i,m) * rmatctem(i,m,k)
