...
subroutine energyBudgetDriver (TBARC, TBARG, TBARCS, TBARGS, THLIQC, THLIQG, & ! Formerly CLASST
                               ...
                               TADP, RHOAIR, QSWINV, QSWINI, QLWIN, UWIND, VWIND, &
                               ...
                               NBS, snoAlbedoParam, DAYL, DAYL_MAX, DLON, DLAT, &
                               peatlandType, mossPresent, ancsmoss, angsmoss, ancmoss, angmoss, &
                               rmlcsmoss, rmlgsmoss, rmlcmoss, rmlgmoss, Cmossmas, dmoss, &
                               iday, REDCOEFF_VCMAX, &
                               redcoeff_vcmaxMoss, Nmossmas, &
                               GLEAFMAS, NGLEAFMAS, Ncycle_on, NcycleConnect_on, VCMAX0_GA, &
                               PSISAT, BI, DSL, DSLC, RB, AVE_SM_FUNC)  
  ...
  !     * JUN 10/13 - M.LAZARE/   ADD SUPPORT FOR "snoAlbedoParam" FORMULATION.
  !     *             M.NAMAZI.
  !
  ...
  character(7), intent(in) :: snoAlbedoParam !< Switch to model snow albedo in two or more wavelength bands
  !
  ...
  real, intent(out) :: STDA  (ILG) !< Diagnosed screen-level dew point temperature [K]
  ...
  real, intent(out) :: AVE_SM_FUNC(ILG,ICTEM)  !< Soil mositure stress function, (unitless) FOR EACH PFT
  !
  !
  !     * INPUT FIELDS.
  !
  ...
  real, intent(inout) :: RC    (ILG) !< Stomatal resistance of vegetation over bare ground \f$[s m^{-1} ]\f$
  real, intent(inout) :: RCS   (ILG) !< Stomatal resistance of vegetation over snow \f$[s m^{-1} ]\f$
  real, intent(inout) :: FRAINC(ILG) !< Fractional coverage of canopy by liquid water over snow-free subarea [ ]
  real, intent(inout) :: FSNOWC(ILG) !< Fractional coverage of canopy by frozen water over snow-free subarea [ ]
  real, intent(inout) :: FRAICS(ILG) !< Fractional coverage of canopy by liquid water over snow-covered subarea [ ]
  real, intent(inout) :: FSNOCS(ILG) !< Fractional coverage of canopy by frozen water over snow-covered subarea [ ]
  ...
  real, intent(inout) :: FROOT (ILG,IG) !< Fraction of total transpiration contributed by soil layer over snow-free subarea  [  ]
  real, intent(inout) :: FROOTS(ILG,IG) !< Fraction of total transpiration contributed by soil layer over snow-covered subarea  [  ]
  !
  ...
  real, intent(out) :: ANCSVEG(ILG,ICTEM) !< NET PHOTOSYNTHETIC RATE FOR CTEM's 8 PFTs FOR CANOPY OVER SNOW SUBAREA
  real, intent(out) :: ANCGVEG(ILG,ICTEM) !< NET PHOTOSYNTHETIC RATE FOR CTEM's 8 PFTs FOR CANOPY OVER GROUND SUBAREA
  real, intent(out) :: RMLCSVEG(ILG,ICTEM)!< LEAF RESPIRATION RATE FOR CTEM's 8 PFTs FOR CANOPY OVER SNOW SUBAREA
  real, intent(out) :: RMLCGVEG(ILG,ICTEM)!< LEAF RESPIRATION RATE FOR CTEM's 8 PFTs FOR CANOPY OVER GROUND SUBAREA

  real, intent(inout) :: CFLUXCG(ILG)
  real, intent(inout) :: CFLUXCS(ILG)
  ...
  !
  real, intent(inout) :: RB(ILG) !< Leaf boundary resistance of vegetation \f$[s m^{-1}]\f$
  ...
  real :: VA    (ILG), ZRSLDM(ILG), ZRSLDH(ILG), ZRSLFM(ILG), &
          ...
          SHT   (ILG), & 
          STDAT (ILG)  ! temporary screen level dew point temperature 
  !
  ...
  real, intent(in) :: Cmossmas(ilg), dmoss(ilg)
  real, intent(out) :: ancsmoss(ilg), angsmoss(ilg), &
  ...
  real, intent(in) :: redcoeff_vcmaxMoss(ILG)   !< Reduction coefficient of the VCMAX0 for moss
  logical, intent(in) :: Ncycle_on              !< True if simulation includes Nitrogen Cycle processes
  logical, intent(in) :: NcycleConnect_on       !< When set false and Ncycle_on is true, N limitation will not impact Vcmax
  real,intent(in) :: Nmossmas(ilg)              !< moss biomass N pool, \f$g N/m^2\f$
  !
  !     * TEMPORARY VARIABLES.
  !
  real :: THTOT, WACSAT, QACSAT, RATIOM, RATIOH, FACTM, FACTH
  real :: VCMAX0_CS(ILG,ICTEM), VCMAX0_CG(ILG,ICTEM)                  !< Vcmax over canopy/snow and canopy/ground subareas
  real :: AVE_SM_FUNC_CS(ILG,ICTEM)  !< Soil mositure stress function, (unitless) FOR EACH PFT temorary for use in averaging the canopy over snow subareas
  real :: AVE_SM_FUNC_CG(ILG,ICTEM)  !< Soil mositure stress function, (unitless) FOR EACH PFT temorary for use in averaging the canopy over bare subareas
  !
  !----------------------------------------------------------------------
  !
  ...
  do J = 1, ICTEM ! loop 51
    do I = IL1,IL2 
      ...
      VCMAX0_CG(I,J) = 0.0
      AVE_SM_FUNC(I,J) = 0.0
      AVE_SM_FUNC_CS(I,J) = 0.0
      AVE_SM_FUNC_CG(I,J) = 0.0
    enddo 
  enddo ! loop 51
  !
  ...
  call  energyBudgetPrep(THLIQC, THLIQG, THICEC, THICEG, TBARC, TBARG, & ! Formerly TPREP
                         ...
                         THPOR, THLMIN, THLRET, HCPS, TCS, &
                         TA, RHOSNO, TSNOW, ZSNOW, WSNOW, TCAN, &
                         FC, FCS, DELZ, DELZW, ZBOTW, &
                         ISAND, peatlandType, ILG, IL1, IL2, JL, IG, &
  ...
  ! Initializations
  SFCUBS = 0.0 ; SFCVBS = 0.0 ; USTARBS = 0.0 ; GTBS = 0.0
  !
  ...
  if (NLANDCS > 0) then
    ...
    call energBalVegSolve(ISNOW, FCS, & ! Formerly TSOLVC
                          ...
                          RA, RB, RAGINV, RBINV, TVRTAC, TPOTG, RESID, TCANP, &
                          ...
                          DAYL, DAYL_MAX, peatlandType, mossPresent, Cmossmas, dmoss, &
                          ancsmoss, rmlcsmoss, iday, &
                          REDCOEFF_VCMAX, redcoeff_vcmaxMoss, Nmossmas, &
                          GLEAFMAS, NGLEAFMAS, Ncycle_on, VCMAX0_CS, NcycleConnect_on, &
                          CEVAPC, RSOILC, AVE_SM_FUNC_CS)	
    ...
    if (ISLFD == 0) then
      ...
      !
      call screenRelativeHumidity(SHT, STDAT, STT, SQT, PRESSG, FCS, ILG, IL1, IL2) ! Formerly SCREENRH
      !
      do I = IL1,IL2
        if (FCS(I) > 0.) then
          ST (I) = ST (I) + FCS(I) * STT(I)
          STDA(I) = STDA(I) + FCS(I) * STDAT(I)
  ...
  if (NLANDGS > 0) then
    ...
    call energBalNoVegSolve(ISNOW, FGS, & ! Formerly TSOLVE
                            ...
                            IWATER, IEVAP, ITERCT, ISAND, DLON, DLAT, &
                            ISLFD, ITG, ILG, IG, IL1, IL2, JL, NBS, snoAlbedoParam, &
                            TSTEP, TVIRTS, EVBETA, Q0SAT, RESID, &
                            DCFLXM, CFLUXM, WZERO, TRTOPG, AC, BC, &
                            LZZ0, LZZ0T, FM, FH, ITER, NITER, JEVAP, KF, &
                            mossPresent, co2conc, pressg, Cmossmas, dmoss, &
                            angsmoss, rmlgsmoss, iday, DAYL, DAYL_MAX, &
                            redcoeff_vcmaxMoss, Ncycle_on, Nmossmas, RSOIL)
    !
    ...
    if (ISLFD == 0) then
      ...
      !
      call screenRelativeHumidity(SHT, STDAT, STT, SQT, PRESSG, FGS, ILG, IL1, IL2) ! Formerly SCREENRH
      !
      do I = IL1,IL2
        if (FGS(I) > 0.) then
          ST (I) = ST (I) + FGS(I) * STT(I)
          STDA(I) = STDA(I) + FGS(I) * STDAT(I)
  ...
  if (NLANDC > 0) then
    ...
    call energBalVegSolve(ISNOW, FC, & ! Formerly TSOLVC
                          ...
                          DAYL, DAYL_MAX, peatlandType, mossPresent, Cmossmas, dmoss, &
                          ancmoss, rmlcmoss, iday, &
                          REDCOEFF_VCMAX, redcoeff_vcmaxMoss, Nmossmas, &
                          GLEAFMAS, NGLEAFMAS, Ncycle_on, VCMAX0_CG, NcycleConnect_on, &
                          CEVAPC, RSOILC, AVE_SM_FUNC_CG)

    !
    !  * CALCULATE GRID AVERAGED VCMAX0 and AVE_SM_FUNC USING VALUES FROM CANOPY/SNOW AND CANOPY/GROUND
    !  * SUBAREAS
    !
    do J = 1, ICTEM ! loop 51
      do I = IL1,IL2 
        if ((FCANC(I,J) + FCANCS(I,J)) > 0.) then
          ...
          AVE_SM_FUNC(I,J) = AVE_SM_FUNC_CG(I,J) * FCANC(I,J) + AVE_SM_FUNC_CS(I,J) * FCANCS(I,J)
          AVE_SM_FUNC(I,J) =  AVE_SM_FUNC(I,J) / (FCANC(I,J) + FCANCS(I,J))
        end if
      end do 
    end do ! loop 51
    ...
    if (ISLFD == 0) then
      ...
      !
      call screenRelativeHumidity(SHT, STDAT, STT, SQT, PRESSG, FC, ILG, IL1, IL2) ! Formerly SCREENRH
      !
      do I = IL1,IL2
        if (FC(I) > 0.) then
          ST (I) = ST (I) + FC(I) * STT(I)
          STDA(I) = STDA(I) + FC(I) * STDAT(I)
      ...
      call SLDIAG2(SUT, SVT, STT, SQT, &
                   ...
                   ZDSLM, ZDSLH, ILG, IL1, IL2, JL)
      !
      call screenRelativeHumidity(SHT, STDA, STT, SQT, PRESSG, FC, ILG, IL1, IL2) ! Formerly SCREENRH
      !
      do I = IL1,IL2
        if (FC(I) > 0.) then
          ST (I) = ST (I) + FC(I) * STT(I)
  ...
  if (NLANDG > 0) then
    ...
    call soilHeatFluxPrep(A1, A2, B1, B2, C2, GDENOM, GCOEFF, & ! Formerly TNPREP
                          GCONST, CPHCHG, IWATER, &
                          TBAR, TCTOPG, TCBOTG, &
                          FG, ZPOND, TBAR1P, DELZ, TCSNOW, ZERO, &     !replace ZSNOW with ZERO
                          ISAND, ILG, IL1, IL2, JL, IG)
    ISNOW = 0
    ...
    if (ISLFD == 0) then
      ...
      !
      call screenRelativeHumidity(SHT, STDAT, STT, SQT, PRESSG, FG, ILG, IL1, IL2) ! Formerly SCREENRH
      !
      do I = IL1,IL2
        if (FG(I) > 0.) then
          ST (I) = ST (I) + FG(I) * STT(I)
          STDA(I) = STDA(I) + FG(I) * STDAT(I)
