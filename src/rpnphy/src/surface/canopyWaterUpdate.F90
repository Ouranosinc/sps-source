...
subroutine canopyWaterUpdate (EVAP, SUBL, RAICAN, SNOCAN, TCAN, THLIQ, TBAR, ZSNOW, & ! Formerly CANVAP
  ...
  !     *                         ratio of rb to rc (EcTResistancePart switch; loop 300). 
  ...
  !     * INPUT/OUTPUT ARRAYS.
  !
  real(r8), intent(in) :: TBAR(ILG,IG) !< Temperature of soil layer \f$[K] (T_g)\f$
  ...
  real, intent(in)    :: TCAN  (ILG)  !< Temperature of vegetation canopy \f$[K] (T_c)\f$
  ...
  EcTResistancePart => c_switch%EcTResistancePart                 & !< Switch determining whether to use the leaf boundary layer and stomatal resistances to determine the
                                                                    !! partitioning into canopy evaporation and transpiration or the previous standard approach, where  
                                                                    !! transpiration only occurs, when there is no intercepted water on the canopy; set to false for original 
                                                                    !! calculation 
  )
  ...
  !! variable EVLOST.
  !!
  !! Modifications to canopy evaporation and transpiration (in loop 300 when the switch EcTResistancePart is set to True): \n
