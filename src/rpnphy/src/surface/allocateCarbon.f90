...
subroutine allocate (lfstatus, thliq, ailcg, il1, il2, ilg, iday, & ! In
    ...
    character(8), intent(in) :: peatlandType(ilg) !< Peatland type (else 'None')
    ...
    !! the soil moisture content is same under the entire tile,
    ...
    do j = 1,icc ! loop 200
      do i = il1,il2 ! loop 210
        if (fcancmx(i,j) > 0.0) then
          ...
          if (tot_rmat_ctem(i,j) == 0.0) cycle
...
2000        format(' at (i) = (',i3,'),pft = ',i2,'  allocation fractions not adding to one. sum  = ',e14.7)
    ...
    do i = il1,il2 ! loop 543
      do j = 1, icc ! loop 544
        ...
        case ('NdlEvgTr','CNEvgTr','INEvgTr','BdlEvgTr','BdlDDrTr','CropC3  ','CropC4  ','GrassC4 ','BdlEvgSh')
...
2300        format(' at (i) = (',i3,'),pft = ',i2,'  allocation fractions &
 not adding to one. sum  = ',f12.7)
  ...
  subroutine updatePoolsAllocateRepro (il1, il2, ilg, sort, ailcg, lfstatus, nppveg, & ! In
                                       ...
                                       tracerNPP, tracerRMS, tracerRMR, tracerRML, tracerGPP, leapnow, & ! In
    ...
    !! modified by land-use change,and competition between pfts
    real, intent(in) :: tracerNPP(ilg,icc)    !< tracer NPP for individual pfts, (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    real, intent(in) :: tracerRMS(:,:)        !< Tracer maintenance respiration for stem for the CTEM pfts (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    real, intent(in) :: tracerRMR(:,:)        !< Tracer maintenance respiration for root for the CTEM pfts both (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    real, intent(in) :: tracerRML(:,:)        !< Tracer leaf maintenance respiration (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    real, intent(in) :: tracerGPP(ilg,icc)    !< Tracer GPP (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    ...
    real, intent(inout) :: tracerStemMass(ilg,icc) !< Tracer mass in the stem for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$
    real, intent(inout) :: tracerRootMass(ilg,icc) !< Tracer mass in the root for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$
    real, intent(inout) :: tracerGLeafMass(ilg,icc) !< Tracer mass in the green leaf pool for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$
    real, intent(inout) :: tracerBLeafMass(ilg,icc) !< Tracer mass in the brown leaf pool for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$
    ...
    real, intent(out) :: tracerReproCost(ilg,icc)   !< Tracer cost of making reproductive tissues, only non-zero when NPP is positive (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)

    ! Local
    integer :: i, j
    character(8) :: pftkind
    real :: gppvgstp(ilg,icc) !< GPP converted to units of \f$(kg C/m^2)\f$
    real :: nppvgstp(ilg,icc) !< NPP converted to units of \f$(kg C/m^2)\f$
    real :: rmlvgstp(ilg,icc) !< Leaf maintenance respiration converted to units of \f$(kg C/m^2)\f$
    real :: rmsvgstp(ilg,icc) !< Stem maintenance respiration converted to units of \f$(kg C/m^2)\f$
    real :: rmrvgstp(ilg,icc) !< Root maintenance respiration converted to units of \f$(kg C/m^2)\f$
    real :: tracerNPPstp      !< Tracer NPP converted to units of, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracerRMLstp      !< Tracer leaf maintenance respiration converted to units of 14C: \f$ng ^{14}C/m^2\f$
    real :: tracerRMSstp      !< Tracer stem maintenance respiration converted to units of 14C: \f$ng ^{14}C/m^2\f$
    real :: tracerRMRstp      !< Tracer root maintenance respiration converted to units of 14C: \f$ng ^{14}C/m^2\f$
    real :: ntchlTracer       !< tracer fluxes for each pft: Net change in leaf biomass, amol 14CO2/m2.sec
    real :: ntchsTracer       !< tracer fluxes for each pft: Net change in stem biomass, amol 14CO2/m2.sec
    real :: ntchrTracer       !< tracer fluxes for each pft: Net change in root biomass, amol 14CO2/m2.sec
    ...
    real :: oldvalleaf, oldvalstem, oldvalroot, temp, chg !< Temp variables used to keep track of biomass prior to updates.
    ...
    do j = 1,icc ! loop 600
      ...
      do i = il1,il2 ! loop 610
        ...
        ! remove that from NPP. Convert units of respiration from amol 14CO2/m2/s
        ! to nkg 14C/m2
        if (useTracer /= 'None') then
          ...
          tracerRMSstp = tracerRMS(i,j) * convertUnits
          tracerRMRstp = tracerRMR(i,j) * convertUnits
        end if
        ...
        if (lfstatus(i,j) /= 4) then ! real leaves exist
          if (nppvgstp(i,j) > 0.0) then ! and there is C to allocate.
            ...
            if (useTracer /= 'None') then ! If tracer in use,do the same for the tracer. Note
              ! the tracerGPP units are converted in place.
              ntchlTracer = -tracerRMLstp + afrleaf(i,j) * (tracerGPP(i,j) * convertUnits)
              ntchsTracer = -tracerRMSstp + afrstem(i,j) * (tracerGPP(i,j) * convertUnits)
              ntchrTracer = -tracerRMRstp + afrroot(i,j) * (tracerGPP(i,j) * convertUnits)
            end if
          ...
          if (useTracer /= 'None') then
            ...
            ntchsTracer = -tracerRMSstp
            ntchrTracer = -tracerRMRstp
          end if
        ...
        select case (pftkind)
        case ('NdlEvgTr' ,'CNEvgTr','INEvgTr', 'NdlDcdTr', 'BdlEvgTr','BdlDCoTr', 'BdlDDrTr', &
        ...
        if (useTracer /= 'None') then
          oldvalleaf = gleafmas(i,j)    
          oldvalstem = stemmass(i,j)
          oldvalroot = rootmass(i,j)
        end if
        ...
        !! the below assigns priority to ntchlveg and applies ntchlveg and the ns -> s 
        !! while maintaining mass balance the logic here is that net changes in vegetation
        !! biomass must be satisfied ideally with carbon from the more labile non structural pool
        !! if the vegetation is rapidly decreasing in size and the non structural pool is depleted
        !! net changes in vegetaiton biomass will come from the structural pool and investment of 
        !! more labile non structural carbon into the less labile structural carbon pool will be paused
        !! until the non structural pool re accumulates these routines will resolve any such
        !! inconsitensies and warn the user while ensuring mass balance

        !> First apply ntchlveg
        !> check is ntchlveg(i,j) is negative and there is not enough ns leaf mass to flux
        if (ntchlveg(i,j) < 0.0 .and. (gleafmas_ns(i,j) + ntchlveg(i,j)) < 0.0) then
          !> case 1
          !> check if the remainder can be taken from the s leaf mass
          if (((gleafmas_ns(i,j) + gleafmas_s(i,j)) + ntchlveg(i,j)) < 0.0) then
            !> case 1a
            !write(6,*)'warning ntchlveg(i,j) exceeds total leaf mass at i = ',i,' for pft = ',j
            !here we zero the pools and reduce the flux
            ntchlveg(i,j) = -(gleafmas_s(i,j) + gleafmas_ns(i,j))
            gleafmas_s(i,j) = 0.0
            gleafmas_ns(i,j) = 0.0
          else
            !> case 1b
            !> it can so take from the s pool and zero the ns pool
            gleafmas_s(i,j) = gleafmas_s(i,j) + (ntchlveg(i,j) + gleafmas_ns(i,j))  
            gleafmas_ns(i,j) = 0.0
          end if
        else
          !> case 2 (default)
          !> if not run the standard case (there is enough ns leaf mass to flux or mass is being added)
          gleafmas_ns(i,j) = gleafmas_ns(i,j) + ntchlveg(i,j)
        end if

        !> Second deal with leafns2s_stp(i,j)
        !>make sure leafns2s_stp(i,j) is zero or greater
        if (leafns2s_stp(i,j) < 0.0) leafns2s_stp(i,j) = 0.0

        !> Recreate the total leaf pool in prep for the next step
        gleafmas(i,j) = gleafmas_ns(i,j) + gleafmas_s(i,j)

        !>check if there is enough gleafmas_ns to accommodate leafns2s_stp
        if (gleafmas_ns(i,j) < leafns2s_stp(i,j)) then
          !> case 1
          !>if not take what ever there is respecting min_ns2t_l(sort(j)) and reduce the flux 
          if(gleafmas_ns(i,j) == 0.0) then
            !> case 1b-1 the ns pool is already empty
            !write(6,*)'warning gleafmas_ns(i,j) = 0.0 at i = ',i,' for pft = ',j,' no ns->s flux can occur'
            leafns2s_stp(i,j) = 0.0
          else if (gleafmas_ns(i,j) < (min_ns2t_l(sort(j)) * gleafmas(i,j))) then
            if(ntchlveg(i,j) < 0.0 .and. -ntchlveg(i,j) > ((min_ns2t_l(sort(j)) * gleafmas(i,j)) - gleafmas_ns(i,j))) then
              !> case 1b-2 there's not enough ns green leaf mass, but we can revise the way ntchlveg(i,j) was applied
              !write(6,*)'warning ntchlveg(i,j) revised at i = ',i,' for pft = ',j
              gleafmas_s(i,j) = gleafmas_s(i,j) -  ((min_ns2t_l(sort(j)) * gleafmas(i,j)) - gleafmas_ns(i,j))
              gleafmas_ns(i,j) = gleafmas_ns(i,j) + ((min_ns2t_l(sort(j)) * gleafmas(i,j)) - gleafmas_ns(i,j))
              leafns2s_stp(i,j) = 0.0
            else if(ntchlveg(i,j) > 0.0) then
              !write(6,*)'warning min_ns2t_l(sort(j)) not satisfied at i = ',i,' for pft = ',j,'but ntchlveg is positive ... continuing'
              leafns2s_stp(i,j) = 0.0
            else
              !> case 1b-3 there's not enough ns green leaf mass and we can't revise so warn 
              !write(6,*)'warning min_ns2t_l(sort(j)) not satisfied at i = ',i,' for pft = ',j
              leafns2s_stp(i,j) = 0.0
            end if
          else
            !> case 1c take whatever can be taken
            leafns2s_stp(i,j) = gleafmas_ns(i,j) - (min_ns2t_l(sort(j)) * gleafmas(i,j))
            gleafmas_s(i,j)  = gleafmas_s(i,j) + (gleafmas_ns(i,j) - (min_ns2t_l(sort(j)) * gleafmas(i,j)))
            gleafmas_ns(i,j) = (min_ns2t_l(sort(j)) * gleafmas(i,j))
          end if
        else
          !> case 2
          !> if there is then run the standard case
          gleafmas_ns(i,j) = gleafmas_ns(i,j) - leafns2s_stp(i,j)
        ...
        !> Then apply ntchrveg
        !> check is ntchrveg(i,j) is negative and there is not enough ns root mass to flux
        if (ntchrveg(i,j) < 0.0 .and. (rootmass_ns(i,j) + ntchrveg(i,j)) < 0.0) then
          !> case 1
          !> check if the remainder can be taken from the s root mass
          if (((rootmass_ns(i,j) + rootmass_s(i,j)) + ntchrveg(i,j)) < 0.0) then
            !> case 1a
            !write(6,*)'warning ntchrveg(i,j) exceeds total root mass at i = ',i,' for pft = ',j
            !alt method there's not enough so zero the pools and reduce the flux
            ntchrveg(i,j) = -(rootmass_s(i,j) + rootmass_ns(i,j))
            rootmass_s(i,j) = 0.0
            rootmass_ns(i,j) = 0.0
          else
            !> case 1b
            !> it can so take from the s pool and zero the ns pool
            rootmass_s(i,j) = rootmass_s(i,j) + (ntchrveg(i,j) + rootmass_ns(i,j))  
            rootmass_ns(i,j) = 0.0
          end if
        else
          !> case 2 (default)
          !> if not run the standard case (there is enough ns root mass to flux or mass is being added)
          rootmass_ns(i,j) = rootmass_ns(i,j) + ntchrveg(i,j)
        end if

        !> Second deal with rootns2s_stp(i,j)
        !>make sure rootns2s_stp(i,j) is zero or greater
        if (rootns2s_stp(i,j) < 0.0) rootns2s_stp(i,j) = 0.0

        !> Recreate the total root pool in prep for the next step
        rootmass(i,j) = rootmass_ns(i,j) + rootmass_s(i,j)

        !>check if there is enough rootmass_ns to accommodate rootns2s_stp
        if (rootmass_ns(i,j) < rootns2s_stp(i,j)) then
          !> case 1
          !>if not take what ever there is respecting min_ns2t_r(sort(j)) and reduce the flux 
          if(rootmass_ns(i,j) == 0.0) then
            !> case 1b-1 the ns pool is already empty
            !write(6,*)'warning rootmass_ns(i,j) = 0.0 at i = ',i,' for pft = ',j,' no ns->s flux can occur'
            rootns2s_stp(i,j) = 0.0
          else if (rootmass_ns(i,j) < (min_ns2t_r(sort(j)) * rootmass(i,j))) then
            if(ntchrveg(i,j) < 0.0 .and. -ntchrveg(i,j) > ((min_ns2t_r(sort(j)) * rootmass(i,j)) - rootmass_ns(i,j))) then
              !> case 1b-2 there's not enough ns green root mass, but we can revise the way ntchrveg(i,j) was applied
              !write(6,*)'warning ntchrveg(i,j) revised at i = ',i,' for pft = ',j
              rootmass_s(i,j) = rootmass_s(i,j) -  ((min_ns2t_r(sort(j)) * rootmass(i,j)) - rootmass_ns(i,j))
              rootmass_ns(i,j) = rootmass_ns(i,j) + ((min_ns2t_r(sort(j)) * rootmass(i,j)) - rootmass_ns(i,j))
              rootns2s_stp(i,j) = 0.0
            else if(ntchrveg(i,j) > 0.0) then
              !write(6,*)'warning min_ns2t_r(sort(j)) not satisfied at i = ',i,' for pft = ',j,' but ntchrveg is positive ... continuing'
              rootns2s_stp(i,j) = 0.0
            else
              !> case 1b-3 there's not enough ns root mass and we can't revise so warn
              write(6,*)'min_ns2t_r(sort(j)) not satisfied at i = ',i,' for pft = ',j
              rootns2s_stp(i,j) = 0.0
            end if
          else
            !> case 1c take whatever can be taken
            rootns2s_stp(i,j) = rootmass_ns(i,j) - (min_ns2t_r(sort(j)) * rootmass(i,j))
            rootmass_s(i,j)  = rootmass_s(i,j) + (rootmass_ns(i,j) - (min_ns2t_r(sort(j)) * rootmass(i,j)))
            rootmass_ns(i,j) = (min_ns2t_r(sort(j)) * rootmass(i,j))
          end if
        else
          !> case 2
          !> if there is then run the standard case
          rootmass_ns(i,j) = rootmass_ns(i,j) - rootns2s_stp(i,j)
        ...
        !> First apply ntchsveg
        !> check is ntchsveg(i,j) is negative and there is not enough ns stem mass to flux
        if (ntchsveg(i,j) < 0.0 .and. (stemmass_ns(i,j) + ntchsveg(i,j)) < 0.0) then
          !> case 1
          !> check if the remainder can be taken from the s stem mass
          if (((stemmass_ns(i,j) + stemmass_s(i,j)) + ntchsveg(i,j)) < 0.0) then
            !> case 1a
            !write(6,*)'warning ntchsveg(i,j) exceeds total stem mass at i = ',i,' for pft = ',j
            !there's not enough so zero the pools and reduce the flux
            ntchsveg(i,j) = -(stemmass_s(i,j) + stemmass_ns(i,j))
            stemmass_s(i,j) = 0.0
            stemmass_ns(i,j) = 0.0
          else
            !> case 1b
            !> it can so take from the s pool and zero the ns pool
            stemmass_s(i,j) = stemmass_s(i,j) + (ntchsveg(i,j) + stemmass_ns(i,j))  
            stemmass_ns(i,j) = 0.0
          end if
        else
          !> case 2 (default)
          !> if not run the standard case (there is enough ns stem mass to flux or mass is being added)
          stemmass_ns(i,j) = stemmass_ns(i,j) + ntchsveg(i,j)
        end if

        !> Second deal with stemns2s_stp(i,j)
        !>make sure stemns2s_stp(i,j) is zero or greater
        if (stemns2s_stp(i,j) < 0.0) stemns2s_stp(i,j) = 0.0

        !> Recreate the total stem pool in prep for the next step
        stemmass(i,j) = stemmass_ns(i,j) + stemmass_s(i,j)

        !>check if there is enough stemmass_ns to accommodate stemns2s_stp
        if (stemmass_ns(i,j) < stemns2s_stp(i,j)) then
          !> case 1
          !>if not take what ever there is respecting min_ns2t_s(sort(j)) and reduce the flux 
          if(stemmass_ns(i,j) == 0.0) then
            !> case 1b-1 the ns pool is already empty
            stemns2s_stp(i,j) = 0.0
          else if (stemmass_ns(i,j) < (min_ns2t_s(sort(j)) * stemmass(i,j))) then
            if(ntchsveg(i,j) < 0.0 .and. -ntchsveg(i,j) > ((min_ns2t_s(sort(j)) * stemmass(i,j)) - stemmass_ns(i,j))) then
              !> case 1b-2 there's not enough ns green stem mass, but we can revise the way ntchsveg(i,j) was applied
              stemmass_s(i,j) = stemmass_s(i,j) -  ((min_ns2t_s(sort(j)) * stemmass(i,j)) - stemmass_ns(i,j))
              stemmass_ns(i,j) = stemmass_ns(i,j) + ((min_ns2t_s(sort(j)) * stemmass(i,j)) - stemmass_ns(i,j))
              stemns2s_stp(i,j) = 0.0
            else if(ntchsveg(i,j) > 0.0) then
              !write(6,*)'min_ns2t_s(sort(j)) not satisfied at i = ',i,' for pft = ',j,' ntchsveg is positive ... continuing'
              stemns2s_stp(i,j) = 0.0
            else
              !> case 1b-3 there's not enough ns stem mass and we can't revise so warn
              stemns2s_stp(i,j) = 0.0
            end if
          else
            !> case 1c take whatever can be taken
            stemns2s_stp(i,j) = stemmass_ns(i,j) - (min_ns2t_s(sort(j)) * stemmass(i,j))
            stemmass_s(i,j)  = stemmass_s(i,j) + (stemmass_ns(i,j) - (min_ns2t_s(sort(j)) * stemmass(i,j)))
            stemmass_ns(i,j) = (min_ns2t_s(sort(j)) * stemmass(i,j))
          end if
        else
          !> case 2
          !> if there is then run the standard case
          stemmass_ns(i,j) = stemmass_ns(i,j) - stemns2s_stp(i,j)
...
1901      format(a11,f12.8)
        ...
        !!
        if (bleafmas(i,j) < zero) then
          bleafmas(i,j) = 0.0
          if (useTracer /= 'None') tracerBLeafMass(i,j) = 0.0
        end if 
        ...
        if (gleafmas(i,j) < zero) then 
          ...
          if (useTracer /= 'None') tracerGLeafMass(i,j) = 0.0
        end if 
        ! stem
        ...
        if (stemmass(i,j) < zero) then 
          ...
          if (useTracer /= 'None') tracerStemMass(i,j) = 0.0
        end if 
        ! root
        ...
        if (rootmass(i,j) < zero) then 
          ...
          if (useTracer /= 'None') tracerRootMass(i,j) = 0.0
        end if 

        ! To ensure the tracer can balance, adjust it by how much the total
        ! gleafmas, stemmass, and rootmass pools have changed.
        if (useTracer /= 'None') then
          if (gleafmas(i,j) > zero) then 
            temp = tracerGLeafMass(i,j) !save old value for check
            tracerGLeafMass(i,j) = tracerGLeafMass(i,j) + ntchlTracer

            !Check is the nonstructural/structural changes caused the change in the 
            ! normal pools to be much different, and if so, correct.
            if (tracerGLeafMass(i,j) > 0.) then
              if (temp > 0.) then ! We can only do this check if we are not starting from
                ! empty tracer pools. 
                ! If we originally had any tracer, then check for proportional change
                chg = abs((tracerGLeafMass(i,j)/ temp) - (gleafmas(i,j) / oldvalleaf))
                if (chg > 1.E-3) then ! limit is 0.1%
                  ! Rescale the tracer by the amount that the total C pools were 
                  ! adjusted due to the NS/S changes.
                  tracerGLeafMass(i,j) = temp  *  gleafmas(i,j) / oldvalleaf
                end if
              end if 
            else ! no tracer there
              tracerGLeafMass(i,j) = 0.
            end if 
          else ! no C in the total C pool so set tracer also to zero.
            tracerGLeafMass(i,j) = 0.
          end if 

          if (rootmass(i,j) > zero) then 
            temp = tracerRootMass(i,j) !save old value for check
            tracerRootMass(i,j) = tracerRootMass(i,j) + ntchrTracer

            !Check is the nonstructural/structural changes caused the change in the 
            ! normal pools to be much different, and if so, correct.
            if (tracerRootMass(i,j) > 0.) then
              if (temp > 0.) then ! We can only do this check if we are not starting from
                ! empty tracer pools. 
                ! If we originally had any tracer, then check for proportional change
                chg = abs((tracerRootMass(i,j)/ temp) - (rootmass(i,j) / oldvalroot))
                if (chg > 1.E-3) then ! limit is 0.1%
                  ! Rescale the tracer by the amount that the total C pools were 
                  ! adjusted due to the NS/S changes.
                  tracerRootMass(i,j) = temp  *  rootmass(i,j) / oldvalroot
                end if
              end if 
            else ! no tracer there
          ...
          if (stemmass(i,j) > zero) then 
            temp = tracerStemMass(i,j) !save old value for check
            tracerStemMass(i,j) = tracerStemMass(i,j) + ntchsTracer

            !Check is the nonstructural/structural changes caused the change in the 
            ! normal pools to be much different, and if so, correct.
            if (tracerStemMass(i,j) > 0.) then
              if (temp > 0.) then ! We can only do this check if we are not starting from
                ! empty tracer pools. 
                ! If we originally had any tracer, then check for proportional change
                chg = abs((tracerStemMass(i,j)/ temp) - (stemmass(i,j) / oldvalstem))
                if (chg > 1.E-3) then ! limit is 0.1%
                  !Rescale the tracer by the amount that the total C pools were 
                  !adjusted due to the NS/S changes. 
                  tracerStemMass(i,j) = temp  *  stemmass(i,j) / oldvalstem
                end if
              end if 
            else ! no tracer there
