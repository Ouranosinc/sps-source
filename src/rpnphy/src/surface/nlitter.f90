...
subroutine nlitter(il1, il2, sort, fcancmx, lfstatus, iday, & !In
  ...
  real,intent(in), dimension(ilg)     :: litrfallmoss       !< moss litter fall (kgC/m2/timestep)
  character(8), intent(in) :: mossPresent(ilg)              !< Type of moss present (else 'None')
  real,intent(in), dimension(ilg)     :: Cmossmas           !< moss biomass C pool (kgC/m2)
  ...
  real,intent(out),dimension(ilg)     :: nlitrveg_moss      !< moss nitrogen litter fall rate, \f$g N m^{-2} day^{-1}\f$
  ...
  real :: mossltr_fr   !< fraction of moss becoming litter 
  ...
  real,dimension(ilg)     :: Nmossmas_in        !< previous moss nitrogen, \f$g N m^{-2}\f$
  real,dimension(ilg)     :: litrmsmossN_in     !< previous litter nitrogen for moss, \f$g N m^{-2}\f$
  ...
  !> Perform the moss litter calculations, they are similar in form to the vascular pools
  do i = il1, il2
   if (mossPresent(i) /= 'None') then
      if (Cmossmas(i) > 0.) then
        mossltr_fr =  min(litrfallmoss(i) / (litrfallmoss(i) + Cmossmas(i)), 1.0)
      else 
        mossltr_fr = 0.
      end if 
      nlitrveg_moss(i) = mossltr_fr * Nmossmas(i)
   end if 
  end do 
  ...
  do j= 1,icc
     ...
     do i = il1,il2
        ...
        if (ngleafmas_in(i,j) > 0.0) then
           ...
           case ('NdlEvgTr','CNEvgTr','INEvgTr', &
            'NdlDcdTr','BdlEvgTr','BdlDCoTr','BdlDDrTr',&
  ...
  !> Now update the moss pools and check conservation
  do i = il1, il2
    if (mossPresent(i) /= 'None') then 
      ! Living moss pool
      Nmossmas_in(i) = Nmossmas(i)
      Nmossmas(i) = Nmossmas(i) - nlitrveg_moss(i)

      !! Checking conservation for Nmossmas
      if ((Nmossmas_in(i) - Nmossmas(i) - nlitrveg_moss(i))  &
                                            > ntolerance) then
         write(*,*)'imbalance in Nmossmas at (i)=',i
         call errorHandler('nlitter', -15)
      end if

      ! Moss litter pool
      litrmsmossN_in(i) = litrmsmossN(i)
      litrmsmossN(i) = litrmsmossN(i) + nlitrveg_moss(i)

      !! Checking conservation for litrmsmossN
      if ((litrmsmossN_in(i) - litrmsmossN(i) + nlitrveg_moss(i))  &
                                             > ntolerance         &
                                             ) then
         write(*,*)'imbalance in litrmsmossN at (i)=',i
         write(*,*)'litrmsmossN_in(i) = ',litrmsmossN_in(i)
         write(*,*)'litrmsmossN(i)    = ',litrmsmossN(i)
         write(*,*)'nlitrveg_moss(i)   = ',nlitrveg_moss(i)
         call errorHandler('nlitter', -16)
      end if
    end if 
  end do
