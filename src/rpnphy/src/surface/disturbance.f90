...
contains
  ...
  subroutine disturbance (thliq, THLW, THFC, THPOR, uwind, useTracer, peatlandType, & ! In
                 ...
                 fiengat, burnarea, num_fires, & ! Out
    ...
    use classicParams, only : ignd, icc, ican, zero, kk, pi, c2dom, crop, &
                              ...
                              f0, maxsprd, maxsprd_FWI, frco2glf, frco2blf, &
                              frltrglf, frltrblf, frco2stm, frltrstm, frco2rt, frltrrt, &
                              frltrbrn, emif_co2, emif_co, emif_ch4, emif_nmhc, emif_h2, &
                              emif_nox, emif_n2o, emif_nh3, emif_pm25, emif_tpm, emif_tc, emif_bc, emif_oc, emif_so2, &
                              grass, extnmois_veg, extnmois_duff, iccp1, nol2pfts, ctempfts, c_switch, &
                              switch_lightning, switch_cffeps_plume
    ...
    real, intent(in) :: THPOR(ilg,ignd)    !< pore volume
    ...
    real, intent(in)  :: elcggat(ilg)      !< cloud-to-ground lightning from CanAM \f$[flashes/(cm^2.sec)]\f$
    ...
    real, intent(out) :: emit_bc(ilg,icc)  !< black carbon fire emissions \f$[kg{species}/m2/s]\f$
    real, intent(out) :: emit_oc(ilg,icc)  !< organic carbon
    real, intent(out) :: emit_so2(ilg,icc) !< sulfur dioxide
    real, intent(out) :: burnarea(ilg)     !< total area burned, \f$km^2\f$
    real, intent(out) :: fiengat(ilg,icc)  !< fire plume energy using CFFEPS method per PFT \f$[J]\f$
    real, intent(out) :: num_fires(ilg,icc)  !< number of fires per PFT in each grid cell
    
    ! variables for FWI calculations S.R.C.
    real, dimension(ilg), intent(in) :: precip             !< daily precipitation (mm/day)
    real, dimension(ilg), intent(in) :: ta                  !< air temp, K
    real, dimension(ilg) :: windtot !< wind speed from u and v components in km/h
    real, dimension(ilg) :: tempc !< temp in C
    real, intent(in) :: QACGAT(ilg)     !< specific humidity
    real, intent(in) :: PRESGAT(ilg)    !< atospheric pressure
    real, intent(out) :: isi(ilg)     !< Initial Spread Index 
    real, intent(out) :: bui(ilg)     !< Build-Up Index 
    real, intent(out) :: fwi(ilg)     !< Fire Weather Index
    real, intent(inout) :: ffmc(ilg)     !<  Fine Fuel Moisture Code
    real, intent(inout) :: dmc(ilg)     !<  Duff Moisture Code
    real, intent(inout) :: dc(ilg)     !< Drought Code
    integer, intent(in) :: IMONTH        !< Month of the year simulation is in.
    real, dimension(ilg), intent(in) :: NoonTempc
    ...
    real :: tracer_glfltrdt(ilg,icc)  !< tracer green leaf litter generated due to disturbance, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_blfltrdt(ilg,icc)  !< tracer brown leaf litter generated due to disturbance, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_stemltdt(ilg,icc)  !< tracer stem litter generated due to disturbance, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_rootltdt(ilg,icc)  !< tracer root litter generated due to disturbance, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_glcaemls(ilg,icc)  !< tracer green leaf carbon emission losses, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_blcaemls(ilg,icc)  !< tracer brown leaf carbon emission losses, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_stcaemls(ilg,icc)  !< tracer stem carbon emission losses, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_rtcaemls(ilg,icc)  !< tracer root carbon emission losses, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_ltrcemls(ilg,icc)  !< tracer litter carbon emissions losses, 14C: \f$ng ^{14}C/m^2\f$
    ...
    real :: tracerlitterTemp(ilg,icc) !< temporary total per pft/per tile tracer litter mass for calculation purposes
    
    real :: v_in  !< Exponent term of Canada specific supression function
 
    associate( &
    Ncycle_on => c_switch%Ncycle_on,                     & !< logical: true if nitrogen cycle is on. 
    perlayerfireON => c_switch%perlayerfireON                 & !< logical: true if per-layer fire is on. 
    )
    !
    ...
    do j = 1,icc
      do i = il1,il2
        ...
        fFireLitter(i,j) = 0.0
        fFireCsoil(i,j) = 0.0
        tracer_glfltrdt(i,j) = 0.0
        tracer_blfltrdt(i,j) = 0.0
        tracer_stemltdt(i,j) = 0.0
        tracer_rootltdt(i,j) = 0.0
        tracer_glcaemls(i,j) = 0.0
        tracer_blcaemls(i,j) = 0.0
        tracer_stcaemls(i,j) = 0.0
        tracer_rtcaemls(i,j) = 0.0
        tracer_ltrcemls(i,j) = 0.0
  ...
  if (dofire .or. prescribedFire) then
    ...
    !> do FWI calculations if fwiFire is being used
    if(fireSubmodule == 'FWI') then
      do i = il1,il2
        if (fsnow(i) == 0.) then
          snwct(i) = snwct(i) + 1
        else
        ...
        !< This counter tracks snow free days. As in Turner and Lawson 1978 \cite Turner1978-pk FWI is updated after three snow free days.
        if (snwct(i) > 3) then
          !< FWI is calculated using et values from the hottest point in the day as in Turner and Lawson 1978 \cite Turner1978-pk.
          call fwicalculation(NoonTempc(i),NoonWindtot(i),NoonPrecip(i),NoonRhum(i), & !in
                      isi(i), bui(i), fwi(i), & !out
                      ffmc(i), dmc(i), dc(i), IMONTH) !inout
        end if
      end do
    end if
    ...
    if (dofire) then !skip the burned area model if using prescribed fire
      ...
      do j = 1,icc
        do i = il1,il2
          if (.not. crop(j)) then !> don't allow it to bring in crops since they are not allowed to burn.
            ...
            if (perlayerfireON) then
            !! If per layer fire is on consider litter from the top layer only
              biomass(i,j) = gleafmas(i,j) + bleafmas(i,j) + stemmass(i,j) + litrmass(i,j,1)
            else
              biomass(i,j) = gleafmas(i,j) + bleafmas(i,j) + stemmass(i,j) + sum(litrmass(i,j,:))
            end if              
      ...
      !! do not allow fire so set betadrgt to 1.0
      !! for all soil layers (so that mterm_veg is 0), otherwise calculate as per normal.
      do i = il1,il2
        if (fsnow(i) == 0.) then
          do j = 1,ignd
            if (isand(i,j) > - 3 .and. peatlandType(i) == 'None') then !was isand > -2, but added the peatlandType check instead
      ...
      do j = 1,icc
        do i = il1,il2
          ...
          if (biomass(i,j) > 0. .and. fsnow(i) == 0.) then
            if (perlayerfireON) then
              !! If per layer fire is on consider only the first layer of litter mass
              duff_frac_veg(i,j) = (bleafmas(i,j)+litrmass(i,j,1)) / biomass(i,j)
            else
              !! If not consider all layers
          ...
          if (fcancmx(i,j) > zero) then
            if(fireSubmodule == 'FWI') then
              ...
              case ('NdlEvgTr','NdlDcdTr','INEvgTr', 'CNEvgTr','BdlEvgTr','BdlDCoTr','BdlDDrTr','GrassC3','GrassC4 ','Sedge','BdlEvgSh','BdlDCoSh') !All Trees
              !are assumed to have the same ignition probabilities similar to the standard model
                !For Blackmarr 1972 \cite Blackmarr1972-fq
                if (snwct(i) > 3) then
                  !mterm_veg(i,j) = min(1.0,max(0.0,(ffmc(i)-66.7)/(78.5-66.7))) !threshold version
                  mterm_veg(i,j) = exp(-25.2+0.34*ffmc(i))/(1+exp(-25.2+0.34*ffmc(i))) !logistic version
                else
                  mterm_veg(i,j) = 0.0
                end if
              case ('CropC3','CropC4')
                mterm_veg(i,j) = 0.0
              case default
                write(6,*)'unkown ctem PFT in disturbance'
                write(6,*)"i = ",i," j= ",j," PFT = ",ctempfts(j)
                call errorHandler('disturb',-10)
              end select
            else
              !> standard mterm per PFT
      ...
      if (switch_lightning) then
         ! use CanAM's new lightning scheme as lightning instead of the climatological input
           c2glgtng(il1:il2) = elcggat(il1:il2) * 3.154E17 ! factor to convert flashes/cm2/sec to flashes/km2/year ! [CW] I think CLASSIC expects per day.
      else
         !use climatological lightning
         ...
         c2glgtng(il1:il2)=lightng(il1:il2) ! FLAG FireMIP lightning is already C2G ! If using 'normal' lightning climatology use line below.
         ! c2glgtng(il1:il2) = 0.219913 * exp(0.0058899 * abs(currlat(il1:il2))) * lightng(il1:il2)
      end if
      ...
      do j = 1,icc
        do i = il1,il2
          if (fire_veg(i,j) ) then
            ...
            betmsprd_veg = (1.0 - min(1.0, (drgtstrs(i,j)/extnmois_veg) )) ** 2
            betmsprd_duff = (1.0 - min(1.0, (betadrgt(i,1)/extnmois_duff) )) ** 2
            smfunc_veg(i,j) = betmsprd_veg * (1.0 - duff_frac_veg(i,j)) + betmsprd_duff * duff_frac_veg(i,j)
            ...
            n = sort(j)
            if(fireSubmodule == 'FWI') then
              !> FWI based spread rates are calculated per PFT
              select case (ctempfts(j))
              case ('NdlEvgTr','NdlDcdTr','INEvgTr', 'CNEvgTr') ! Needleaf Trees
                if (snwct(i) > 3) then
                  !From forestry canada fire danger group 1992 \cite Canada_Fire_Danger_Group1992-bt eqns. 26, 54
                  sprdrate_veg(i,j) = maxsprd_FWI(n) * min(1.0,(((110.0*(1-exp(-0.0282*isi(i)))**1.5)/110.0)*((exp(50.0*log(0.7)*(((1.0/BUI(i))-(1.0/64.0)))))/1.32135)))
                else
                  sprdrate_veg(i,j) = 0.0
                end if
              case('BdlEvgTr','BdlDCoTr','BdlDDrTr','BdlEvgSh','BdlDCoSh') ! Broadleaf trees
                if (snwct(i) > 3) then
                  !From forestry canada fire danger group 1992 eqns. 26, 54
                  !once normalized these functions are very similar
                  sprdrate_veg(i,j) = maxsprd_FWI(n) * min(1.0,(((30.0*(1-exp(-0.0232*isi(i)))**1.6)/30.0)*((exp(50.0*log(0.9)*(((1.0/BUI(i))-(1.0/32.0)))))/1.17895)))
                else
                  sprdrate_veg(i,j) = 0.0
                end if
              case('GrassC3','GrassC4 ','Sedge')
                if (snwct(i) > 3) then
                  !From forestry canada fire danger group 1992 eqns. 26, 54
                  !again normalized grass spread rated, it lack BUI
                  !but take brown grass into account
                  if ((gleafmas(i,j) + bleafmas(i,j)) > 0.0) then
                    sprdrate_veg(i,j) = &
                    ((maxsprd_FWI(n) * min(1.0,((190.0*(1-exp(-0.0310*isi(i)))**1.4)/190.0))) * &
                    (gleafmas(i,j)/(gleafmas(i,j) + bleafmas(i,j)))) + & !for the green fraction
                    (((maxsprd_FWI(n) * 1.3) * min(1.0,((250.0*(1-exp(-0.0350*isi(i)))**1.7)/250.0))) * &
                    (bleafmas(i,j)/(gleafmas(i,j) + bleafmas(i,j)))) ! for the brown fraction
                  else
                    sprdrate_veg(i,j) = 0.0
                  end if
                else
              ...
              case default
                write(6,*)'unkown ctem PFT in disturbance'
                write(6,*)"i = ",i," j= ",j," PFT = ",ctempfts(j)
                call errorHandler('disturb',-11)
              end select
            else
              !> standard spread rate is calculated per PFT
            ...
            if(fireSubmodule == 'FWI') then
              !> canada specific supression function
              v_in = (log(-(log(1-exp(-pi))/pi)))/(log(0.001/0.1))
              extnprob(i) = max(0.0,1.0 - exp(-pi * (popdin(i) / 0.1) ** v_in))
            else
            ...
            !>
            !! NOTE: This is not used in CLASSIC in general and only kept for testing purposes. 
            !! It is to be used at your own risk and needs to be carefully evaluated, if used.
            !! [CW] Note: uncommenting this and using it for fire energy/plume height calculations is desired.
            !!
            !! Calculate natural and anthropogenic ignitions/km2/day.
            !! The constant 0.25 assumes that not all c2g lightning hits causes ignitions, only 0.25 of them do.
            !! The constant (1/365) converts c2g lightning from flashes/km2/year to flashes/km2/day.
            !! MAKE SURE LIGHTNING IS IN UNITS OF FLASHES/KM2/YEAR
            !!
            !! Eqs. (4) and (5) of Li et al. 2012 doi:10.5194/bg-9-2761-2012 + also see corrigendum

            natural_ignitions(i) = c2glgtng(i) * 0.25 * (1.0/365.)
            if (popdin(i) > 0.0) then
              anthro_ignitions(i) = 9.72E-4 * (1.0/30.4) * popdin(i) * (6.8 * (popdin(i)**(-0.6)))
            else
              anthro_ignitions(i) = 0.0
            end if

            !> Calculate fire suppression also as a function of population density.
            !! Li et al. (2012)'s formulation is quite similar to what we already use based
            ...
            !> Calculate total number of ignitions based on natural and anthropogenic ignitions
            !! for the whole grid cell.

            num_ignitions(i,j) = ( natural_ignitions(i) + anthro_ignitions(i) ) * fcancmx(i,j) * grclarea(i)

            !> Finally, calculate the number of fires, noting that not all ignitions turn into fire
            !! because moisture and biomass may not allow that to happen, and some of those
            !! will be suppressed due to fire fighting efforts.

            ! default:
            ! num_fires(i,j) = num_ignitions(i,j)*(1-fire_supp(i))*bterm_veg(i,j)*mterm_veg(i,j)
            ! suggested to try by Gesa (extnprob instead of fire_supp):
            num_fires(i,j) = num_ignitions(i,j)*(1-extnprob(i))*bterm_veg(i,j)*mterm_veg(i,j)

            !> ----------------------- Number of fire calculations ----------------------//
            !!          ------- Area burned based on number of fire calculations ----------------------\\

            !!         The constant 4 is supposed to address the fact that Li et al. (2012) suggested to
            !!         double the fire spread rates. However, if we do that, then our usual calculations
            !!         based on CTEM's original parameterization will be affected. Rather than doing that,
            ...
            !! If area burned is greater than the area of the PFT, set it to the area of the PFT.
      ...
      do j = 1,icc
        ...
        do i = il1,il2
          if (pftareab(i,j) > zero) then
            ...
            if (useTracer /= 'None') tracer_glfltrdt(i,j) = frltrglf(n) * &
                                              tracerGLeafMass(i,j) * fractionPFTburned
            ...
            if (useTracer /= 'None') tracer_blfltrdt(i,j) = frltrblf(n) * &
                                              tracerBLeafMass(i,j) * fractionPFTburned
            stemltdt(i,j) = frltrstm(n) * stemmass(i,j) * fractionPFTburned
            if (useTracer /= 'None') tracer_stemltdt(i,j) = frltrstm(n) * &
                                              tracerStemMass(i,j) * fractionPFTburned
            ...
            if (useTracer /= 'None') tracer_rootltdt(i,j) = frltrrt(n)  * &
                                              tracerRootMass(i,j) * fractionPFTburned
            ...
            if (useTracer /= 'None') tracer_glcaemls(i,j) = frco2glf(n) * tracerGLeafMass(i,j) * fractionPFTburned
            ...
            if (useTracer /= 'None') tracer_blcaemls(i,j) = frco2blf(n) * tracerBLeafMass(i,j) * fractionPFTburned
            stcaemls(i,j) = frco2stm(n) * stemmass(i,j) * fractionPFTburned
            if (useTracer /= 'None') tracer_stcaemls(i,j) = frco2stm(n) * tracerStemMass(i,j) * fractionPFTburned
            ...
            if (useTracer /= 'None') tracer_rtcaemls(i,j) = frco2rt(n)  * tracerRootMass(i,j) * fractionPFTburned
            rtcaemls_ns(i,j) = frco2rt(n)  * rootmass_ns(i,j) * fractionPFTburned
            rtcaemls_s(i,j)  = frco2rt(n)  * rootmass_s(i,j)  * fractionPFTburned
            ! Burned litter
          if (perlayerfireON) then
            !! If per layer fire is on burn litter from the top layer only
            ltrcemls(i,j)= frltrbrn(n) * litrmass(i,j,1) * fractionPFTburned
            if (useTracer /= 'None') tracer_ltrcemls(i,j)= frltrbrn(n) * &
                                            tracerLitrMass(i,j,1) * fractionPFTburned
          else
            !! If not consider the bulk pool across all layers
            ltrcemls(i,j)= frltrbrn(n) * sum(litrmass(i,j,:)) * fractionPFTburned
            if (useTracer /= 'None') tracer_ltrcemls(i,j)= frltrbrn(n) * &
                                            sum(tracerLitrMass(i,j,:)) * fractionPFTburned
          end if
          ! end FLAG 

          ! Calculate the fFireCveg, fFireLitter and fFireCsoil

          ! fFireCveg is only from live pools:
          fFireCveg(i,j) = glcaemls(i,j) + stcaemls(i,j) + rtcaemls(i,j) 

          ! fFireLitter considers the aboveground necromass burned, so brown leaves and 
          ! layer 1 of the litter pool:
          fFireLitter(i,j) = blcaemls(i,j) + frltrbrn(n) * litrmass(i,j,1) * fractionPFTburned

          ! fFireCsoil is the soil C consumed, we then include the litter pool from layers 2 to ignd,
          ! once the soil C pool can burn, that can be added to this:
          fFireCsoil(i,j) = frltrbrn(n) * sum(litrmass(i,j,2:ignd)) * fractionPFTburned
            ...
            if (useTracer /= 'None') tracerGLeafMass(i,j) = tracerGLeafMass(i,j) - &
                                          tracer_glfltrdt(i,j) - tracer_glcaemls(i,j)
            bleafmas(i,j) = bleafmas(i,j) - blfltrdt(i,j) - blcaemls(i,j)
            if (useTracer /= 'None') tracerBLeafMass(i,j) =  tracerBLeafMass(i,j) - &
                                            tracer_blfltrdt(i,j) - tracer_blcaemls(i,j)
            ...
            if (useTracer /= 'None') tracerStemMass(i,j) = tracerStemMass(i,j) - &
                                            tracer_stemltdt(i,j) - tracer_stcaemls(i,j)
            ...
            if (useTracer /= 'None') tracerRootMass(i,j) = tracerRootMass(i,j) - &
                                            tracer_rootltdt(i,j) - tracer_rtcaemls(i,j)
            ...
            if (perlayerfireON) then
              ! With per layer fire the burned litter is generated and then placed on the top litter layer only
              litrmass(i,j,1) = litrmass(i,j,1) + glfltrdt(i,j) + blfltrdt(i,j) + stemltdt(i,j) &
                            + rootltdt(i,j) * rmatctem(i,j,1) - ltrcemls(i,j)
              if (useTracer /= 'None') tracerLitrMass(i,j,1) = tracerLitrMass(i,j,1) + tracer_glfltrdt(i,j) + &
                                        tracer_blfltrdt(i,j) + tracer_stemltdt(i,j) + &
                                        tracer_rootltdt(i,j) * rmatctem(i,j,1) - &
                                        tracer_ltrcemls(i,j)
              do k = 2,ignd
                litrmass(i,j,k) = litrmass(i,j,k) + rootltdt(i,j) * rmatctem(i,j,k)
                if (useTracer /= 'None') tracerLitrMass(i,j,k) = tracerLitrMass(i,j,k) + tracer_rootltdt(i,j) * rmatctem(i,j,k)
              end do
            else
              ...
              if (useTracer /= 'None') then
                tracerlitterTemp = sum(tracerLitrMass(i,j,:))
                if (tracerlitterTemp(i,j) > 0.) then
              
                  tracerLitrMass(i,j,1) = tracerLitrMass(i,j,1) + tracer_glfltrdt(i,j) + &
                                          tracer_blfltrdt(i,j) + tracer_stemltdt(i,j) + &
                                          tracer_rootltdt(i,j) * rmatctem(i,j,1) - &
                                          tracer_ltrcemls(i,j) * (tracerLitrMass(i,j,1)/tracerlitterTemp(i,j))
                else 
                  tracerLitrMass(i,j,1) = tracerLitrMass(i,j,1) + tracer_glfltrdt(i,j) + &
                                          tracer_blfltrdt(i,j) + tracer_stemltdt(i,j) + &
                                          tracer_rootltdt(i,j) * rmatctem(i,j,1) 
              ...
              do k = 2,ignd
                if (litterTemp(i,j) > 0.0) then
                  litrmass(i,j,k) = litrmass(i,j,k) + rootltdt(i,j) * rmatctem(i,j,k) - &
                                      ltrcemls(i,j) * (litrmass(i,j,k)/litterTemp(i,j))
                else
                  litrmass(i,j,k) = litrmass(i,j,k) + rootltdt(i,j) * rmatctem(i,j,k)
                end if
                if (useTracer /= 'None') then
                  if (tracerlitterTemp(i,j) > 0.) then
                    tracerLitrMass(i,j,k) = tracerLitrMass(i,j,k) + tracer_rootltdt(i,j) * rmatctem(i,j,k) - &
                                          tracer_ltrcemls(i,j) * (tracerLitrMass(i,j,k)/tracerlitterTemp(i,j))
                  else 
                    tracerLitrMass(i,j,1) = tracerLitrMass(i,j,1) + tracer_rootltdt(i,j) * rmatctem(i,j,k) 
                  end if 
                end if 
              end do
            end if
  ...
  end if ! end dofire or prescribedFire check.
  ...
  do i = il1,il2
    ...
    do j = 1,icc ! loop 620
      ...
      do i = il1,il2 ! loop 630
        ...
        emit_so2(i,j)  = emif_so2(n) * tot_emit_dom * 1.1574e-8


        if(switch_cffeps_plume) then
           !> calculate the fire plume energy per PFT from tot_emit_dom (which is per PFT) and burnarea
           call fireenergycl(tot_emit_dom,burnarea_veg(i,j),fien,icc,num_fires(i,j)) ! burnveg(i,j)
           ! write(*,*) 'fiengat =',fien
           fiengat(i,j) = fiengat(i,j)+ fien
        endif
  ...
  subroutine burntobare (il1, il2, nilg, sort, pvgbioms, pnvgbioms, pgavltms, & ! In
    ...
    use classicParams, only : crop, icc, standreplace, grass, zero, seed, &
    ...
    real :: tracerLitrLost(nilg,ignd)       !< tracer litter that is transferred to bare, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracerSoilCLost(nilg,ignd)      !< tracer soilc that is transferred to bare, 14C: \f$ng ^{14}C/m^2\f$
      ...
      do j = 1,icc ! loop 50
        if (.not. crop(j)) then
          ...
          if (pftfraca(i,j) /= pftfracb(i,j)) then
            ...
            if (useTracer /= 'None') then ! Now same operation for tracer
  ...
  !< Subroutines for FWI calculations based upon Wang et al., 2015 \cite Wang2015-bf.
  !< and verified against table 7 therein. More backgound is provided in Van Wagner 1987 \cite Van_Wagner1987-nx.
  !< As well as the original source code and equations in Van Wagner and Pickett 1985 \cite Van_Wagner1985-xi
  !< "Equations and Fortran Program for the Canadian Forest Fire Weather Index System"
  !< FWI required the previous ISI, BUI, FWI, FFMC, DMC, and DC as well as
  !< max daily temperature (C), relative humidity (%), wind speed (km/h),
  !< rainfall over the last 24 hours (mm), and information about what month it is.
  subroutine fwicalculation(NoonTempc,NoonWindtot,NoonPrecip,NoonRhum, & !in
                            isi, bui, fwi, & !out
                            ffmc, dmc, dc, IMONTH) !inout
    implicit none
    real, intent(in) :: NoonTempc,NoonPrecip,NoonWindtot,NoonRhum !!! four weather variables
    real, intent(inout) :: ffmc, dmc, dc !!! FFMC, DMC, and DC to be calculated
    real, intent(out) :: isi, bui, fwi !!! ISI, BUI, FWI, and DSR to be calculated
    real :: ffmc0, dmc0, dc0, rhum !!! initial or yesterday’s FFMC, DMC, and DC values
    integer :: mth, day !!! month and day for each observation
    integer :: io !!! utilit y integer
    integer, intent(in) :: IMONTH
    
    ffmc0 = ffmc !!! use previous values
    dmc0 = dmc !!! for tomorrow’s
    dc0 = dc !!! calculation

    call FFMCcalc(NoonTempc,NoonRhum,NoonWindtot,NoonPrecip,ffmc0,ffmc) !!! calculate FFMC
    if (ffmc <= 0.0 ) ffmc = 0.01
    call DMCcalc(NoonTempc,NoonRhum,NoonPrecip,dmc0,IMONTH,dmc) !!! calcualte DMC
    if (dmc <= 0.0 ) dmc = 0.01
    call DCcalc(NoonTempc,NoonPrecip,dc0,IMONTH,dc) !!! calculate DC
    if (dc <= 0.0 ) dc = 0.01
    call ISIcalc(ffmc,NoonWindtot,isi) !!! calcualte ISI
    if (isi <= 0.0 ) isi = 0.01
    call BUIcalc(dmc,dc,bui) !!! calculate BUI
    if (BUI <= 0.0 ) BUI = 0.01
  ...
  ! Calculate the fine fuel moisture code FFMC Van Wagner and Pickett 1985 pg 5
  subroutine FFMCcalc(T,H,W,Ro,Fo,ffmc)
  implicit none
    real, intent(in) :: T,H,W,Ro,Fo
    real, intent(out) :: ffmc
    real :: Mo,Rf,Ed,Ew,M,Kl,Kw,K,Pr,Ra,Re,B,Mr,V,Rd,Dr,Qo,ko,Kd

    Mo = (147.2*(101.0 - Fo))/(59.5 + Fo) !Van Wagner and Pickett 1985 eqn 1
    if (Ro > 0.5) then
      rf = Ro - 0.5 !Van Wagner and Pickett 1985 eqn 2
      if(Mo <= 150.0) then
        Mr = Mo+42.5*Rf*exp(-100.0/(251.0-Mo))*(1.0 - exp(-6.93/Rf)) !Van Wagner and Pickett 1985 eqn 3a
      else
        Mr = (Mo+42.5*Rf*exp(-100.0/(251.0-Mo))*(1.0 - exp(-6.93/Rf))) + (.0015*(Mo - 150.0)**2)*sqrt(Rf) !Van Wagner and Pickett 1985 eqn 3b
      endif
      if (Mr > 250.0) Mr = 250.0
      Mo = Mr
    endif
    Ed =.942*(H**.679) + (11.0*exp((H-100.0)/10.0))+0.18*(21.1-T) * (1.0 - 1.0/exp(.1150 * H)) !Van Wagner and Pickett 1985 eqn 4
    
    if(Mo > Ed) then
      Ko = .424*(1.0-(H/100.0)**1.7)+(.0694*sqrt(W))*(1.0- (H/100.0)**8) !Van Wagner and Pickett 1985 eqn 6a
      Kd = Ko * (.581*exp(.0365*T)) !Van Wagner and Pickett 1985 eqn 6b
      M = Ed + (mo-ed)/10.0 ** Kd !Van Wagner and Pickett 1985 eqn 8
    else
      Ew = .618*(H**.753) + (10.0*exp((H-100.0)/10.0)) + .18*(21.1-T) * (1.0 - 1.0/exp(.115 * H)) !Van Wagner and Pickett 1985 eqn 5
      if(Mo < Ew) then
        Kl = .424*(1.0-((100.0 - H)/100.0)**1.7) + (.0694*sqrt(W)) * (1.0 - ((100.0 - H)/100.0)**8) !Van Wagner and Pickett 1985 eqn 7a
        Kw = Kl * (.581 * exp(.0365 * T)) !Van Wagner and Pickett 1985 eqn 7b
        M = Ew - (Ew - Mo)/10.0**Kw !Van Wagner and Pickett 1985 eqn 9
      else
        M = Mo
      endif
    endif
    ffmc = (59.5 * (250.0 - m)) / (147.2 + m) !Van Wagner and Pickett 1985 eqn 10
  ...
  ! Calculate the duff moisture code DMC  Van Wagner and Pickett 1985 pg 6
  subroutine DMCcalc(T, H, Ro, Po, I, dmc)
  implicit none
    integer, intent(in) :: I
    real, intent(in) :: T,H,Ro,Po
    real, intent(out) :: dmc
    real :: Mo,Rf,Ed,Ew,M,Kl,Kw,K,Pr,Ra,Re,B,Mr,V,Rd,Dr,Qo
    real :: El(12) = (/6.5,7.5,9.0,12.8,13.9,13.9,12.4,10.9,9.4,8.0,7.0,6.0/)

    if(Ro <= 1.5) then
      Pr = Po
    else
      Re = 0.92*Ro - 1.27 !Van Wagner and Pickett 1985 eqn 11
      Mo = 20.0 + 280.0/exp(0.023*Po) !Van Wagner and Pickett 1985 eqn 12
      if(Po <= 33.0) then
        B = 100.0 /(0.5 + 0.3*Po) !Van Wagner and Pickett 1985 eqn 13a
      else
        B = 6.2 * log(Po) - 17.2 !Van Wagner and Pickett 1985 eqn 13c
        if(Po-65.0 <= 0.0) B = 14.0 - 1.3*log(Po) !Van Wagner and Pickett 1985 eqn 13b
      endif
      Mr = Mo + (1000.0*Re) / (48.77+B*Re) !Van Wagner and Pickett 1985 eqn 14
      Pr = 43.43 * (5.6348 - log(Mr-20.0)) !Van Wagner and Pickett 1985 eqn 15
    endif
    if(T >= -1.1) then
      K = 1.894*(T+1.1) * (100.0-H) * (El(I+1)*0.0001) !Van Wagner and Pickett 1985 eqn 16/17
    else
      K = 0.0
    endif
    if(Pr < 0.0) Pr = 0.0
    dmc = Pr + K !Van Wagner and Pickett 1985 eqn 17
    if(dmc <= 0.0) dmc = 0.0

  return
  end subroutine

  ! Calculate the drought code DC  Van Wagner and Pickett 1985 pg 6/7
  subroutine DCcalc (T, Ro, D0, I, dc)
    ...
    integer,intent(in) :: I
    real :: V,Rd,Dr,Qo,Qr
    real :: Fl(12) = (/-1.6,-1.6,-1.6,0.9,3.8,5.8,6.4,5.0,2.4,0.4,-1.6,-1.6/)
    
    if (Ro > 2.8) then
      Rd = 0.83*Ro - 1.27 !Van Wagner and Pickett 1985 eqn 18
      Qo = 800.0 * exp(-D0/400.0) !Van Wagner and Pickett 1985 eqn 19
      Qr = Qo + 3.937*Rd !Van Wagner and Pickett 1985 eqn 20
      Dr = 400.0*log(800.0/Qr) !Van Wagner and Pickett 1985 eqn 21
      if(Dr > 0.0) then
        D0 = Dr
      else
        D0 = 0.0
    ...
    if(T < -2.8) then
      ...
      V = (0.36*(T+2.8) + Fl(I+1)) !Van Wagner and Pickett 1985 eqn 22
    endif
    if (V <= 0.0) V = 0.0
    dc = D0 + 0.5*V !Van Wagner and Pickett 1985 eqn 23

    return
  end subroutine

  ! Calculate the initial spread index ISI Van Wagner and Pickett 1985 pg 7/8
  subroutine ISICalc(F,W, isi)
    implicit none
    real,intent(in) :: F, W
    real,intent(out):: isi
    real :: Mo,Rf,Ed,Ew,M,Kl,Kw,K,Pr,Ra,Re,B,Mr,V,Rd,Dr,Qo,Fw,Ff

    Mo = (147.2*(101.0-F)) / (59.5+F) !Van Wagner and Pickett 1985 eqn 1
    Fw = exp(0.05039*W) !Van Wagner and Pickett 1985 eqn 24
    Ff = 91.9*exp(-0.1386*Mo) * (1.0+(Mo**5.31)/4.93e7) !Van Wagner and Pickett 1985 eqn 25
    isi = 0.208 * Fw * Ff !Van Wagner and Pickett 1985 eqn 26

    return
  end subroutine

  ! Calculate the buildup index BUI Van Wagner and Pickett 1985 pg 7/8
  subroutine BUICalc(P,D,bui)
    ...
    if (P <= 0.4*D) then
      bui = (0.8*P*D) / (P+0.4*D) !Van Wagner and Pickett 1985 eqn 27a
    else
      bui = P-(1.0-0.8*D/(P+0.4*D))*(0.92+(0.0114*P)**1.7) !Van Wagner and Pickett 1985 eqn 27b
    endif
  ...
  ! Calculate the fire weather index FWI Van Wagner and Pickett 1985 pg 7/8
  subroutine FWICalc(R,U,fwi)
    implicit none
    real, intent(in) :: R,U
    real, intent(out):: fwi
    real :: Mo,Rf,Ed,Ew,M,Kl,Kw,K,Pr,Ra,Re,B,Mr,V,Rd,Dr,Qo,Fd

    if (U <= 80.0) then
      Fd = 0.626*U**0.809 + 2.0 !Van Wagner and Pickett 1985 eqn 28a
    else
      Fd = 1000.0/(25.0+108.64*exp(-0.023*U)) !Van Wagner and Pickett 1985 eqn 28b
    endif
    B = 0.1*R*Fd !Van Wagner and Pickett 1985 eqn 29
    if(B > 1.0) then
      fwi = exp(2.72*(0.434*log(B))**0.647) !Van Wagner and Pickett 1985 eqn 30
    else
      fwi = B !Van Wagner and Pickett 1985 eqn 30b
    endif
