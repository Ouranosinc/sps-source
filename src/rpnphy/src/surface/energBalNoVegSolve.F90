...
subroutine energBalNoVegSolve (ISNOW, FI, & ! Formerly TSOLVE
                               ...
                               mossPresent, co2conc, pressg, Cmossmas, &
                               dmoss, anmoss, rmlmoss, iday, daylength, daylength_max, &
  ...
  !     *                           BARE SOIL for ISNOW=0 and snoAlbedoParam='4-band' (NEED
  !     *                           TO SUM OVER THE 3 NEAR-IR BANDS).
  !     * JUN 22/13 - J.COLE/     - ADD "snoAlbedoParam" OPTION (4-BAND SOLAR).
  ...
  real, intent(inout) :: CDH   (ILG)  !< Surface drag coefficient for heat \f$[ ] (C_{DH}) \f$
  real, intent(inout) :: CDM   (ILG)  !< Surface drag coefficient for momentum [ ]
  real, intent(inout) :: RIB   (ILG)  !< Bulk Richardson number at surface [ ]
  real, intent(inout) :: CFLUX (ILG)  !< Product of surface drag coefficient and wind speed \f$[m s^{-1}]\f$
  real, intent(inout) :: FTEMP (ILG)  !< Product of surface-air temperature gradient,
  !< drag coefficient and wind speed \f$[K m s^{-1}]\f$
  real, intent(inout) :: FVAP  (ILG)  !< Product of surface-air humidity gradient, drag
  !< coefficient and wind speed \f$[kg kg^{-1} m s^{-1}]\f$
  real, intent(inout) :: ILMO  (ILG)  !< Inverse of Monin-Obukhov roughness length \f$(m-1]\f$
  real, intent(inout) :: UE    (ILG)  !< Friction velocity of air \f$[m s^{-1}]\f$
  real, intent(inout) :: H     (ILG)  !< Height of the atmospheric boundary layer [m]
  real, intent(in) :: ZPOND (ILG)  !< Depth of ponded water on surface [m]
  real, intent(out) :: anmoss(ilg)  !<
  real, intent(out) :: rmlmoss(ilg) !<
  real :: cevapmoss(ilg) !< evaporation coefficent for moss surface
  !
  !     * INPUT ARRAYS.
  !
  ...
  real, intent(in) :: daylength_max(ilg)      !< maximum daylength for this location (hours)
  ...
  !     * Local VARIABLES.
  !
  real :: QSWNV(ilg)    !< Absorbed visible part of shortwave radiation, \f$W/m^2\f$
  real :: QSWNI, DCFLUX, DRDT0, TZEROT, EZERO, EVPMAX(ILG)
  !
  !>
  ...
  !! in two bands (one visible and one near-IR, snoAlbedoParam='2-band') or four bands
  !! (one visible and three near-IR, snoAlbedoParam='4-band').  For each band the net
  ...
  !! snoAlbedoParam='2-band' the absorbed near-IR radiation is calculated from the second
  !! band and if snoAlbedoParam='4-band' it is calculated from the sum of the second to
  !! fourth bands.  The total net shortwave radiation QSWNET  is then evaluated.
  !! If a snow pack is present, then if snoAlbedoParam='2-band' the calculations are done
  !! separately for the visible and near-IR bands; if snoAlbedoParam='4-band', they are done
  !! separately over the four wavelength bands. The transmissivity is calculated
  !! as an average value for snoAlbedoParam='2-band', and separately for the four wavelength
  !! ranges for snoAlbedoParam='4-band'. The net shortwave radiation at the surface is
  ...
  if (ISNOW == 0) then ! No snow
    ...
    do I = IL1,IL2
      if (FI(I) > 0.) then
        ...
        if (snoAlbedoParam == '2-band') then
          QSWNI = FSSB(I,2) * (1.0 - ALNIRG(I))
        else if (snoAlbedoParam == '4-band') then
          QSWNI = 0.0
          do IB = 2,NBS
            QSWNI = QSWNI + FSSB(I,IB) * (1.0 - ALNIRG(I))
          end do ! IB
        else
          print*,'unknown snoAlbedoParam!',snoAlbedoParam
          call errorHandler('energBalNoVegSolve', 0) 
        end if
    ...
    if (snoAlbedoParam == '2-band') then ! Use the existing snow albedo and transmission
      do I = IL1,IL2
        if (FI(I) > 0.) then
    ...
    else if (snoAlbedoParam == '4-band') then ! Use the band-by-band snow albedo and transmission
      do I = IL1,IL2
        QTRANS(I) = 0.0
        QSWNET(I) = 0.0
      end do ! I
  ...
  call mossPht(il1, il2, iday, qswnv, thliq, co2conc, tstart, zsnow, & ! In
               pressg, Cmossmas, dmoss, mossPresent, Nmossmas, & ! In 
...
100 continue ! anchor for a GO TO statement that needs to be removed
  ...
  do I = IL1,IL2 ! loop 150
    if (FI(I) > 0. .and. ITER(I) == 1) then
      ...
      if (IWATER(I) > 0) then
        ...
        ! The evaporation coefficient is calculated for non-moss covered areas.
        ! if there is moss cover, it is replaced by the moss values calculated in
        ! mossPht.     
  ...
  !
  if (IBAD /= 0) then
      write(6,*) 'Longitude,Latitude:', DLON(ibad), DLAT(ibad)
