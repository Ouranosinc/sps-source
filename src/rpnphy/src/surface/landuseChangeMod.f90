...
contains
  ...
  subroutine initializeLandCover(vrot, class_rot)
    !
    ...
    use classicParams,   only : nmos, nlat, ilg, ignd, icc, ican, icp1, seed, crop, &
                                numcrops, minbare, reindexPFTs, c_switch
  ...
  subroutine luc (il1, il2, nilg, PFTCompetition, leapnow, useTracer, & ! In
    ...
    use classicParams,    only : icc, ican, zero, km2tom2, iccp1, &
                                 ...
                                 reindexPFTs, &
                                 rel_tolerance_amount, rel_tolerance_density, c_switch
    ...
    real, intent(inout) :: tracerGLeafMass(nilg,icc)        !< Tracer mass in the green leaf pool for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$
    real, intent(inout) :: tracerBLeafMass(nilg,icc)        !< Tracer mass in the brown leaf pool for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$
    real, intent(inout) :: tracerStemMass(nilg,icc)         !< Tracer mass in the stem for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$
    real, intent(inout) :: tracerRootMass(nilg,icc)         !< Tracer mass in the roots for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$
    real, intent(inout) :: tracerLitrMass(nilg,iccp2,ignd)  !< Tracer mass in the litter pool for each of the CTEM pfts + bareground and LUC products, 14C: \f$ng ^{14}C/m^2\f$
    real, intent(inout) :: tracerSoilCMass(nilg,iccp2,ignd) !< Tracer mass in the soil carbon pool for each of the CTEM pfts + bareground and LUC products, 14C: \f$ng ^{14}C/m^2\f$
    ...
    real :: tracer_totlmass(nilg) !< Tracer total c mass (live), 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_totdmas1(nilg) !< Tracer total c mass (dead) litter, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_totdmas2(nilg) !< Tracer total c mass (dead) soil c, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_totcmass(nilg) !< Total c mass (live+dead), 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_abvgmass(nilg,icc)   !< Tracer above-ground biomass, 14C: \f$ng ^{14}C/m^2\f$
    real :: tracer_redubmas1      !< Tracer version of redubmas1
    real :: tracer_redubmas2      !< Tracer version of redubmas2
    real :: tracer_chopedbm(nilg) !< Tracer version of chopped off biomass
    real :: tracer_combustc(nilg,icc)   !< total tracer carbon from deforestation- combustion
    real :: tracer_paperc(nilg,icc)     !< total tracer carbon from deforestation- paper
    real :: tracer_furnturc(nilg,icc)   !< total tracer carbon from deforestation- furniture
    real :: tracer_grsumcom(nilg) !< grid sum of combustion tracer carbon for all pfts that are chopped
    real :: tracer_grsumpap(nilg) !< grid sum of paper tracer carbon for all pfts that are chopped
    real :: tracer_grsumfur(nilg) !< grid sum of furniture tracer carbon for all pfts that are chopped
    real :: tracer_incrlitr             !< Tracer version of incrlitr
    real :: tracer_incrsolc             !< Tracer version of incrsolc
    real :: tracer_grsumlit(nilg,ignd)   !< grid sum of litter tracer carbon for all pfts that are chopped
    real :: tracer_grsumsoc(nilg,ignd)   !< grid sum of soil c tracer carbon for all pfts that are chopped
    real :: tracer_grdencom(nilg) !< grid averaged densities for combustion tracer carbon
    real :: tracer_grdenpap(nilg) !< grid averaged densities for paper tracer carbon
    real :: tracer_grdenfur(nilg) !< grid averaged densities for furniture tracer carbon
    real :: tracer_grdenlit(nilg,ignd) !< grid averaged densities for litter carbon
    real :: tracer_grdensoc(nilg,ignd) !< grid averaged densities for soil c carbon    
    real :: tracer_ntotlmas(nilg) !< total tracer C mass (live) after luc treatment
    real :: tracer_ntotdms1(nilg) !< total tracer C mass (dead) litter after luc treatment
    real :: tracer_ntotdms2(nilg) !< total tracer C mass (dead) soil C after luc treatment
    real :: tracer_ntotcmas(nilg) !< total tracer C mass (live+dead) after luc treatment
    ...
    do i = il1,il2
      if (luctkplc(i)) then
        ...
        tracer_grsumcom(i) = 0.0   ! grid sum of combustion tracer carbon for all
                                   ! pfts that are chopped
        grsumpap(i) = 0.0          ! similarly for paper,
        grsumnpap(i) = 0.0
        tracer_grsumpap(i) = 0.0
        grsumfur(i) = 0.0          ! furniture,
        grsumnfur(i) = 0.0
        tracer_grsumfur(i) = 0.0
        grsumnlit(i) = 0.0
        grsumson(i) = 0.0
        grsumnh4(i) = 0.0
        grsumno3(i) = 0.0
        do k = 1,ignd
            ...
            tracer_grsumlit(i,k)=0.0   ! grid sum of combustion tracer carbon for all chopped PFT's litter, and
            tracer_grsumsoc(i,k)=0.0   ! tracer soil c     
            tracer_grdenlit(i,k)=0.0       
            tracer_grdensoc(i,k)=0.0
        end do
        grdennlit(i) = 0.0
        grdenson(i)  = 0.0
        grdennh4(i)  = 0.0
        ...
        grdenncom(i) = 0.0
        tracer_grdencom(i) = 0.0
        grdenpap(i) = 0.0          ! paper,
        grdennpap(i) = 0.0
        tracer_grdenpap(i) = 0.0
        grdenfur(i) = 0.0          ! furniture,
        grdennfur(i) = 0.0
        tracer_grdenfur(i) = 0.0
        ...
        do j = 1,icc
          totlmass(i) = totlmass(i) &
                        ...
                        * km2tom2)
          if (Ncycle_on)  totlnmass(i) = totlnmass(i) &
                            + (fcancmy(i,j) * (ngleafmas(i,j) + nbleafmas(i,j) &
                            + nstemmass(i,j) + nrootmass(i,j)) * grclarea(i) &
                            * km2tom2)
          if (useTracer /= 'None') tracer_totlmass(i) = tracer_totlmass(i) &
                        + (fcancmy(i,j) * (tracerGLeafMass(i,j) + tracerBLeafMass(i,j) &
                        + tracerStemMass(i,j) + tracerRootMass(i,j)) * grclarea(i) &
                        * km2tom2)            
        end do ! loop 320
        ...
        do j = 1,iccp2
          ...
          do k = 1,ignd
            ...
            if (useTracer /= 'None') tracer_totdmas1(i)=tracer_totdmas1(i)+ &
                                          (term*tracerLitrMass(i,j,k)*grclarea(i)*km2tom2)
            totdmas2(i)=totdmas2(i)+ &
                       (term*soilcmas(i,j,k)*grclarea(i)*km2tom2)
            if (useTracer /= 'None') tracer_totdmas2(i)=tracer_totdmas2(i)+ &
                                          (term*tracerSoilCMass(i,j,k)*grclarea(i)*km2tom2) 
          end do           
        ...
        if (useTracer /= 'None') tracer_totcmass(i) =  tracer_totlmass(i) + &
                                          tracer_totdmas1(i) + tracer_totdmas2(i)
        ...
        do j = 1,ican
          do m = reindexPFTs(j,1),reindexPFTs(j,2)
            ...
            if (useTracer /= 'None') tracer_abvgmass(i,m) = tracerGLeafMass(i,m) + &
                                           tracerBLeafMass(i,m) + tracerStemMass(i,m)
        ...
        do j = 1,ican
          do m = reindexPFTs(j,1),reindexPFTs(j,2)
            if (fraciord(i,m) == - 1) then
              ...
              if (useTracer /= 'None') tracer_redubmas1 = &
                                      (fcancmy(i,m) - fcancmx(i,m)) * &
                                      grclarea(i) * tracer_abvgmass(i,m) * km2tom2
              ...
              if (Ncycle_on) redunbmas2 = (fcancmy(i,m) - fcancmx(i,m)) * grclarea(i)  &
                              * nrootmass(i,m) * km2tom2
              if (useTracer /= 'None') tracer_redubmas2 = &
                                          (fcancmy(i,m) - fcancmx(i,m)) * grclarea(i) * &
                                          tracerRootMass(i,m) * km2tom2
              ...
              if (Ncycle_on)  chopednbm(i) = chopednbm(i) + redunbmas1 + redunbmas2
              if (useTracer /= 'None') tracer_chopedbm(i) = tracer_chopedbm(i) + &
                                          tracer_redubmas1 + tracer_redubmas2
              ...
              if (useTracer /= 'None') then 
                tracer_combustc(i,m) = combust(treatind(i,m)) * tracer_redubmas1
                tracer_paperc(i,m) = paper(treatind(i,m)) * tracer_redubmas1 + tracer_redubmas2
                tracer_furnturc(i,m) = furniture(treatind(i,m)) * tracer_redubmas1
              end if 
              ...
              if (useTracer /= 'None') then
                tracer_grsumcom(i) = tracer_grsumcom(i) + tracer_combustc(i,m)
                tracer_grsumpap(i) = tracer_grsumpap(i) + tracer_paperc(i,m)
                tracer_grsumfur(i) = tracer_grsumfur(i) + tracer_furnturc(i,m)
              end if                 
              ...
              do k = 1,ignd
                ...
                if (useTracer /= 'None') tracer_incrlitr=(fcancmy(i,m)-fcancmx(i,m))*&
                                              grclarea(i)*tracerLitrMass(i,m,k)*km2tom2

                incrsolc=(fcancmy(i,m)-fcancmx(i,m))*grclarea(i) &
                        *soilcmas(i,m,k)*km2tom2
                if (useTracer /= 'None') tracer_incrsolc=(fcancmy(i,m)-fcancmx(i,m))*&
                                              grclarea(i)*tracerSoilCMass(i,m,k)*km2tom2

                grsumlit(i,k)=grsumlit(i,k)+incrlitr
                if (useTracer /= 'None') tracer_grsumlit(i,k)=tracer_grsumlit(i,k)+tracer_incrlitr

                grsumsoc(i,k)=grsumsoc(i,k)+incrsolc
                if (useTracer /= 'None') tracer_grsumsoc(i,k)=tracer_grsumsoc(i,k)+tracer_incrsolc
              end do 
        ...
        if (bareiord(i) == - 1) then
          do k = 1,ignd
            ...
            if (useTracer /= 'None') tracer_redubmas1=(pbarefra(i)-barefrac(i))*grclarea(i) &
                                                      *tracerLitrMass(i,iccp1,k)*km2tom2
            redubmas2=(pbarefra(i)-barefrac(i))*grclarea(i) &
                      *soilcmas(i,iccp1,k)*km2tom2
            if (useTracer /= 'None') tracer_redubmas2=(pbarefra(i)-barefrac(i))*grclarea(i) &
                                                      *tracerSoilCMass(i,iccp1,k)*km2tom2

            grsumlit(i,k)=grsumlit(i,k)+redubmas1
            if (useTracer /= 'None') tracer_grsumlit(i,k)=tracer_grsumlit(i,k)+tracer_redubmas1

            grsumsoc(i,k)=grsumsoc(i,k)+redubmas2
            if (useTracer /= 'None') tracer_grsumsoc(i,k)=tracer_grsumsoc(i,k)+tracer_redubmas2
        ...
        if (Ncycle_on) then
          sum1=grsumncom(i)+grsumnpap(i)+grsumnfur(i)
          diff=abs(chopednbm(i)-sum1)
          if (chopednbm(i)==0.) then
            if (diff==0.) then
              rdif=0.              ! relative accuracy
            else
              rdif=1.              ! relative accuracy
            ...
            rdif=diff/chopednbm(i)  ! relative accuracy
          end if
        end if
        if (Ncycle_on .and. rdif > rel_tolerance_amount) then
        ...
        if (useTracer /= 'None') tracer_grdencom(i) = tracer_grsumcom(i)/(grclarea(i) * km2tom2)

        grdenpap(i) = grsumpap(i)/(grclarea(i) * km2tom2)
        if (useTracer /= 'None') tracer_grdenpap(i) = tracer_grsumpap(i)/(grclarea(i) * km2tom2)

        grdenfur(i) = grsumfur(i)/(grclarea(i) * km2tom2)
        if (useTracer /= 'None') tracer_grdenfur(i) = tracer_grsumfur(i)/(grclarea(i) * km2tom2)

        do k = 1,ignd
          grdenlit(i,k)=grsumlit(i,k)/(grclarea(i)*km2tom2)
          if (useTracer /= 'None') tracer_grdenlit(i,k)=tracer_grsumlit(i,k)/(grclarea(i)*km2tom2)

          grdensoc(i,k)=grsumsoc(i,k)/(grclarea(i)*km2tom2)
          if (useTracer /= 'None') tracer_grdensoc(i,k)=tracer_grsumsoc(i,k)/(grclarea(i)*km2tom2)
        end do
        ...
        if (useTracer /= 'None') then ! Now same operation for tracer
          tracerLitrMass(i,iccp2,1) = tracerLitrMass(i,iccp2,1) + tracer_grdenpap(i)
          tracerSoilCMass(i,iccp2,1) = tracerSoilCMass(i,iccp2,1) + tracer_grdenfur(i)
        end if
        if (Ncycle_on) then
        ...
        do j = 1,icc
          if (fcancmx(i,j) > zero) then
            ...
            if (useTracer /= 'None') then ! Now same operation for tracer
              tracerLitrMass(i,j,:) = tracerLitrMass(i,j,:) + tracer_grdenlit(i,:)
              tracerSoilCMass(i,j,:) = tracerSoilCMass(i,j,:) + tracer_grdensoc(i,:)
            end if
          else
            gleafmas(i,j) = 0.0
            gleafmas_ns(i,j) = 0.0
        ...
        if (barefrac(i) > zero) then
          ...
          if (useTracer /= 'None') then ! Now same operation for tracer
            tracerLitrMass(i,iccp1,:) = tracerLitrMass(i,iccp1,:) + tracer_grdenlit(i,:)
            tracerSoilCMass(i,iccp1,:) = tracerSoilCMass(i,iccp1,:) + tracer_grdensoc(i,:)
          end if
        else
        ...
        if (( 1 - barefrac(i) ) < zero ) then
          do j = 1,icc
            if (fcancmx(i,j) > zero) then
              ...
              ! For the tracer, just use the tracer_grdencom since we aren't going to track the 
              ! tracer emitted and lucemcom is just the value of grdencom.
              if (useTracer /= 'None')tracerLitrMass(i,j,1) = tracerLitrMass(i,j,1) + tracer_grdencom(i)
            end if
          end do
          lucemcom(i) = 0.0
          lucemcomn(i) = 0.0
        ...
        do j = 1,icc
          ntotlmas(i) = ntotlmas(i) + &
                        ...
                        stemmass(i,j) + rootmass(i,j)) * grclarea(i) &
                        * km2tom2)
          if (Ncycle_on)  ntotlnmas(i) = ntotlnmas(i)+ &
                              (fcancmx(i,j) * (ngleafmas(i,j) + nbleafmas(i,j) + &
                              nstemmass(i,j) + nrootmass(i,j)) * grclarea(i) &
                              * km2tom2)
          if (useTracer /= 'None') tracer_ntotlmas(i) = tracer_ntotlmas(i) + &
                                        (fcancmx(i,j) * (tracerGLeafMass(i,j) + tracerBLeafMass(i,j) + &
                                        tracerStemMass(i,j) + tracerRootMass(i,j)) * grclarea(i) &
                                        * km2tom2)
        end do ! loop 700
        ...
        do j = 1,iccp2
          ...
          do k = 1,ignd
            ntotdms1(i) = ntotdms1(i) + (term * litrmass(i,j,k) * grclarea(i) * km2tom2)
            if (useTracer /= 'None') tracer_ntotdms1(i) = tracer_ntotdms1(i) + &
                                          (term * tracerLitrMass(i,j,k) * grclarea(i) * km2tom2)
            ntotdms2(i) = ntotdms2(i) + (term * soilcmas(i,j,k) * grclarea(i) * km2tom2)
            if (useTracer /= 'None') tracer_ntotdms2(i) = tracer_ntotdms2(i) + &
                                          (term * tracerSoilCMass(i,j,k) * grclarea(i) * km2tom2)
          end do
          if (Ncycle_on) then
        ...
        if (Ncycle_on) ntotnmas(i) = ntotlnmas(i) + ntotdnms1(i) + ntotdnms2(i) &
                         + ntotdnh4ms(i) + ntotdno3ms(i)
        if (useTracer /= 'None') tracer_ntotcmas(i) = tracer_ntotlmas(i) + tracer_ntotdms1(i) + tracer_ntotdms2(i)
        ...
        if (rdif > rel_tolerance_amount .and. diff > 1E-15) then
          write(6, * )'at grid cell = ',i
        ...
        if (Ncycle_on) then
          sum1=totdnmas1(i)+grsumnpap(i)
          diff=abs(sum1-ntotdnms1(i))
          if (ntotdnms1(i)==0.) then
            if (diff==0.) then
              rdif=0.             ! relative accuracy
            else
              rdif=1.             ! relative accuracy
            ...
            rdif=diff/ntotdnms1(i) ! relative accuracy
          end if
        end if
        if ( Ncycle_on .and. rdif > rel_tolerance_amount) then
        ...
        if(useTracer /= 'None' .and. abs(tracer_totdmas1(i)+ tracer_grsumpap(i) - tracer_ntotdms1(i)) > tolrnce1 ) then
           write(6, * )'at grid cell = ',i
           write(6, * )'total tracer litter does not balance after luc'
           write(6, * )'tracer_totdmas1(i) = ',tracer_totdmas1(i)
           write(6, * )'tracer_grsumpap(i) = ',tracer_grsumpap(i)
           write(6, * )'tracer_totdmas1(i) + tracer_grsumpap(i) = ', &
                      tracer_totdmas1(i) + tracer_grsumpap(i)
           write(6, * )'tracer_ntotdms1(i) = ',tracer_ntotdms1(i)
        ...
        if (Ncycle_on) then
          sum1=ntotnmas(i)+grsumncom(i)
          diff=abs(totnmass(i)-sum1)
          if (totnmass(i)==0.) then
            if (diff==0.) then
              rdif=0.             ! relative accuracy
            else
              rdif=1.             ! relative accuracy
            ...
            rdif=diff/totnmass(i) ! relative accuracy
          end if
        end if
        if ( Ncycle_on .and. rdif > rel_tolerance_amount) then
        ...
        if (useTracer /= 'None' .and. abs(tracer_totcmass(i) - tracer_ntotcmas(i) - tracer_grsumcom(i)) > tolrnce1) then
           write(6, * )'at grid cell = ',i
           write(6, * )'total tracer C does not balance after luc'
           write(6, * )'tracer_totcmass(i) = ',tracer_totcmass(i)
           write(6, * )'tracer_ntotcmas(i) = ',tracer_ntotcmas(i)
           write(6, * )'tracer_grsumcom(i)= ',tracer_grsumcom(i)
           call errorHandler('luc', - 15)
        end if
        !>
