...
subroutine waterBaseflow (THLIQX, THICEX, TBARWX, ZPOND, TPOND, & ! Formerly WEND
  ...
  real, intent(in)    :: THICEX(ILG,IGP1) !< Volumetric frozen water content of soil layer \f$[m^3 m^{-3}]\f$
  ...
  !
  integer, intent(in) :: LZF(ILG)         !< index of the soil layer in which the wetting front occurs
  integer, intent(in) :: IGRN(ILG)        !< IGRN is set to 1 for all tiles where the calculations in this subroutine are to be performed.
  ...
  real, intent(inout) :: ZRMDR(ILG,IGP1)       !< depth that has not been affected by infiltration for each soil layer [m]
  real, intent(inout) :: WADD(ILG)             !< Water movement accounting variable \f$[m^3 m^{-2}]\f$
  real, intent(inout) :: TADD(ILG)             !< Temperature of water associated with WADD [C]
  !
  !     * INTERNAL ARRAYS USED IN CALLING waterFlowNonInfiltrate.
  !
  real, intent(in) :: TFDT(ILG,IGP1)          !< Temperature of water flowing between soil layers [C]
    ...
    do J = 1,IGP1
      do I = IL1,IL2
        if (IGRN(I) > 0 .and. K <= NINF(I)) then
          ...
          ZMAT(I,K,J) = 0. ! FLAG temp fix, this was hitting line 404 with NANs. JM Mar 2024
          WMOVE(I,K) = 0. ! FLAG temp fix, this was hitting line 299 with NANs. JM Mar 2024
        end if
      end do
    end do
  end do ! loop 400
