...
subroutine n_processes(il1, il2, spinfast, thliq, thice, thpor, zbotw, tbar, isand, & ! In
  ...
  character(8), intent(in), dimension(ilg) :: peatlandType !< Peatland type (else 'None')
  ...
  real,intent(inout), dimension(ilg) :: soilpH             !< soil pH
  ...
  real, dimension(ilg), intent(in) :: Cmossmas          !< moss biomass C pool (kgC/m2)
  real, dimension(ilg,ignd), intent(in) :: litrmsmoss   !< moss litter C pool (kgC/m2)
  real, dimension(ilg,ignd), intent(in) :: upMossSoilC  !< Moss soil C mass (non-peat), \f$kg C/m^2\f$
  real, dimension(ilg), intent(in) :: nppmosstep        !< moss NPP (kgC/m2/timestep)
  real, dimension(ilg), intent(in) :: litrfallmoss      !< moss litter fall (kgC/m2/timestep)
  real, dimension(ilg,ignd), intent(in) :: humstepmoss  !< moss humification (transfer from litter to SOM) (kgC/m2/timestep)
  real, dimension(ilg,ignd), intent(in) :: ltrestepmoss !< litter respiration from moss (kgC/m2/timestep)
  real, dimension(ilg,ignd), intent(in) :: socres_moss  !< heterotrophic respiration from moss-derived soil C (\f$\mu mol CO_2 m^{-2} s^{-1}\f$)
  ...
  real,intent(inout), dimension(ilg) :: Nmossmas           !< moss biomass N pool, \f$g N/m^2\f$
  ...
  real,intent(inout), dimension(ilg) :: bnf_moss           !< moss-associated biological nitrogen fixation, \f$g N m^{-2} day^{-1}\f$
  ...
  real :: ldoy  !< Last day of the year
  ...
  real,dimension(ilg) :: pNmossmas              !< previous moss nitrogen, \f$g N m^{-2}\f$
  real,dimension(ilg) :: pupMossSoilN           !< previous moss detrital N, \f$g N m^{-2}\f$
  real,dimension(ilg) :: plitrmsmossN           !< previous moss litter N, \f$g N m^{-2}\f$
  ...
  real, dimension(ilg) :: litrmsmoss_bulk   !< Moss litter C pool converted to total soil value (kgC/m2)
  real, dimension(ilg) :: upMossSoilC_bulk  !< Moss soil C mass (non-peat) converted to total soil value, \f$kg C/m^2\f$
  real, dimension(ilg) :: humstepmoss_bulk  !< moss humification (transfer from litter to SOM) converted to total soil value, (kgC/m2/timestep)
  real, dimension(ilg) :: ltrestepmoss_bulk !< litter respiration from moss converted to total soil value, (kgC/m2/timestep) 
  real, dimension(ilg) :: socres_moss_bulk  !< soil C respiration from moss converted to total soil value,  (\f$\mu mol CO_2 m^{-2} s^{-1}\f$) 
  real, dimension(ilg) :: nhumtrsmoss       !< nitrogen flux from litter to soil for moss, \f$g N m^{-2} day^{-1}\f$ 
  real, dimension(ilg) :: c2nMoss_litr      !< Moss simulated C:N ratio for detritus, \f$g C g N^{-1}\f$
  real, dimension(ilg) :: c2nMoss_humus     !< Moss simulated C:N ratio for SOM, \f$g C g N^{-1}\f$
  real, dimension(ilg) :: c2nMoss_live      !< Moss simulated C:N ratio for live tissue, \f$g C g N^{-1}\f$
  real, dimension(ilg) :: ndep_nh4_moss     !< daily nitrogen deposition into NH4+ for moss, \f$g N m^{-2} day^{-1}\f$
  real, dimension(ilg) :: ndep_no3_moss     !< daily nitrogen deposition into NO3- for moss, \f$g N m^{-2} day^{-1}\f$
  real :: Nmossmas_in                       !< moss biomass N pool initially, \f$g N/m^2\f$
  real, dimension(ilg) :: nmineralmoss_litr !< nitrogen mineralization from moss litter, \f$g N m^{-2} day^{-1}\f$
  real, dimension(ilg) :: nmineralmoss_humus!< nitrogen mineralization from moss SOM, \f$g N m^{-2} day^{-1}\f$
  real,dimension(ilg)  :: nlitrveg_moss     !< moss nitrogen litter fall rate, \f$g N m^{-2} day^{-1}\f$
  real,dimension(ilg)  :: exsc2n_moss       !< moss C:N ratio excess above maximum, \f$g C g N^{-1}\f$
  real,dimension(ilg)  :: ndemandmoss_npp   !< plant nitrogen demand for moss based on NPP, \f$g N m^{-2} day^{-1}\f$
  real,dimension(ilg)  :: depToMoss         !< Deposition to the moss, \f$g N m^{-2} day^{-1}\f$
...
!***************************************************************************************************************
  ...
  scresveg_bulk(:,:) = 0.0
   ...
   do k = 1,ignd
     do j = 1,iccp2
        do i = il1,il2
           soilcmas_bulk(i,j) = soilcmas_bulk(i,j) + soilcmas(i,j,k)
           litrmass_bulk(i,j) = litrmass_bulk(i,j) + litrmass(i,j,k)
           humtrsvg_bulk(i,j) = humtrsvg_bulk(i,j) + humtrsvg(i,j,k)
           ltresveg_bulk(i,j) = ltresveg_bulk(i,j) + ltresveg(i,j,k)
           scresveg_bulk(i,j) = scresveg_bulk(i,j) + scresveg(i,j,k)
        end do
     end do
   end do
 
  ! if moss present, do the same for its pools
  litrmsmoss_bulk = 0.0
  upMossSoilC_bulk = 0.0
  humstepmoss_bulk = 0.0
  ltrestepmoss_bulk = 0.0 
  socres_moss_bulk = 0.0
  if (any(mossPresent /= 'None')) then
    do j = 1,ignd
       do i = il1,il2
          litrmsmoss_bulk(i) = litrmsmoss_bulk(i) + litrmsmoss(i,j)
          upMossSoilC_bulk(i) = upMossSoilC_bulk(i) + upMossSoilC(i,j)
          humstepmoss_bulk(i) = humstepmoss_bulk(i) + humstepmoss(i,j)
          ltrestepmoss_bulk(i) = ltrestepmoss_bulk(i) + ltrestepmoss(i,j)
          socres_moss_bulk(i) = socres_moss_bulk(i) + socres_moss(i,j)
       end do
    end do
  end if
  ...
  do i = il1,il2
     ...
     bnf_free_g(i) = 0.0
     appl_fert_g(i) = 0.0
     c2nMoss_litr(i) = mossInitial_c2n
     c2nMoss_humus(i) = mossInitial_c2n
     c2nMoss_live(i) = mossInitial_c2n
     exsc2n_moss(i) = 0.
     depToMoss(i) = 0.
     do j = 1,iccp1
        appl_fert(i,j) = 0.0
  ...
  do i = il1, il2
   if (mossPresent(i) /= 'None') then ! Find the C:N for the moss pools
      ! live moss
      if (Nmossmas(i) > 1.E-3 .and. 1000. * Cmossmas(i) > 1.E-3) &
         c2nMoss_live(i) = Cmossmas(i) * 1000. / Nmossmas(i)
      ! detrital
      if (litrmsmossN(i) > 1.E-3 .and. 1000. * litrmsmoss_bulk(i) > 1.E-3) &
         c2nMoss_litr(i) = litrmsmoss_bulk(i) * 1000. / litrmsmossN(i)
      ! SOM
      if (upMossSoilN(i) > 1.E-3 .and. 1000. * upMossSoilC_bulk(i) > 1.E-3) &
         c2nMoss_humus(i) = upMossSoilC_bulk(i) * 1000. / upMossSoilN(i)
   end if 
  end do 
  ...
  ! Do the same for the moss N pools
  do i = il1,il2
    if (mossPresent(i) /= 'None') then
      if (abs(Nmossmas(i))  < zero) Nmossmas(i) = 0.0
      if (abs(upMossSoilN(i))  < zero) upMossSoilN(i) = 0.0
      if (abs(litrmsmossN(i))  < zero) litrmsmossN(i) = 0.0
      pNmossmas(i) = Nmossmas(i)
      pupMossSoilN(i) = upMossSoilN(i)
      plitrmsmossN(i) = litrmsmossN(i)
    end if 
  end do 
  ...
  do j = 1,iccp1
     do i = il1,il2
      ...
      ndep_nh4(i,j) = 0.5 * ndeposit(i) / ldoy
      ndep_no3(i,j) = 0.5 * ndeposit(i) / ldoy
      ...
      !! If the tile has moss, its coverage is 100% of the ground 
      !! so any deposition is given to the moss (after this loop), 
      !! otherwise add it to the NH4+ pool of the vascular PFT.
      if (mossPresent(i) == 'None') then 
         nh4_mass(i,j) = nh4_mass(i,j) + ndep_nh4(i,j)
      else 
         ndep_nh4(i,j) = 0. !set to zero for the accounting.
      end if 
      ...
      !! If the tile has moss, its coverage is 100% of the ground 
      !! so any NO3- deposition is given to the moss (after this loop), 
      !! otherwise add it to the NO3- pool of the vascular PFT.
      if (mossPresent(i) == 'None') then 
         no3_mass(i,j) = no3_mass(i,j) + ndep_no3(i,j)
      else 
         ndep_no3(i,j) = 0. !set to zero for the accounting.
      end if 
  ...
  !> Handle the deposition on the moss, if present.
  do i = il1, il2
      if (mossPresent(i) /= 'None') then 
         ! Calculate N deposition per PFT + bare soil:
         ndep_nh4_moss(i) = 0.5 * ndeposit(i) / ldoy
         ndep_no3_moss(i) = 0.5 * ndeposit(i) / ldoy

         Nmossmas_in = Nmossmas(i) 
         depToMoss(i) = ndep_nh4_moss(i) + ndep_no3_moss(i)
         Nmossmas(i) = Nmossmas(i) + depToMoss(i)

         if (Nmossmas(i) < 0.0) then
            write(*,*)'Nmossmas negative at i = ',i
            write(*,*)'Nmossmas(i,j) = ',Nmossmas(i)
            write(*,*)'ndep_nh4_moss = ',ndep_nh4_moss(i)
            write(*,*)'ndep_no3_moss = ',ndep_no3_moss(i)
            call errorHandler('n_processes', - 7)
         end if

         if ((Nmossmas_in - Nmossmas(i) + ndep_nh4_moss(i)  + ndep_no3_moss(i)) &
                                          > ntolerance) then
               write(*,*)'imbalance in moss deposition at (i)=',i
  ...
  call nitrific(il1, il2, zbotw, tbar, thliq, thice, thpor, & ! In
                THFC, sort, isand, peatlandType, & ! In
                psisat, bi,   & ! In
  ...
  call nuptake(il1, il2, sort, fcancmx, zbotw, delzw, & ! In
               ...
               nuptakeveg_nh4, nuptakeveg_no3, ndemandmoss_npp) ! Out

  !> **K.** Call the plant N allocation subroutine
  !! The N contribution to moss from their symbiotic cyanobacteria is calculated
  !! here and added to the moss N pool.
  call nallocate(il1, il2, fcancmx, lfstatus, sort, ailcg, & ! In
                 lfthrs, radj, iday, ntchlveg, ntchsveg, ntchrveg, & ! In
                 tltrleaf, ndemandveg_l_npp, ndemandmoss_npp, & ! In
                 ndemandveg_s_npp, ndemandveg_r_npp, ndemandveg_wp_npp, & ! In
                 nuptakeveg_nh4, nuptakeveg_no3, depToMoss, & ! In
                 gleafmas, rootmass_ns, mossPresent, Cmossmas, & ! In
                 re_alloc_sr2l, leafns2s, stemns2s, rootns2s, tbar, isand, & ! In
                 ngleafmas, ngleafmas_ns, ngleafmas_s, Nmossmas, & ! In/Out
                 ...
                 nre_allocveg_r2l, bnf_nat, bnf_ant, bnf_moss, nstress) ! Out

  !> **L.** Call the N litter fall subroutine
  call nlitter(il1, il2, sort, fcancmx, lfstatus, iday, & ! In
               tltrleaf,tltrstem, tltrroot, gl2bl_grass_cflux,  & ! In
               bl2ltr_grass_cflux, gleafmas, bleafmas, stemmass, & ! In
               rootmass, litrfallmoss, mossPresent, Cmossmas, & ! In
               ngleafmas, ngleafmas_ns, ngleafmas_s, nbleafmas, & ! In/Out
               nstemmass, nstemmass_ns, nstemmass_s, nrootmass, & ! In/Out
               nrootmass_ns, nrootmass_s, nlitrmass, litrmsmossN, Nmossmas, & ! In/Out
               gl2bl_grass_nflux, nlitrveg_l, & ! Out
               nlitrveg_s, nlitrveg_r, nlitrveg_moss) !Out

  !> **M.** Call the N humification subroutine
  call nhumific(il1, il2, sort, fcancmx, spinfast, humtrsvg_bulk, & ! In 
  ...
  do i = il1,il2
    ...
    if (abs(litrmsmossN(i)) < zero) litrmsmossN(i) = 0.
    if (abs(upMossSoilN(i)) < zero) upMossSoilN(i) = 0.
    if (abs(Nmossmas(i)) < zero) Nmossmas(i) = 0.
  end do ! loop 160
  ...
  do i = il1,il2
     ...
     ! If mosses present, add their contribution
     if (mossPresent(i) /= 'None') then 
        nlitrmass_g(i) = nlitrmass_g(i) + litrmsmossN(i)
        soilnmas_g(i) = soilnmas_g(i) + upMossSoilN(i)
        nhumtrs(i) = nhumtrs(i) + nhumtrsmoss(i)
        nmineral_litr(i) = nmineral_litr(i) + nmineralmoss_litr(i)
        nmineral_humus(i) = nmineral_humus(i) + nmineralmoss_humus(i)
        fNnetland(i) = fNnetland(i) + bnf_moss(i) + ndep_nh4_moss(i) + ndep_no3_moss(i)
        bnf_free_g(i) = bnf_free_g(i) + bnf_moss(i)
        litrmass_bulk_g(i) = litrmass_bulk_g(i) + litrmsmoss_bulk(i)
        soilcmas_bulk_g(i) = soilcmas_bulk_g(i) + upMossSoilC_bulk(i)
     end if 
  end do ! loop 180
  ...
  do i = il1,il2
     ...
     do j = 1,icc
      ...
      !! stem C:N ratio
      select case (pftkind)
      case ('NdlEvgTr','CNEvgTr','INEvgTr','NdlDcdTr','BdlEvgTr','BdlDCoTr',&
  ...
  do j = 1,icc
     ...
     do i = il1,il2
        ...
        case default
           print * ,'Unknown CTEM PFT in nprocesses ',pftkind
  ...
  !> Calculate the moss reduction coefficient of V_c,max similar to the vascular PFTs
  do i = il1, il2
   if (mossPresent(i) /= 'None') then 
      !! Excesses above maximum C:N ratios. 
      if (mossPresent(i) == 'Sphagnum') then 
         if (c2nMoss_live(i) < 2000. .and. c2nMoss_live(i) > c2n_Mossmax(1))&
                           exsc2n_moss(i) = c2nMoss_live(i) - c2n_Mossmax(1)
      else if (mossPresent(i) == 'Feather') then 
         if (c2nMoss_live(i) < 2000. .and. c2nMoss_live(i) > c2n_Mossmax(2))&
                           exsc2n_moss(i) = c2nMoss_live(i) - c2n_Mossmax(2)
      else 
         print*,'Unknown moss type in n_processes!', mossPresent(i)
         call errorHandler('nprocesses', - 15)
      end if 

      redcoeff_vcmaxMoss(i) = max(0.,min(1.,exp(-k_redcoeff_vcmax * exsc2n_moss(i)))) !FLAG for now use the vascular k_redcoeff_vcmax parameter, ROSE
   end if 
  end do

  !> **S.** Check if all N pools and fluxes are in balance and we are conserving N to within some specified tolerance
  !> The pools are leaf, stem, root, litter, soil, ammonium, and nitrate (and moss if present)
  if (spinfast == 1) then
     call balnit(il1, il2, iday, leapnow, radj, appl_fert, appl_fert_g, fcancmx,                          &
         ndeposit, bnf_free, bnf_free_g,                                                                  &
         ...
         nmineralmoss_litr, nmineralmoss_humus)
  end if
