...
subroutine energyBudgetPrep (THLIQC, THLIQG, THICEC, THICEG, TBARC, TBARG, & ! Formerly TPREP
                             ...
                             RSOIL, RSOILC)  
  ! 
  !     * Dec 05/25 - L. Wang.    Change calculation of TCSNOW to a function - 
  !                               snowThermalProp added in snowProcesses.
  ...
  use snowProcesses,       only : snowThermalProp
    ...
    do I = IL1,IL2
      if    (ISAND(I,1) == -4) then ! glacier
        ...
        if (THLIQG(I,J) > (THLRET(I,J) + 0.0001)) then
          ...
          if (J == 1 .and. peatlandType(i) == 'None' .and. DSL(I) > 0) then
            TCTOPG(I,J) = - ((TCSOIL - TCDRY) / DELTAZ) * DSL(I) + TCSOIL
          end if
          if (J == 1 .and. peatlandType(i) == 'None' .and. DSLC(I) > 0) then
  ...
  do i = il1, IL2
    if (peatlandType(i) /= 'None') then ! peatlands
