...
contains
  ...
  subroutine turnoverStemRoot (stemmass, stemmass_ns, stemmass_s, & ! In
    ...
    real, intent(in) :: tracerStemMass(ilg,icc)  !< Tracer mass in the stem for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$
    real, intent(in) :: tracerRootMass(ilg,icc)  !< Tracer mass in the roots for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$
    ...
    real, intent(inout) :: rothrlos(ilg,icc)  !< root death for crops. when in "harvest" mode for crops, root is assumed
                                              !! to die in a similar way as stem is harvested. (1/day)
    real, intent(inout) :: tracerRothrlos(ilg,icc)   !< tracer root death for crops. When in "harvest" mode for crops, root is assumed
                                              !! to die in a similar way as stem is harvested. (1/day)
    real, intent(inout) :: stmhrlos(ilg,icc)  !< stem harvest loss for crops. when in "harvest" mode for crops, stem is
                                              !! also assumed to be harvested and this generates litter. (1/day)
    real, intent(inout) :: tracerStmhrlos(ilg,icc)   !< tracer stem harvest loss for crops. (1/day)
    ...
    real, intent(out) :: tracerStemLitr(ilg,icc) !< Tracer stem litter14C:  \f$ng ^{14}C/m^2\f$
    real, intent(out) :: tracerRootLitr(ilg,icc) !< Tracer root litter14C:  \f$ng ^{14}C/m^2\f$
    ...
    real :: tracerNrmlsmlr(ilg, icc)  !< tracer stem litter from normal turnover
    ...
    real :: tracerNrmlrtlr(ilg, icc)  !< root litter from normal turnover
    real :: stmhrlos_ns(ilg,icc)      !< non-structural stem harvest loss for crops. (1/day)
    real :: stmhrlos_s(ilg,icc)       !< structural stem harvest loss for crops. (1/day)
    real :: rothrlos_ns(ilg,icc)      !< non-structural root death for crops. (1/day)
    real :: rothrlos_s(ilg,icc)       !< structural root death for crops. (1/day)
    real :: term                      !< temp. var.
    real :: ldoy                      !< last doy
      ...
      do i = il1,il2
        if (fcancmx(i,j) > 0.0) then
          ...
          if (stemlife(n) > zero) then
            term = 1.0 - exp( - 1.0 / (ldoy * stemlife(n)))
            nrmlsmlr_ns(i,j) = stemmass_ns(i,j) * term
            nrmlsmlr_s(i,j)  = stemmass_s(i,j)  * term
            if (useTracer /= 'None') tracerNrmlsmlr(i,j) = tracerStemMass(i,j) * term
          end if

          if (rootlife(n) > zero) then
            term = 1.0 - exp( - 1.0 / (ldoy * rootlife(n)))
            nrmlrtlr_ns(i,j) = rootmass_ns(i,j) * term
            nrmlrtlr_s(i,j)  = rootmass_s(i,j)  * term
            if (useTracer /= 'None') tracerNrmlrtlr(i,j) = tracerRootMass(i,j) * term
          end if
        end if
      end do ! loop 210
    end do ! loop 200
        ...
        do i = il1,il2
          if (fcancmx(i,m) > 0.0) then
            ...
            case ('NdlTr' , 'BdlTr', 'Grass', 'BdlSh')
              ...
              ! Do nothing, already set to 0 at start.

            case ('Crops')

              if (lfstatus(i,m) == 3) then
                if (stmhrlos(i,m) <= zero .and. stemmass(i,m) > zero) then 
                  stmhrlos(i,m) = stemmass(i,m) * (1.0 / stmhrspn)
                  if (useTracer /= 'None') tracerStmhrlos(i,m) = tracerStemMass(i,m) * (1.0 / stmhrspn)
                end if 
                if (rothrlos(i,m) <= zero .and. rootmass(i,m) > zero) then
                  rothrlos(i,m) = rootmass(i,m) * (1.0 / stmhrspn)
                  if (useTracer /= 'None') tracerRothrlos(i,m) = tracerRootMass(i,m) * (1.0 / stmhrspn)
                end if 
              end if

              !> Next figure out how much stmhrlos is structural vs. non.
              ...
              if (stemmass(i,m) <= zero .or. lfstatus(i,m) == 1 .or. lfstatus(i,m) == 2) then
                ! reset the stmhrlos variables to 0.
                stmhrlos(i,m) = 0.0
                stmhrlos_ns(i,m) = 0.0
                stmhrlos_s(i,m)  = 0.0
                if (useTracer /= 'None') tracerStmhrlos(i,m) = 0.0
              end if
              if (rootmass(i,m) <= zero .or. lfstatus(i,m) == 1 .or. lfstatus(i,m) == 2) then
                ! reset the rothrlos variables to 0.
                rothrlos(i,m) = 0.0
                rothrlos_ns(i,m) = 0.0
                rothrlos_s(i,m)  = 0.0
                if (useTracer /= 'None') tracerRothrlos(i,m) = 0.0
              end if
    ...
    do j = 1,icc ! loop 350
      do i = il1,il2 ! loop 360
        if (fcancmx(i,j) > 0.0) then
          ...
          stemlitr(i,j) = stemlitr_ns(i,j) + stemlitr_s(i,j)
          if (useTracer /= 'None') tracerStemLitr(i,j) = tracerNrmlsmlr(i,j) + tracerStmhrlos(i,j)
          ...
          rootlitr(i,j) = rootlitr_ns(i,j) + rootlitr_s(i,j)
          if (useTracer /= 'None') tracerRootLitr(i,j) = tracerNrmlrtlr(i,j) + tracerRothrlos(i,j)
        end if
      end do ! loop 360
    end do ! loop 350
  ...
  subroutine updatePoolsTurnover (il1, il2, ilg, reprocost, rmatctem, useTracer, tracerReproCost, & ! In
    ...
    !! NPP is positive (\f$ng ^{14}C m^{-2} s^{-1}\f$)
    ...
    real, intent(inout) :: tracerStemLitr(:,:) !< Tracer stem litter14C:  \f$ng ^{14}C/m^2\f$
    real, intent(inout) :: tracerRootLitr(:,:) !< Tracer root litter14C:  \f$ng ^{14}C/m^2\f$
      ...
      do m = reindexPFTs(j,1),reindexPFTs(j,2)
        do i = il1,il2
          ...
          case ('NdlTr' , 'BdlTr', 'Crops', 'BdlSh') ! tree,crops and shrub
            ...
            if (useTracer /= 'None') tracerGLeafMass(i,m) = tracerGLeafMass(i,m) - tracerLeafLitr(i,m)
            ...
            if (gleafmas(i,m) < 0.0 .or. gleafmas_ns(i,m) < 0.0 .or. gleafmas_s(i,m) < 0.0) then
              ...
              if (useTracer /= 'None') then
                ! If the total pools are being reset, enforce this on the tracer.
          ...
          case ('Grass')
            ...
            if (useTracer /= 'None') tracerBLeafMass(i,m) = tracerBLeafMass(i,m) - tracerLeafLitr(i,m)
    ...
    do j = 1,icc ! loop 780
      do i = il1,il2 ! loop 790
        ...
        if (useTracer /= 'None') tracerStemMass(i,j) = tracerStemMass(i,j) - tracerStemLitr(i,j)
        ...
        if (useTracer /= 'None') tracerRootMass(i,j) = tracerRootMass(i,j) - tracerRootLitr(i,j)
      ...
      do j = 1,icc ! loop 805
        do k = 1,ignd
          if (k == 1) then
            ...
            if (useTracer /= 'None') tracerLitrMass(i,j,k) = tracerLitrMass(i,j,k) + tracerLeafLitr(i,j) + tracerStemLitr(i,j) &
            ...
            if (useTracer /= 'None') tracerLitrMass(i,j,k) = tracerLitrMass(i,j,k) + tracerRootLitr(i,j) * rmatctem(i,j,k)
          end if
        end do 
      end do ! loop 805
    end do ! loop 800
