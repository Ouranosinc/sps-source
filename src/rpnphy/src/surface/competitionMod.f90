...
contains
  ...
  subroutine existence (iday, il1, il2, nilg, sort, twarmm, tcoldm, gdd5, &
    ...
    do i = il1,il2 ! loop 100
      do j = 1,icc ! loop 102
        ...
        case ('NdlEvgTr','CNEvgTr','INEvgTr') !> needleleaf evergreen
  ...
  subroutine competition (iday, il1, il2, nilg, nppveg, dofire, leapnow, useTracer, &! In
    ...
    use classicParams,       only : zero, kk, numcrops, numgrass, numtreepfts, &
                                    ...
                                    nol2pfts, reindexPFTs, minbare, c_switch
    ...
    real, intent(inout) :: tracerGLeafMass(:,:)      !< Tracer mass in the green leaf pool for each of the CTEM pfts, 14C: \f$nkg ^{14}C/m^2\f$
    real, intent(inout) :: tracerBLeafMass(:,:)      !< Tracer mass in the brown leaf pool for each of the CTEM pfts, 14C: \f$nkg ^{14}C/m^2\f$
    real, intent(inout) :: tracerStemMass(:,:)       !< Tracer mass in the stem for each of the CTEM pfts, 14C: \f$nkg ^{14}C/m^2\f$
    real, intent(inout) :: tracerRootMass(:,:)       !< Tracer mass in the roots for each of the CTEM pfts, 14C: \f$nkg ^{14}C/m^2\f$
    real, intent(inout) :: tracerLitrMass(:,:,:)     !< Tracer mass in the litter pool for each of the CTEM pfts + bareground and LUC products, 14C: \f$nkg ^{14}C/m^2\f$
    real, intent(inout) :: tracerSoilCMass(:,:,:)    !< Tracer mass in the soil carbon pool for each of the CTEM pfts + bareground and LUC products, 14C: \f$nkg ^{14}C/m^2\f$
    ...
    real :: tracerGrSumLit(nilg,ignd)  !14C: \f$nkg ^{14}C/m^2\f$
    real :: tracerGrSumSOC(nilg,ignd)  !14C: \f$nkg ^{14}C/m^2\f$
    real :: tracerIncrLitr(nilg,iccp1,ignd) !14C: \f$nkg ^{14}C/m^2\f$
    real :: tracerIncrSolC(nilg,iccp1,ignd) !14C: \f$nkg ^{14}C/m^2\f$
  ...
  !! \f[\label{concepteqn} \frac{\mathrm{d}f_\alpha}{\mathrm{d}t} = g(f_\alpha, f_B) - m_{\alpha} f_\alpha.\qquad (Eqn 1)
  !! \f]
  !!
  ...
  !!
  !! \f[\label{efold} X(t+1)=X(t)e^{-1/T} + x(t) (1 - e^{-1/T}).\qquad (Eqn 17)\f]
  !!
  ...
  !! of the change is reflected in \f$2T\f$ years \f$(1-e^{2T(-1/T)}= 1-e^{-2} = 0.86)\f$.
  !!
end module
