...
contains
  ...
  subroutine main_core_driver(class_gat, class_rot, vrot, vgat, ctem_tile, tracer, &
                              nltest, NML, NDAY, ncount, IDAY, lastDOY, leapnow, N, IMONTH, DOM)

    use classicParams,      only : nlat, nmos, ilg, ican, ignd, icc, l2max, NBS, nol2pfts, inibioclim, c_switch
    use ctemStateVars,      only : veg_rot, veg_gat, ctem_tile_level, tracersType, resetMosaicAccum
    use classStateVars,     only : class_gather, class_rowtile, initDiagnosticVars
    use classGatherScatter, only : classGather, classScatter, classGatherPrep
    use ctemGatherScatter,  only : ctems1, ctems2, ctemg1, ctemg2
    ...
    SFCTDGAT => class_gat%SFCTDGAT,                     & !< real, dimension(:) : Diagnosed screen-level dew point temperature [K]
    ...
    maxSNOGAT => class_gat%maxSNOGAT,                   & !< real, dimension(:) : Effective maximum snow mass used to reconcile snow accumulation and recession curves [kg m^{-2} ]\f$
    stddevTopographyGAT => class_gat%stddevTopographyGAT, & !< real, dimension(:) : Standard deviation of elevation [m]
    ...
    snwct   => class_gat%snwct,                         & !< real, dimension(:) : 
    ...
    TBARC   => class_gat%TBARC,                         & !< real(r8), dimension(:,:) : Temperature of soil layers for ground under canopy subarea [K] 
    TBARG   => class_gat%TBARG,                         & !< real(r8), dimension(:,:) : Temperature of soil layers for bareground subarea [K] 
    TBARCS  => class_gat%TBARCS,                        & !< real(r8), dimension(:,:) : Temperature of soil layers for snow-covered ground under canopy subarea [K] 
    TBARGS  => class_gat%TBARGS,                        & !< real(r8), dimension(:,:) : Temperature of soil layers for bareground subarea [K] 
    ...
    BCSNROW => class_rot%BCSNROW,                       & !< real, dimension(:) : Black carbon mixing ratio in snow \f$[kg m^{-2} s^{-1}]\f$ 
    ...
    REFROT  => class_rot%REFROT,                        & !< real, dimension(:,:) : Snow grain size (for snoAlbedoParam=`4-band' option)  [m] 
    ...
    SFCTDROT => class_rot%SFCTDROT,                     & !< real, dimension(:,:) : Diagnosed screen-level dew point temperature [K]
    ...
    maxSNOROT => class_rot%maxSNOROT,                   & !< real, dimension(:,:) : Effective maximum SWE to reconcile accumulation and ablation curves [kg m^{-2} ]\f$
    stddevTopographyROT => class_rot%stddevTopographyROT, & !< real, dimension(:,:) : Standard deviation of elevation [m]
    ...
    IALG              => c_switch%IALG,                 & !< integer: if ipai, ihgt, ialc, ials and ialg are zero, the values of
                                                          ...
                                                          !!          corresponding parameter calculated by class is overridden by
                                                          !!          a user-supplied input value. 
    fracSnowParam     => c_switch%fracSnowParam,        & !< character: if fracSnowParam = 'Default', the original default snow cover scheme is used. 
                                                          !!            if fracSnowParam= 'SL12', the SL12 snow cover scheme is used.
    snoAlbedoParam    => c_switch%snoAlbedoParam,       & !< character: chooses the snow albedo parameterization: '2-band' = the original CLASS two-band snow albedo algorithms are used.
                                                          !< '4-band' uses the four-band routines from CanESM.
    ...
    EcTResistancePart => c_switch%EcTResistancePart,    & !< Switch determining whether to use the leaf boundary layer and stomatal resistances to determine the
                                                          !! partitioning into canopy evaporation and transpiration or the previous standard approach, where  transpiration 
                                                          !! only occurs, when there is no intercepted water on the canopy; set to false for original calculation 
    ...
    NcycleConnect_on  => c_switch%NcycleConnect_on,     & !< logical:
    dofire            => c_switch%dofire,               & !< logical: 
    fireSubmodule     => c_switch%fireSubmodule,        & !< character(10):
    ...
    co2i1cgrow        => vrot%co2i1cg,                  & !< real, dimension(:,:,:) : Intercellular CO2 concentration for CTEM PFTs for canopy over ground subarea (Pa) - for single/sunlit leaf
    co2i1csrow        => vrot%co2i1cs,                  & !< real, dimension(:,:,:) : Intercellular CO2 concentration for CTEM PFTs for canopy over snow subarea (Pa) - for single/sunlit leaf
    co2i2cgrow        => vrot%co2i2cg,                  & !< real, dimension(:,:,:) : Intercellular CO2 concentration for CTEM PFTs for canopy over ground subarea (Pa) - for shaded leaf
    co2i2csrow        => vrot%co2i2cs,                  & !< real, dimension(:,:,:) : Intercellular CO2 concentration for CTEM PFTs for canopy over snow subarea (Pa) - for shaded leaf
    ...
    peatlandTyperow   => vrot%peatlandType,             & !< character(8), dimension(:,:) : Peatland type (else 'None')
    mossPresentrow    => vrot%mossPresent,              & !< character(8), dimension(:,:) : Type of moss present (else 'None')
    litrmsmossrow     => vrot%litrmsmoss,               & !< real, dimension(:,:,:) : Moss litter C mass, \f$kg C/m^2\f$ 
    Cmossmasrow       => vrot%Cmossmas,                 & !< real, dimension(:,:) : Moss C mass, \f$kg C/m^2\f$ 
    avesmfuncrow      => vrot%avesmfunc,                   & !< real, dimension(:,:,:) :
    ...
    ! FLAG: EC - not used and note that gat version is not initialized before being passed to energyBudgetDriver.
    !redcoeff_vcmaxMossrow  => vrot%redcoeff_vcmaxMoss, & !< real, dimension(:,:) :
    ...
    fNleachrow        => vrot%fNleach,                  & !< real, dimension(:,:) :
    fNVolrow          => vrot%fNVol,                    & !< real, dimension(:,:) :
    ...
    co2i1cggat        => vgat%co2i1cg,                  & !< real, dimension(:,:) : Intercellular CO2 concentration for CTEM PFTs for canopy over ground subarea (Pa) - for single/sunlit leaf
    co2i1csgat        => vgat%co2i1cs,                  & !< real, dimension(:,:) : Intercellular CO2 concentration for CTEM PFTs for canopy over snow subarea (Pa) - for single/sunlit leaf
    co2i2cggat        => vgat%co2i2cg,                  & !< real, dimension(:,:) : Intercellular CO2 concentration for CTEM PFTs for canopy over ground subarea (Pa) - for shaded leaf
    co2i2csgat        => vgat%co2i2cs,                  & !< real, dimension(:,:) : Intercellular CO2 concentration for CTEM PFTs for canopy over snow subarea (Pa) - for shaded leaf
    ...
    avesmfuncgat      => vgat%avesmfunc,                   & !< real, dimension(:,:) : 
    redcoeff_vcmaxgat => vgat%redcoeff_vcmax,           & !< real, dimension(:,:) : 
    redcoeff_vcmaxMossgat => vgat%redcoeff_vcmaxMoss,   & !< real, dimension(:)

    peatlandTypegat  => vgat%peatlandType,              & !< character(8), dimension(:) : Peatland type (else 'None')
    mossPresentgat   => vgat%mossPresent,               & !< character(8), dimension(:) : Type of moss present (else 'None') 
    Cmossmasgat      => vgat%Cmossmas,                  & !< real, dimension(:) : Moss C mass, \f$kg C/m^2\f$
    Nmossmasgat      => vgat%Nmossmas,                  & !< real, dimension(:) : Moss N mass, \f$g N/m^2\f$
    dmossgat         => vgat%dmoss,                     & !< real, dimension(:) : 
    fNleachgat       => vgat%fNleach,                   & !< real, dimension(:) :
    fNVolgat         => vgat%fNVol,                     & !< real, dimension(:) :
    ...
    tracerMossCMassrot   => tracer%mossCMassrot,        & !< real, dimension(:,:) : Tracer mass in moss biomass, 14C: \f$ng ^{14}C/m^2\f$ 
    tracerMossLitrMassrot => tracer%mossLitrMassrot,    & !< real, dimension(:,:) : Tracer mass in moss litter, 14C: \f$ng ^{14}C/m^2\f$ 

    tracerGLeafMassrot   => tracer%gLeafMassrot,        & !< real, dimension(:,:,:) : Tracer mass in the green leaf pool for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$ 
    tracerBLeafMassrot   => tracer%bLeafMassrot,        & !< real, dimension(:,:,:) : Tracer mass in the brown leaf pool for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$ 
    tracerStemMassrot    => tracer%stemMassrot,         & !< real, dimension(:,:,:) : Tracer mass in the stem for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$ 
    tracerRootMassrot    => tracer%rootMassrot,         & !< real, dimension(:,:,:) : Tracer mass in the roots for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$ 
    tracerRothrlosrot    => tracer%rothrlosrot,         & !< real, dimension(:,:,:) : tracer root death as crops are harvested, 14C: \f$ng ^{14}C/m^2/d\f$
    tracerStmhrlosrot    => tracer%stmhrlosrot,         & !< real, dimension(:,:,:) : tracer stem harvest loss for crops, 14C: \f$ng ^{14}C/m^2/d\f$
    tracerFlHrlossrot    => tracer%FlHrLossrot,         & !< real, dimension(:,:,:) : Tracer fall & harvest loss for bdl dcd plants and crops, respectively, \f$tracer units/m^2/d\f$.

    ! allocated with nlat, nmos, iccp2, ignd:
    tracerLitrMassrot    => tracer%litrMassrot,         & !< real, dimension(:,:,:,:) : Tracer mass in the litter pool for each of the CTEM pfts + bareground and LUC products, 14C: \f$ng ^{14}C/m^2\f$ 
    tracerSoilCMassrot   => tracer%soilCMassrot         & !< real, dimension(:,:,:,:) : Tracer mass in the soil carbon pool for each of the CTEM pfts + bareground and LUC products, 14C: \f$ng ^{14}C/m^2\f$ 
    )

    !    =================================================================================

    ! Set the DynTilInitializeFlag to false to ensure the model isn't accidentaly re-initialized
    ...
    if (timberHarvest .or. dynamicTilingOn .or. prescribedFire) then
      call tiledDisturbancePrep(ctem_on, lnduseon, PFTCompetition, timberHarvest, prescribedFire, &
                            dynamicTilingOn, iday, controlVector, DynTilInitializeFlag, timharvrow, & 
                            ...
                            flhrloss_nsrow, flhrloss_srow, gleafmas_NSrow, gleafmassrow, grwtheffrow, peatlandTyperow, & 
                            ...
                            nstemmasssrow, pandaysrow, peatSoilCrow, rootmass_NSrow, &
                            rootmasssrow, rothrlosrow, slopefracrow, &
                            soilcmasrow, soilnmasrow, stemmass_NSrow, stemmasssrow, stmhrlosrow, &
                            tracerBLeafMassrot, tracerGLeafMassrot, tracerLitrMassrot, &
                            tracerMossCMassrot, tracerMossLitrMassrot, tracerRootMassrot, &
                            tracerSoilCMassrot, tracerStemMassrot, &
                            tymaxlairow, RSMNROT, QA50ROT, VPDAROT, VPDBROT, PSGAROT, PSGBROT,&
                            ngleafmasrow, nstemmassrow, nrootmassrow, soilpHrow, gleafmasrow, &
                            stemmassrow, rootmassrow, flhrlossrow, TBASROT, CMAIROT, &
                            WSNOROT, ZSNLROT, TSFSROT, TACROT, QACROT, ITCTROT, DLZWROT, &
                            useTracer, Ncycle_on, DELZ, THPROT, HCPSROT, tileAgerow, tileAgegat, NDAY, NCOUNT, veghghtrow, DOM)
    end if
    ...
    if ((N == 0) .or. DynTilInitializeFlag) then
      ...
      !NOTE: This is called outside of CLASSIC by the AGCM driver.
      ...
      call soilProperties(THPROT, THRROT, THMROT, BIROT, PSISROT, GRKSROT, & ! Formerly CLASSB
                          ...
                          NLAT, NMOS, 1, NLTEST, NMTEST, IGND, peatlandTyperow, &
                          mossPresentrow, KsatScalaron)
#endif
      ...
      call findPermafrostVars(nmtest, nltest, iday, class_rot, class_gat)
    ...
    ! Ensure that all elements of the gat arrays dimensioned ilg are initialized to None or else statements
    ! like the following could fail even when all elements of the associated rot arrays are set to None: 
    !   if (any(peatlandTypegat /= 'None')) 
    ! This is because the gather operates from 1 to nml and ilg can be > nml, which is the case for CanESM.
    peatlandTypegat = 'None'
    mossPresentgat = 'None'
    ...
    if (N == 0) then
      ...
      !if (ctem_on) then
        ...
        call allometry(gleafmasgat, gleafmasgat_ns, gleafmasgat_s, & ! In
                       ...
                       peatlandTypegat, maxAnnualActLyrGAT, & ! In
    ...
    call classGather(TBARGAT, THLQGAT, THICGAT, TPNDGAT, ZPNDGAT, & ! Formerly CLASSG
                     ...
                     THFCGAT, THLWGAT, PSIWGAT, DLZWGAT, &
                     ...
                     DLATGAT, maxAnnualActLyrGAT, FSNOGAT, maxSNOGAT, &
                     stddevTopographyGAT, ILMOS, JLMOS, NML, NLAT, NTLD, NMOS, &
                     ...
                     THFCROT, THLWROT, PSIWROT, DLZWROT, &
                     ...
                     DLATROW, maxAnnualActLyrROT, FSNOROT, maxSNOROT, &
                     stddevTopographyROT)
    ...
    call initDiagnosticVars(class_gat, nml, ignd, ilg, nbs)
    ...
    call energyWaterBalanceCheck(0, CTVSTP, CTSSTP, CT1STP, CT2STP, CT3STP, & ! Formerly CLASSZ
                                 ...
                                 DELZ, FCS, FGS, FC, FG, DLONGAT, DLATGAT, &
                                 1, NML, ILG, IGND, N)
#endif
    ...
    !     * cmasvegc, alvsctm, alirctm, peatlandTypegat in the arguments.
    ...
    call radiationDriver(FC, FG, FCS, FGS, ALVSCN, ALIRCN, & ! Formerly CLASSA
                         ...
                         fracSnowParam, snoAlbedoParam, alvsctmgat, alirctmgat, mossPresentgat, &
    ...
    call energyBudgetDriver(TBARC, TBARG, TBARCS, TBARGS, THLIQC, THLIQG, & ! Formerly CLASST
                            ...
                            SFCTGAT, SFCTDGAT, SFCUGAT, SFCVGAT, SFCQGAT, SFRHGAT, &
                            ...
                            TADPGAT, RHOAGAT, FSVHGAT, FSIHGAT, FDLGAT, ULGAT, VLGAT, &
                            ...
                            ANCSVEGGAT, ANCGVEGGAT, RMLCSVEGGAT, RMLCGVEGGAT, & !out
                            TCSNOW, GSNOW, ITC, ITCG, ITG, ILG, 1, NML, JLAT, N, ICAN, &
                            IGND, IZREF, ISLFD, NLANDCS, NLANDGS, NLANDC, NLANDG, NLANDI, &
                            NBS, snoAlbedoParam, daylgat, dayl_maxgat, DLONGAT, DLATGAT, &
                            peatlandTypegat, mossPresentgat, ancsmoss, angsmoss, ancmoss, angmoss, &
                            rmlcsmoss, rmlgsmoss, rmlcmoss, rmlgmoss, &
                            Cmossmasgat, dmossgat, iday, redcoeff_vcmaxgat, &
                            redcoeff_vcmaxMossgat, Nmossmasgat, &
                            GLEAFMASGAT, NGLEAFMASGAT, Ncycle_on, NcycleConnect_on, vcmax0gat, &
                            PSISGAT, BIGAT, DSL, DSLC, RB, avesmfuncgat)
    ...
    call waterBudgetDriver(THLQGAT, THICGAT, TBARGAT, TCANGAT, RCANGAT, SCANGAT, & ! Formerly CLASSW
                           ...
                           DLZWGAT, ZBTWGAT, ISNDGAT, IGDRGAT, DLONGAT, DLATGAT, &
                           IWF, ILG, 1, NML, N, &
                           JLAT, IGND, IGND + 1, IGND + 2, &
                           NLANDCS, NLANDGS, NLANDC, NLANDG, NLANDI, &
                           RB, RC, RCS, FRAINC, FSNOWC, FRAICS, FSNOCS, &  
                           LAIPAIRatio, LAISPAISRatio,FSNOGAT, maxSNOGAT, &
                           ZSNLGAT, stddevTopographyGAT, fracSnowParam, TCSNOW)  
    ...
    if (ctem_on) then
      ...
      call accumulateForCTEM(class_gat, vgat, ctem_tile, nml, dofire)
      ...
      emit_so2row       => vrot%emit_so2,                 & !< real, dimension(:,:,:) :
      elcgrow           => vrot%elcgrow,                  & !< real, dimension(:) : online cloud-to-ground lightning \f$flashes/km^2/year\f$
      ...
      drgtstrsrow       => vrot%drgtstrs,                 & !< real, dimension(:,:,:) : 
      betadrgtrow       => vrot%betadrgt,                 & !< real, dimension(:,:,:) : 
      ...
      cproductrow       => vrot%cproduct,                 & !< real, dimension(:,:) : 
      fproductdecomprow => vrot%fproductdecomp,           & !< real, dimension(:,:) : 
      cleafrow          => vrot%cleaf,                    & !< real, dimension(:,:) :
      cstemrow          => vrot%cstem,                    & !< real, dimension(:,:) :
      crootrow          => vrot%croot,                    & !< real, dimension(:,:) :
      nppleafrow        => vrot%nppleaf,                  & !< real, dimension(:,:) :
      nppstemrow        => vrot%nppstem,                  & !< real, dimension(:,:) :
      npprootrow        => vrot%npproot,                  & !< real, dimension(:,:) :
      nMineralNH4row    => vrot%nMineralNH4,              & !< real, dimension(:,:) :
      nMineralNO3row    => vrot%nMineralNO3,              & !< real, dimension(:,:) :
      nVegrow           => vrot%nVeg,                     & !< real, dimension(:,:) :
      nLitterrow        => vrot%nLitter,                  & !< real, dimension(:,:) :
      nSoilrow          => vrot%nSoil,                    & !< real, dimension(:,:) :
      nLeafrow          => vrot%nLeaf,                    & !< real, dimension(:,:) :
      nStemrow          => vrot%nStem,                    & !< real, dimension(:,:) :
      nRootrow          => vrot%nRoot,                    & !< real, dimension(:,:) :
      fBNFrow           => vrot%fBNF,                     & !< real, dimension(:,:) :
      fNdeprow          => vrot%fNdep,                    & !< real, dimension(:,:) :
      fNfertrow         => vrot%fNfert,                   & !< real, dimension(:,:) :
      fNgasNonFirerow   => vrot%fNgasNonFire,             & !< real, dimension(:,:) :
      fNgasFirerow      => vrot%fNgasFire,                & !< real, dimension(:,:) :
      fNgasrow          => vrot%fNgas,                    & !< real, dimension(:,:) :
      fNnetminrow       => vrot%fNnetmin,                 & !< real, dimension(:,:) :
      fNOxrow           => vrot%fNOx,                     & !< real, dimension(:,:) :
      fNuprow           => vrot%fNup,                     & !< real, dimension(:,:) :
      fNvegSoilrow      => vrot%fNvegSoil,                & !< real, dimension(:,:) :
      fN2orow           => vrot%fN2o,                     & !< real, dimension(:,:) :
      ...
      fFireCVegrow      => vrot%fFireCVeg,                & !< real, dimension(:,:,:) : Live biomass combusted by fire, \f$kg c/m^2\f$
      fFireLitterrow    => vrot%fFireLitter,              & !< real, dimension(:,:,:) : above ground necromass combusted by fire (considers layer 1 litter as surface), \f$kg c/m^2\f$
      fFireCSoilrow     => vrot%fFireCSoil,               & !< real, dimension(:,:,:) : soil carbon combusted by fire (considers soil c + litter(2:ignd)), \f$kg c/m^2\f$
      ...
      litrmsmossNrow   => vrot%litrmsmossN,               & !< real, dimension(:,:) : Moss litter N mass, \f$g N/m^2\f$ 
      upMossSoilNrow   => vrot%upMossSoilN,               & !< real, dimension(:,:) : Moss soil N mass (non-peat), \f$g N/m^2\f$
      Nmossmasrow      => vrot%Nmossmas,                  & !< real, dimension(:,:) : Moss N mass, \f$g N/m^2\f$

      bnffreerow          => vrot%bnf_free,               & !< real, dimension(:,:,:) :
      bnftotrow           => vrot%bnf_tot,                & !< real, dimension(:,:,:) :
      bnf_mossrow         => vrot%bnf_moss,               & !< real, dimension(:,:) :
      ...
      emit_so2gat       => vgat%emit_so2,                 & !< real, dimension(:,:) : 
      fiengat           => vgat%fiengat,                  & !< real, dimension(:,:) : Fire energy per PFT \f$J\f$ 
      fabgat            => vgat%fabgat,                   & !< real, dimension(:) : Fire area burned \f$km^2\f$   
      elcggat           => vgat%elcggat,                  & !< real, dimension(:) : prognostic cloud-to-ground lightning \f$flashes/km^2/year\f$
      nmfigat           => vgat%nmfigat,                  & !< real, dimension(:,:) : number of fires per grid cell
      ...
      drgtstrsgat       => vgat%drgtstrs,                 & !< real, dimension(:,:,:) :
      betadrgtgat       => vgat%betadrgt,                 & !< real, dimension(:,:,:) :
      ...
      cproductgat       => vgat%cproduct,                 & !< real, dimension(:,:) : 
      fproductdecompgat => vgat%fproductdecomp,           & !< real, dimension(:,:) : 
      cleafgat          => vgat%cleaf,                    & !< real, dimension(:,:) :
      cstemgat          => vgat%cstem,                    & !< real, dimension(:,:) :
      crootgat          => vgat%croot,                    & !< real, dimension(:,:) :
      nppleafgat        => vgat%nppleaf,                  & !< real, dimension(:,:) :
      nppstemgat        => vgat%nppstem,                  & !< real, dimension(:,:) :
      npprootgat        => vgat%npproot,                  & !< real, dimension(:,:) :
      nMineralNH4gat    => vgat%nMineralNH4,              & !< real, dimension(:,:) :
      nMineralNO3gat    => vgat%nMineralNO3,              & !< real, dimension(:,:) :
      nVeggat           => vgat%nVeg,                     & !< real, dimension(:,:) :
      nLittergat        => vgat%nLitter,                  & !< real, dimension(:,:) :
      nSoilgat          => vgat%nSoil,                    & !< real, dimension(:,:) :
      nLeafgat          => vgat%nLeaf,                    & !< real, dimension(:,:) :
      nStemgat          => vgat%nStem,                    & !< real, dimension(:,:) :
      nRootgat          => vgat%nRoot,                    & !< real, dimension(:,:) :
      fBNFgat           => vgat%fBNF,                     & !< real, dimension(:,:) :
      fNdepgat          => vgat%fNdep,                    & !< real, dimension(:,:) :
      fNfertgat         => vgat%fNfert,                   & !< real, dimension(:,:) :
      fNgasNonFiregat   => vgat%fNgasNonFire,             & !< real, dimension(:,:) :
      fNgasFiregat      => vgat%fNgasFire,                & !< real, dimension(:,:) :
      fNgasgat          => vgat%fNgas,                    & !< real, dimension(:,:) :
      fNnetmingat       => vgat%fNnetmin,                 & !< real, dimension(:,:) :
      fNOxgat           => vgat%fNOx,                     & !< real, dimension(:,:) :
      fNupgat           => vgat%fNup,                     & !< real, dimension(:,:) :
      fNvegSoilgat      => vgat%fNvegSoil,                & !< real, dimension(:,:) :
      fN2ogat           => vgat%fN2o,                     & !< real, dimension(:,:) :
      ...
      fFireCVeg         => vrot%fFireCVeg,                & !< real, dimension(:,:) : Live biomass combusted by fire, \f$kg c/m^2\f$
      fFireLitter       => vrot%fFireLitter,              & !< real, dimension(:,:) : above ground necromass combusted by fire (considers layer 1 litter as surface), \f$kg c/m^2\f$
      fFireCSoil        => vrot%fFireCSoil,               & !< real, dimension(:,:) : soil carbon combusted by fire (considers soil c + litter(2:ignd)), \f$kg c/m^2\f$
      ...
      upMossSoilCgat   => vgat%upMossSoilC,            & !< real, dimension(:,:) : Moss soil C mass (non-peat), \f$kg C/m^2\f$
      litrmsmossNgat   => vgat%litrmsmossN,            & !< real, dimension(:) : Moss N litter mass, \f$g N/m^2\f$
      upMossSoilNgat   => vgat%upMossSoilN,            & !< real, dimension(:) : Moss soil N mass (non-peat), \f$g N/m^2\f$
      ...
      bnf_mossgat         => vgat%bnf_moss,               & !< real, dimension(:) :
      ...
      ! allocated with ilg, ...:
      tracerMossCMassgat   => tracer%mossCMassgat,     & !< real, dimension(:) : Tracer mass in moss biomass, 14C: \f$ng ^{14}C/m^2\f$ 
      tracerMossLitrMassgat => tracer%mossLitrMassgat, & !< real, dimension(:) : Tracer mass in moss litter, 14C: \f$ng ^{14}C/m^2\f$ 
      tracerCO2gat          => tracer%tracerCO2gat,    & !< real, dimension(:) : Atmopspheric tracer CO2 concentration (units vary) 
      
      ! allocated with ilg, icc:
      tracerGLeafMassgat   => tracer%gLeafMassgat,     & !< real, dimension(:,:) : Tracer mass in the green leaf pool for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$ 
      tracerBLeafMassgat   => tracer%bLeafMassgat,     & !< real, dimension(:,:) : Tracer mass in the brown leaf pool for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$ 
      tracerStemMassgat    => tracer%stemMassgat,      & !< real, dimension(:,:) : Tracer mass in the stem for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$ 
      tracerRootMassgat    => tracer%rootMassgat,      & !< real, dimension(:,:) : Tracer mass in the roots for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$ 
      tracerRothrlosgat    => tracer%rothrlosgat,      & !< real, dimension(:,:) : tracer root death as crops are harvested, 14C: \f$ng ^{14}C/m^2/d\f$
      tracerStmhrlosgat    => tracer%stmhrlosgat,      & !< real, dimension(:,:) : tracer stem harvest loss for crops, 14C: \f$ng ^{14}C/m^2/d\f$
      tracerFlHrlossgat    => tracer%FlHrLossgat,      & !< real, dimension(:,:) : Tracer fall & harvest loss for bdl dcd plants and crops, respectively, \f$tracer units/m^2/d\f$.
      ...
      SRH => vgat%SRH, &      ! @rsc - please give these labels for clarity
      isi => vgat%isi, &
      bui => vgat%bui, &
      fwi => vgat%fwi, &
      ffmc => vgat%ffmc, &
      ...
      ! and converts them to the 'gat' format (ilg, ...). .

      call ctemg2(canresgat, ch4concgat, sandgat, grclarea, &
                  ...
                  fNnetlandveggat, drgtstrsgat, betadrgtgat, &
                  ...
                  ilmos, jlmos, nml, &
                  canresrow, ch4concrow, SANDROT, grclarearow, &
                  ...
                  fNnetlandvegrow, drgtstrsrow, betadrgtrow, &
      ...
      if (ncount == nday) then
        ...
        call dayEndCTEMPreparation(vgat, ctem_tile, nml, nday)
        ...
        call ctem(fsnowacc_t, sandgat, ILMOS, & ! In
                  ...
                  taaccgat_t, ancgvgac_t, rmlcgvga_t, & ! In
                  zbtwgat, doMethane, & ! In
                  uvaccgat_t, vvaccgat_t, lightng, & ! In
                  elcggat, tbaraccgat_t, &! In
                  spinfast, todfrac, & ! In
                  ...
                  peatlandTypegat,mossPresentgat, anmossac_t, gppmossac_t, &! In
                  ...
                  ROFBacc_t, QFCacc_t, co2concgat, daylgat, dayl_maxgat, zbot, & ! In
                  prsfireareagat, prescribedFire, & ! In
                  dlzwgat, sdepgat, & ! In/Out
                  ...
                  flhrlossgat, flhrloss_nsgat, flhrloss_sgat, tracerFlHrlossgat, pandaysgat, lfstatusgat, grwtheffgat, & ! In/Out
                  ...
                  gavgltmsgat, gavgscmsgat, stmhrlosgat, tracerStmhrlosgat, slaigat, & ! In/Out
                  bmasveggat, cmasvegcgat, colddays_leaffallgat, & ! In/Out
                  colddays_harvestgat, rothrlosgat, tracerRothrlosgat, & ! In/Out
                  fcangat, alvsctmgat, alirctmgat, gavglaigat, &! In/Out
                  Cmossmasgat, litrmsmossgat, upMossSoilCgat, peatdepgat, peatSoilCgat, fcancmxgat, &! In/Out
                  Nmossmasgat, litrmsmossNgat, upMossSoilNgat, rmlmossac_t, & ! In/Out
                  ...
                  bnfnatgat, bnfantgat, nstressgat, bnf_mossgat, & ! In/Out
                  ...
                  cproductgat, fproductdecompgat, nMineralNH4gat, nMineralNO3gat, nVeggat, nLittergat, nSoilgat, nLeafgat, nStemgat, nRootgat, & ! Out (Primary)
                  fBNFgat, fNdepgat, fNfertgat, fNgasNonFiregat, fNgasFiregat, fNgasgat, fNnetmingat, fNOxgat, fNupgat, fNvegSoilgat, fN2ogat, &  ! Out (Primary)
                  cleafgat, cstemgat, crootgat, nppleafgat, nppstemgat, npprootgat, & ! Out (Secondary)
                  ...
                  emit_tpmgat, emit_tcgat, emit_bcgat, emit_ocgat, emit_so2gat, &! Out (Secondary)
                  ...
                  nvolveggat, nleachveggat, fNleachgat, fNvolgat, &! Out
                  ...
                  fNnetlandveggat, redcoeff_vcmaxgat, redcoeff_vcmaxMossgat, faregat, nepCMIPgat, drgtstrsgat, betadrgtgat, & ! Out
                  QACGAT, PRESGAT, isi, bui, fwi, ffmc, dmc, dc, IMONTH, NoonTempc, &
                  NoonPrecip, NoonWindtot, NoonRhum, snwct)


        ! Once a year,calculate the 14C lost to decay if using the 14C tracer.
        if (useTracer == '14C' .and. iday == lastdoy .and. ncount == nday) call decay14C(1, nml, tracer, vgat)

        ! Reset mosaic accumulator arrays.
        call resetMosaicAccum(vgat, ctem_tile, ilg, ignd)
    ...
    call ctems2(fcancmxrow, rmatcrow, zolncrow, paicrow, &
                ...
                cfluxcsrow, canresrow, SDEPROT, ch4concrow, &
                anvegrow, rmlvegrow, prbfrhucrow, &
                extnprobrow, pfcancmxrow, nfcancmxrow, &
                ...
                fNnetlandvegrow, redcoeff_vcmaxrow, drgtstrsrow, betadrgtrow, &
                ...
                fNnetlandveggat, redcoeff_vcmaxgat, drgtstrsgat, betadrgtgat, &
      ...
      call ctems1(ancsvegrow, ancgvegrow, rmlcsvegrow, rmlcgvegrow, &
                  ilmos, jlmos, nml, &
    ...
    call classScatter(TBARROT, THLQROT, THICROT, TSFSROT, TPNDROT, & ! Formerly CLASSS
                      ZPNDROT, TBASROT, ALBSROT, TSNOROT, RHOSROT, &
                       SNOROT, FSNOROT, maxSNOROT, GTROT, TCANROT, & 
                       RCANROT, SCANROT, GROROT, CMAIROT, TACROT, &
                       QACROT, WSNOROT, REFROT, BCSNROW, EMISROT, &
                       SALBROT, CSALROT, groundHeatFluxROT, &
                       ILMOS, JLMOS, NML, NLAT, NTLD, NMOS, &
                       ILG, IGND, ICAN, ICAN + 1, NBS, &
                       TBARGAT, THLQGAT, THICGAT, TSFSGAT, TPNDGAT, &
                       ZPNDGAT, TBASGAT, ALBSGAT, TSNOGAT, RHOSGAT, &
                       SNOGAT, FSNOGAT, maxSNOGAT, GTGAT, TCANGAT, &
                       RCANGAT, SCANGAT, GROGAT, CMAIGAT, TACGAT, &
                       QACGAT, WSNOGAT, REFGAT, BCSNGAT, EMISGAT, &
                       SALBGAT, CSALGAT, groundHeatFlux)
    ...
    do K = 1,NML
      ...
      SFCTDROT(ILMOS(K),JLMOS(K)) = SFCTDGAT(K)
    ...
    if (ncount == nday) call findPermafrostVars(nmtest, nltest, iday, class_rot, class_gat)
