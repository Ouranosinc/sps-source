...
!!
subroutine  balnit (il1, il2, iday, leapnow, radj, appl_fert, appl_fert_g, fcancmx, &
                    ndeposit, bnf_free, bnf_free_g, nh4_mass, pnh4_mass,    &
...
real,dimension(ilg,icc),intent(in) :: fcancmx        !< max. fractional coverage of each PFT
...
real,intent(in),dimension(ilg)       :: nmineralmoss_litr !< nitrogen mineralization from moss litter, \f$g N m^{-2} day^{-1}\f$
real,intent(in),dimension(ilg)       :: nmineralmoss_humus!< nitrogen mineralization from moss SOM, \f$g N m^{-2} day^{-1}\f$
...
! Find fraction of vegetated ground
...
do 100 j = 1, iccp1
   do 110 i = il1, il2
        ...
        ! Account for moss
        if (j /= iccp1) then 
           diff1 = abs(diff1 + (nmineralmoss_litr(i) + nmineralmoss_humus(i)) * fcancmx(i,j))
        else 
           diff1 = abs(diff1 + (nmineralmoss_litr(i) + nmineralmoss_humus(i)) * (1. - vg(i)))
        end if 
        ...
        if(diff1.gt.ntolerance)then
          ...
          write(*,*)'nmineralmoss_litr(',i,') (unscaled by fcancmx)', nmineralmoss_litr(i)
          write(*,*)'nmineralmoss_humus(',i,') (unscaled by fcancmx)', nmineralmoss_humus(i)
...
! Moss pools check for conservation
