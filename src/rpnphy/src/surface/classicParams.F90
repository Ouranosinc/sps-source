!> \file
!! Contains CLASSIC globally accessible parameters, and
!! c_switch (ctem_switches) - Switches for running the model, read from the joboptions file
!! @author J. Melton, E. Chan, G. Meyer
!!
module classicParams
  ...
  real, parameter :: convertn2o2N = 14.0067/(14.0067*2.0+15.999) !convert n2o mass of species to mass N
  real, parameter :: convertnh32N= 14.0067/(14.0067+3*1.00784) !convert nh3 mass of species to mass N
  real, parameter :: convertnox2N=14.0067/(14.0067+15.999*1.5) !convert nox mass of species to mass N (assumes half NO/ half NO2)
  ...
  real :: RGASV = 461.50       !< Gas constant for water vapour (\f$J kg^{-1} K^{-1}\f$) (GCM name: GASV)
  real :: GRAV = 9.80616       !< Acceleration due to gravity (\f$m s^{-1}\f$) (GCM name: G)
  real :: SBC = 5.66796E-8     !< Stefan-Boltzmann constant (\f$W m^{-2} K^{-4} \f$) (GCM name: SIGMA)
  real :: SPHAIR = 1.00464E3   !< Specific heat of air (\f$J kg^{-1} K^{-1}\f$) (GCM name: CPRES)
  real :: PI = 3.1415926535898 !< pi (-) (GCM name: CPI)
  real :: STD_PRESS = 101325.0 !< Standard atmospheric pressure (Pa)
  ...
  real, parameter :: rel_tolerance_amount = 1.5E-03 
  ...
  ! * These are used in routines vrtdftke.F90 in CanAM and DRCOEF2.f90.
  ...
  logical :: switch_lightning = .false.
  logical :: switch_ctem_fire = .false.
  logical :: switch_cffeps_plume = .false.
  ! --------------------------------------------------------------
  ...
  integer :: ignd      !< Number of soil layers, set in the code
  integer :: nbs       !< Number of modelled shortwave radiation wavelength bands
#endif

  ! Provides a way to restrict execution of code to a particular MPI rank (e.g. execute write statements only when rank==0).
  integer :: rank      !< rank (identifier) of MPI process associated with communicator MPI_COMM_WORLD
  ...
  ! Canada simulations use ican = 5 and icc = 15.
...
#endif
  ! Canada simulations use l2max = 4 while the rest of the model configurations use l2max=3.
  ...
  real :: SNOWCAP_ICE  !< Maximum snow thickness over ice sheets  (m)
  real :: SNOWCAP_LAND !< Maximum snow thickness over land (incl. ice sheets) (m)
  ...
  real :: MIN_KSAT_FRAC   !< Parameters for computing the exponential distribution scalar 
  real :: ADD_2_DEPTH     !< for saturated hydraulic conductivity (GRKSAT).
  real :: EXP_DECAY_PARM  !<  
  ...
  real, dimension(kk) :: maxsprd_FWI  !< max. fire spread rate, km/hr used by the FWI fire module
  ...
  real, dimension(kk) :: emif_so2     !< pft-specific emission factors for sulfur dioxide, g species / (kg DOM)
  ...
  ! moss and peatland parameters:
  ! These parameters are used for both upland mosses and peatlands. 

  ! mossPht subroutine parameters
  
  real, dimension(2) :: mossMaxSnow                 !< Maximum snow depth under which moss photosynthesis is allowed to occur (m)
  real, dimension(2) :: mossMinTemp                 !< Minimum ground surface temperature under which moss photosynthesis is allowed to occur (deg C)
  real, dimension(2) :: rmlmoss25                   !< Base dark respiration rate of moss umol CO2/m2/s
  ...
  real, dimension(2) :: alpha_moss    !< Efficiency of conversion of incident photons into electrons(mol electron/mol photon),
  real, dimension(2) :: vcmax_moss    !< Maximum carboxylation rate for moss at 25 deg C (\f$(mol CO_2 m^{-2} s^{-1}\f$)
  real, dimension(2) :: alpha_vcmax_moss   !< Moss coeff. to relate Vcmax to plant N content
  real, dimension(2) :: beta_vcmax_moss    !< Moss coeff. to relate Vcmax to plant N content

  ! decp (peatland) subroutine parameters:
  ...
  ! moss parameters used in other subroutines:

  real, dimension(2) :: zolnmoss                    !< natual logarithm of roughness length of moss surface.
  real, dimension(2) :: thpmoss                     !< pore volume of moss (\f$m^3/m^3\f$)
  real, dimension(2) :: thrmoss                     !< Liquid water retention capacity of moss (\f$m^3/m^3\f$)
  real, dimension(2) :: thmmoss                     !< residual liquid water content after freezing or evaporation of moss (\f$m^3/m^3\f$)
  real, dimension(2) :: bmoss                       !< Clapp and Hornberger empirical "b" parameter of moss
  real, dimension(2) :: psismoss                    !< Soil moisure suction at saturation of moss (m)
  real, dimension(2) :: grksmoss                    !< Saturated hydrualic conductivity of moss (m)
  real, dimension(2) :: hcpmoss                     !< Volumetric heat capacity of moss (\f$J m^{-3} K^{-1}\f$)
  real, dimension(2) :: bsratelt_moss               !< moss-derived litter base respiration rate in kg C/kg C.year
  real, dimension(2) :: bsratesc_moss               !< moss-derived soil C respiration base rate in kg C/kg C.year
  ...
  real, dimension(2) ::   grescoefmoss               !< Moss growth respiration coefficient
  real, dimension(2) ::   rmortmoss                  !< Moss mortality rate constant (year-1)
  real, dimension(2) ::   humicfacmoss               !< Humification ratio of moss litter
  ...
  real, dimension(kk) :: vmax_peat_redu  !< Scales max. photosynthetic rate for peatland conditions, (fraction)
  ...
  real :: C_cost_mossBNF       !< Carbon cost of moss BNF (g C per g N)
  ...
  real, dimension(2) :: c2n_Mossmax  !< max moss C:N ratio
  real, dimension(2) :: c2n_Mossmin  !< min moss C:N ratio
  ...
  real :: mossInitial_c2n             !< Moss arbitrary value for the initial C:N ratios
  real, dimension(2) :: r_bnf_moss    !< Rate of cyanobacteria-derived BNF for moss; \f$g N m^{-2} day{-1}\f$
  ...
  type ctem_switches
    ...
    logical :: NcycleConnect_on !< When set false and Ncycle_on is true, N limitation will not impact Vcmax
    ...
    character(6) :: useTracer        !< Switch for use of a model tracer. If useTracer is 'None' then the tracer code is not used.
    !! useTracer = 'Simple' turns on a simple tracer that tracks pools and fluxes. The simple tracer then requires that the tracer values in
    ...
    integer :: tracertimeAdjust !< Number of years to adjust the time sought in the tracerCO2file

    integer :: actualMetStartYear!< First year read in from the met files
    integer :: actualMetEndYear  !< Last year read in from the met files
    integer :: runStartYear      !< First year of the run, does not need to correspond to the actualMetStartYear
    integer :: runEndYear        !< Last year of the run, does not need to correspond to the actualMetEndYear
    character(10) :: metOrder   !< Options are 'sequential' - this runs through the meteorology in year-order, or 'random' - this 
                                !! takes the years between actualMetStartYear and actualMetEndYear and puts them in random order
                                !! that will change between loops of the meteorology (intended for spinups). It 
                                !! does use a set random_seed, so it will reproduce the same year order each run given the same starting conditions.
    ...
    integer :: co2timeAdjust    !< Number of years to adjust the time sought in the CO2file
    ...
    character(10) :: fireSubmodule !< This selects the fire submodule. The options here are:
                                ...
                                !! runs.
    logical :: iniFWI           !< boolean, if true use the spun up FWI values from the init file
    ...
    character(10) :: prescribedFireTimestep !< time step of the prescribed gridcell fraction burned file

    logical :: doPeatOutputs             !< logical switch that tells the model if any tiles contain peatlands - only used
                                         !! to set up the output files! The actual peatland component is controled by what 
                                         !! is in the initial conditions file.
    ...
    !< corresponding parameter calculated by class is overridden by
    !< a user-supplied input value.
    character(7) :: fracSnowParam !< Switch for snow cover fraction parameterization schemes in the model. 
                                  !! fracSnowParam = 'Default' means using the original default scheme.
                                  !! fracSnowParam = 'SL12' means using the SL12 scheme (developed by Swenson and Lawrence, 2012).
    character(7) :: snoAlbedoParam !< Choose the snow albedo parameterization: '2-band' = the original CLASS two-band snow albedo algorithms are used.
                                   !< '4-band' is the four-band routines from Can-ESM.
    ...
    real    :: specifiedSoilPermDepth !< If set to a positive value, the soil permeable depth read in from 
                                      !! the initialization is overwritten and the SDEP value is set to be the minimum of 
                                      !! of the value in the initfile or the specifiedSoilPermDepth value. So specifiedSoilPermDepth
                                      !! is the maximum possible SDEP. 
    logical :: EcTResistancePart   !< Switch determining whether to use the leaf boundary layer and stomatal resistances to determine the
