...
contains
  ...
  subroutine moveSplitCopyTiles(controlVector,inputVector,FAREAROUT,mode,DynTilInitializeFlag,outputIndex, & !these are the main intputs that control the routine
    ...
    real, intent(inout), dimension(nlat,nmos,icc) :: co2i1cgrow         !< Intercellular CO2 concentration for CTEM PFTs for canopy over ground subarea (Pa) - for single/sunlit leaf
    real, intent(inout), dimension(nlat,nmos,icc) :: co2i1csrow         !< Intercellular CO2 concentration for CTEM PFTs for canopy over snow subarea (Pa) - for single/sunlit leaf
    real, intent(inout), dimension(nlat,nmos,icc) :: co2i2cgrow         !< Intercellular CO2 concentration for CTEM PFTs for canopy over ground subarea (Pa) - for shaded leaf
    real, intent(inout), dimension(nlat,nmos,icc) :: co2i2csrow         !< Intercellular CO2 concentration for CTEM PFTs for canopy over snow subarea (Pa) - for shaded leaf
    ...
    call initialMassEnergyBalance(FAREROT, DLZWROT, fcancmxrow, gleafmasrow, bleafmasrow, stemmassrow, rootmassrow, Cmossmasrow, peatSoilCrow, litrmsmossrow, &
                                ...
                                TSSTP_initial, TSTP_initial, TSPN_initial, TTOT_initial, Ncycle_on, peatlandTyperow, DELZ, THPROT, HCPSROT, CMAIROT, mode)
    ...
    do i=1,nlat
        if (mode(i) /= "skip") then
            ...
            peatlandTyperow(i,outputIndex(i)) = peatlandTyperow(i,headTile(i))
    ...
    if (Ncycle_on) then
        ...
        nh4_massrow = weightedAvg3d(nh4_massrow,iccp1,nlat,nmos,outputIndex,headTile,inputLength,inputIndex,FAREROT,mode) 
        no3_massrow = weightedAvg3d(no3_massrow,iccp1,nlat,nmos,outputIndex,headTile,inputLength,inputIndex,FAREROT,mode) 
...
!change the fractional areas of the input and output tiles
  ...
  subroutine cleanTiles(controlVector,tneeded,DynTilInitializeFlag,outputIndex, & !these are the main intputs that control the routine
    ...
    do i =1,nlat
        ...
        if(nTilepres > 0) then
            ...
            logical_diff(i,:,int(subsetHeights(i,:))) = .FALSE.
            logical_diff(i,int(subsetHeights(i,:)),:) = .FALSE.
    ...
    wloop5: do while((ANY(cmin < percentthreshold) .or. ANY(cjoined < tneeded)) .and. &
        ...
        call updateHeight(peatlandTyperow, veghghtrow, stemmassrow, gleafmasrow, bleafmasrow)
  ...
  subroutine initialMassEnergyBalance(FAREROT, DLZWROT, fcancmxrow, gleafmasrow, bleafmasrow, stemmassrow, rootmassrow, Cmossmasrow, peatSoilCrow, litrmsmossrow, &
    ...
    logical, intent(in) :: Ncycle_on
    character(8), intent(in), dimension(nlat,nmos) :: peatlandTyperow
    ...
    do i=1,nlat
        if (mode(i) /= "skip") then
            do j=1,nmos
                ...
                if (peatlandTyperow(i,j) /= 'None') then
  ...
  !> @author S.R. Curasi
  subroutine updateHeight(peatlandTyperow, veghghtrow, stemmassrow, gleafmasrow, bleafmasrow)
            ...
            do k = 1,ican
                do l = reindexPFTs(k,1),reindexPFTs(k,2) ! loop 260
                    ...
                    case ('NdlTr','BdlTr') ! Trees
                        if (peatlandTyperow(i,j) == 'None') then ! Uplands
