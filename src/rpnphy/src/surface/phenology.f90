...
subroutine phenology (il1, il2, ilg, leapnow, tbar, thice, & ! In
                     ...
                     flhrloss, flhrloss_ns, flhrloss_s,tracerFlHrLoss,leaflitr, leaflitr_ns, & ! Out
  ...
  real, intent(inout) :: tracerStemMass(ilg,icc)!< tracer stem mass for each of the ctem pfts, 14C: \f$ng ^{14}C/m^2\f$
  ...
  real, intent(inout) :: tracerRootMass(ilg,icc) !< tracer root mass for each of the ctem pfts, 14C: \f$ng ^{14}C/m^2\f$
  real, intent(in) :: fcancmx(ilg,icc)       !< max. fractional coverage of ctem's pfts, but this can be modified
  ...
  integer, intent(inout) :: colddays_harvest(ilg)     !< cold days counter for tracking days below a certain temperature threshold
                                                      !< for crop pfts.
  real, intent(inout) :: flhrloss(ilg,icc)      !< fall & harvest loss for bdl dcd plants and crops, respectively, \f$kg c/m^2/d\f$.
  real, intent(inout) :: flhrloss_ns(ilg,icc)   !< non-structural fall & harvest loss for bdl dcd plants and crops, respectively, \f$kg c/m^2/d\f$.
  real, intent(inout) :: flhrloss_s(ilg,icc)    !< structural fall & harvest loss for bdl dcd plants and crops, respectively, \f$kg c/m^2/d\f$.
  real, intent(inout) :: tracerFlHrLoss(ilg,icc)!< Tracer fall & harvest loss for bdl dcd plants and crops, respectively, 14C: \f$ng ^{14}C/m^2/d\f$
  ...
  real, intent(out) :: tracerLeafLitr(ilg,icc)  !< leaf litter generated by normal turnover, cold and drought stress,
                                                !< and leaf fall/harvest,14C: \f$ng ^{14}C/m^2\f$
  real, intent(out) :: re_alloc_sr2l(ilg,icc) !< amount of nsc reallocated from stem and root to leaves during leaf out, \f$Mg c/m^2\f$ (Assumed -JM)
  real, intent(out) :: re_alloc_s2l(ilg,icc)  !< amount of nsc reallocated from stem to leaves during leaf out, \f$Mg c/m^2\f$ (Assumed -JM)
  real, intent(out) :: re_alloc_r2l(ilg,icc)  !< amount of nsc reallocated from root to leaves during leaf out, \f$Mg c/m^2\f$ (Assumed -JM)
  ...
  real :: rootnstem              !< temp addition of nonstructural (stem+root)
  real :: tracerroottoleaf       !< root tracer that is transferred to leaves, 14C: ng 14C/m2
  real :: tracerstemtoleaf       !< stem tracer that is transferred to leaves, 14C: ng 14C/m2
  real :: oldstem                !< temp variables to store the total C pool values for the tracer calculations. (kg C/m2)
  real :: oldroot                !< temp variables to store the total C pool values for the tracer calculations. (kg C/m2)
  ...
  do j = 1,icc
    n = sort(j)
    do i = il1,il2
      sla(j) = calcSLA(peatlandType(i),sort(j))  !FLAG - why do this every time step when it is based upon parameters that are set at run start? JM 2022
  ...
  do i =  il1,il2
    do j = 1,icc
      ...
      if (fcancmx(i,j) > 0.0) then
        ...
        case ('NdlEvgTr','CNEvgTr','INEvgTr') ! Needleleaf Evergreen Tree ==========================
          ...
          if (lfstatus(i,j) == 1) then ! max growth
            ...
            if (ailcg(i,j) <= zero) then ! no leaves
              ...
              if ((ailcg(i,j) > zero .and. colddays_leaffall(i) >= coldlmt(1)) & 
                ...
                if (useTracer /= 'None') tracerFlHrLoss(i,j) = tracerGLeafMass(i,j) * (1.0 / flhrspan(2))
        ...
        case ('CropC3  ') ! =================================================================
          ...
          if (lfstatus(i,j) == 1) then ! max growth
            ...
            if (ailcg(i,j) >= harvthrs(n)) then
              ...
              if (useTracer /= 'None') tracerFlHrLoss(i,j) = tracerGLeafMass(i,j) * (1.0 / flhrspan(1))
      ...
      if (fcancmx(i,j) > 0.0) then
        if (lfstatus(i,j) == 1 .or. lfstatus(i,j) == 2 .or. lfstatus(i,j) == 4) then
          ...
          flhrloss_ns(i,j) = 0.
          flhrloss_s(i,j) = 0.
          if (useTracer /= 'None') tracerFlHrLoss(i,j) = 0.
        end if 
      end if 
      ...
      !! during early spring (max growth mode) in order to push out leaves faster following
      ...
      if (reallocateWoody(i,j)) then          
        if (stemmass(i,j) > 0.0 .and. rootmass(i,j) > 0.0) then
          ...
          rootnstem = stemmass_ns(i,j) + rootmass_ns(i,j)
          if (stemns2t < 1.0 .and. rootns2t < 1.0 .and. rootnstem > 0.0) then
            ns_s2sr = stemmass_ns(i,j) / rootnstem
            ns_r2sr = rootmass_ns(i,j) / rootnstem
      ...
      if (reallocateHerb(i,j) .or. reallocateWoody(i,j)) then
        if (useTracer /= 'None') then
          ! save the original totals
          oldstem = stemmass(i,j)
          oldroot = rootmass(i,j)
        end if 
        ...
        if (useTracer /= 'None') then 
          ...
          ! We don't track the NS and S components separately for the tracer. So we will take the 
          ! tracer amount (ng 14C/m2) in the root and stem tissues and make it into a ratio
          ! by dividing by the total C (kg C/m2) so we end up with 1E12 14C/C and apply that
          ! to the transfer to the leaves which gives us the amount in ng 14C/m2.

          ! Calc how much tracer is moved from the roots to the leaves
          if (rootmass(i,j) > 0.) then 
            tracerroottoleaf = (1. - rootmass(i,j) / oldroot) * tracerRootMass(i,j)
          else 
            tracerroottoleaf = 0.
          end if 
          tracerRootMass(i,j) = tracerRootMass(i,j) - tracerroottoleaf
          ! Now for the stems:
          if (stemmass(i,j) > 0.) then 
            tracerstemtoleaf = (1. - stemmass(i,j) / oldstem) * tracerStemMass(i,j)
          else 
            tracerstemtoleaf = 0.
          end if 
          tracerStemMass(i,j) = tracerStemMass(i,j) - tracerstemtoleaf

          ! Then add that to the gleaves.
          tracerGLeafMass(i,j) = tracerGLeafMass(i,j) + tracerroottoleaf + tracerstemtoleaf

          ! Reverse the tranfers if anything goes negative.
          if (tracerGLeafMass(i,j) < 0. .or. tracerStemMass(i,j)  < 0. .or. tracerRootMass(i,j)  < 0.) then
            tracerGLeafMass(i,j) = tracerGLeafMass(i,j) - tracerroottoleaf - tracerstemtoleaf
            tracerStemMass(i,j) = tracerStemMass(i,j) + tracerstemtoleaf
            tracerRootMass(i,j) = tracerRootMass(i,j) + tracerroottoleaf
          end if 
  ...
  do j = 1,icc
    ...
    do i = il1,il2
      if (fcancmx(i,j) > 0.0) then
        if (peatlandType(i) == 'None') then ! Uplands
          term = 1.0 - exp( -1.0 / (lastDOY * lfespany(n)))
          nrmlloss(i,j) = gleafmas(i,j) * term
          nrmlloss_ns(i,j) = gleafmas_ns(i,j) * term
          nrmlloss_s(i,j)  = gleafmas_s(i,j)  * term
          if (useTracer /= 'None') tracerNrmlLoss(i,j) = tracerGLeafMass(i,j) * term
        else ! peatlands 
          term = 1.0 - exp( -1.0 / (lastDOY * lfespany_peat(n)))
          nrmlloss(i,j) = gleafmas(i,j) * term
          nrmlloss_ns(i,j) = gleafmas_ns(i,j) * term
          nrmlloss_s(i,j)  = gleafmas_s(i,j)  * term
          if (useTracer /= 'None') tracerNrmlLoss(i,j) = tracerGLeafMass(i,j) * term
  ...
  do j = 1,icc
    ...
    do i = il1,il2
      if (fcancmx(i,j) > 0.0) then
        ...
        term = 1.0 - exp( - drgtlsrt(i,j))
        drgtloss(i,j) = gleafmas(i,j) * term
        drgtloss_ns(i,j) = gleafmas_ns(i,j) * term
        drgtloss_s(i,j)  = gleafmas_s(i,j)  * term
        if (useTracer /= 'None') tracerDrgtLoss(i,j) = tracerGLeafMass(i,j) * term
        ...
        case ('NdlEvgTr','CNEvgTr','INEvgTr','NdlDcdTr','BdlEvgTr','BdlDDrTr','CropC3  ','CropC4  ','GrassC3 ','GrassC4 ',&
              'BdlEvgSh','Sedge   ')
        ...
        term = 1.0 - exp( - coldlsrt(i,j))
        coldloss(i,j) = gleafmas(i,j) * term
        coldloss_ns(i,j) = gleafmas_ns(i,j) * term
        coldloss_s(i,j)  = gleafmas_s(i,j)  * term
        if (useTracer /= 'None') tracerColdLoss(i,j) = tracerGLeafMass(i,j) * term
    ...
    case ('GrassC3 ','GrassC4 ','Sedge   ')
      ...
      do i = il1,il2
        if (fcancmx(i,j) > 0.0) then
          ...
          if (useTracer /= 'None') & ! We only track total for the tracer so use those values calculated earlier.
              tracerGLeafMass(i,j) = tracerGLeafMass(i,j) - tracerNrmlLoss(i,j) - tracerDrgtLoss(i,j) - tracerColdLoss(i,j)
          ...
          if (useTracer /= 'None') then
            if (gleafmas(i,j) < zero .or. tracerGLeafMass(i,j) < 0.) then
          ...
          if (peatlandType(i) == 'None') then ! Uplands
            term = 1.0 - exp( -1.0 / (0.10 * lastDOY * lfespany(n)))
            nrmlloss(i,j) = bleafmas(i,j) * term
            if (useTracer /= 'None') tracerNrmlLoss(i,j) = tracerBLeafMass(i,j) * term
          else ! peatlands 
            term = 1.0 - exp( -1.0 / (0.10 * lastDOY * lfespany_peat(n)))
            nrmlloss(i,j) = bleafmas(i,j) * term
            if (useTracer /= 'None') tracerNrmlLoss(i,j) = tracerBLeafMass(i,j) * term
          end if 
  ...
  do j = 1,icc
    do i = il1,il2
      if (fcancmx(i,j) > 0.0) then
        ...
        if (useTracer /= 'None') tracerLeafLitr(i,j) = tracerNrmlLoss(i,j) &
