...
subroutine waterFlowInfiltrate (IVEG, THLIQ, THICE, TBARW, BASFLW, TBASFL, & ! Formerly GRINFL
  ...
  real, intent(inout) :: QFG   (ILG)  !< Evaporation from soil surface (diagnostic) \f$[kg m^{-2} s^{-1}]\f$
  ...
  real, intent(inout) :: ZPOND (ILG)  !< Depth of ponded water on soil surface [m]
  real, intent(in) :: DT    (ILG)  !< Time period over which water movement takes place [s]

  real :: GRKSATF(ILG,IG)          !< Effective saturated hydraulic conductivity \f$[m s^{-1}]\f$
  real :: THPORF(ILG,IG)           !< Modified pore volume after accounting for ice \f$[m^3 m^{-3}]\f$
  !
  !     * WORK FIELDS (FOR ALL CALLED ROUTINES AS WELL).
  !
  real, intent(inout) :: ZMAT (ILG,IGP2,IGP1) !< depth of each soil layer J (including the dummy soil layer below the lowest layer) that is
  ...
  real, intent(inout) :: TMOVE(ILG,IGP2)      !< Temperature of water in WMOVE [C]
  real, intent(inout) :: THLIQX(ILG,IGP1)     !< Volumetric liquid water content of soil layer \f$[m^3 m^{-3}]\f$
  real, intent(inout) :: THICEX(ILG,IGP1)     !< Volumetric ice content of soil layer \f$[m^3 m^{-3}]\f$
  real, intent(inout) :: TBARWX(ILG,IGP1)     !< Temperature of water in soil layer [C]
  real, intent(inout) :: DELZX (ILG,IGP1)     !< Permeable depth of soil layer \f$[m] (\Delta z_{g,w})\f$
  real, intent(inout) :: ZBOTX (ILG,IGP1)     !< Depth of bottom of soil layer [m]     
  real, intent(inout) :: PSIF  (ILG,IGP1)     !< Soil water suction across the wetting front [m]
  real, intent(inout) :: GRKINF(ILG,IGP1)     !< Hydraulic conductivity of soil behind the wetting front \f$[m s^{-1}]\f$
  real, intent(inout) :: THLINF(ILG,IGP1)     !< Volumetric liquid water content behind the wetting front \f$[m^3 m^{-3}]\f$                    
  real, intent(inout) :: THLMAX(ILG,IG),   THTEST(ILG,IG), &
  ...
  real, intent(inout) :: FDT   (ILG,IGP1)  !< Water flow at soil layer interfaces during current time step [m]
  ...
  real, intent(inout) :: FMAX  (ILG), &
                         DTFLOW(ILG), THLNLZ(ILG), THLQLZ(ILG), &
                         DZDISP(ILG), WDISP (ILG), WABS  (ILG)
  real, intent(inout) :: DZF   (ILG), TIMPND(ILG), TADD  (ILG), TUSED (ILG), &
                         WADD  (ILG), WEXCES(ILG) 
  real, intent(out)   :: WADJ  (ILG)
  !
  !     * SOIL INFORMATION ARRAYS.
  !
  real, intent(in) :: THPOR (ILG,IG)   !< Pore volume in soil layer \f$[m^3 m^{-3}] (\theta_p)\f$
  real, intent(in) :: THLRET(ILG,IG)   !< Liquid water retention capacity for organic soil \f$[m^3 m^{-3}] (\theta_{l,ret})\f$
  real, intent(in) :: THLMIN(ILG,IG)   !< Residual soil liquid water content remaining after freezing or evaporation
  ...
  real, intent(in) :: GRKSAT(ILG,IG)   !< Hydraulic conductivity of soil at saturation \f$[m s^{-1}] (K_{sat})\f$
  real, intent(in) :: THLRAT(ILG,IG)   !< Fractional saturation of soil behind the wetting front \f$[ ] (f_{inf})\f$
  ...
  integer, intent(inout) :: IGRD (ILG)    !< Flag to indicate whether calculations in subroutine waterFlowNonInfiltrate are to be done
  integer, intent(in)    :: IZERO(ILG)
  integer, intent(out)   :: IFIND(ILG)    !< Flag indicating the depth of the wetting front has been found
  integer, intent(inout) :: ITER (ILG)    !< Flag to indicate if the saturated infiltration calculations are occurring for that tile. 0 if not.
  integer, intent(inout) :: NEND (ILG)    !< Maximum number of iterations in waterInfiltrateSat
  integer, intent(inout) :: ISIMP (ILG)   !< Flag for whether water movement occurred (values < 0), used by waterInfiltrateSat

  integer, intent(inout) :: IFILL (ILG)   !< Flag indicating whether unsaturated infiltration is occurring
  integer, intent(inout) :: NINF(ILG)     !< Number of levels involved in water movement
  integer, intent(inout) :: LZF(ILG)      !< index of the soil layer in which the wetting front occurs
  ...
  do I = IL1,IL2 ! loop 75
    if (FI(I) > 0. .and. &                    ! if fractional coverage of subareas is not zero,
        ISAND(I,1) > - 4 .and. &              ! tile is not ice,
        DT(I) > 0. .and. &                    ! time period of water movement is not zero,
        (R(I) > 0. .or. ZPOND(I) > 0.)) then  ! there is rain and no ponded water on surface then
      IGRN(I) = 1                             ! perform calculations on this sub area
      RDUMMY(I) = 0.
    else
      IGRN(I) = 0 ! don't perform calculations for this subarea
    end if
  end do ! loop 75
  !
  ...
  call waterInfiltrateSat(WMOVE, TMOVE, LZF, NINF, TRMDR, TPOND, ZPOND, & ! Formerly WFLOW
                          ...
                          IGRN, IGP1, IGP2, ILG, IL1, IL2, JL, N)
  !
  ...
  call waterFlowNonInfiltrate(IVEG, THLIQ, THICE, TBARW, FDUMMY, TDUMMY, BASFLW, & ! Formerly GRDRAN
                              ...
                              IZERO, IGRD, IGDR, IG, IGP1, ILG, IL1, IL2, JL, N)
  !
  return
