...
contains
  ...
  !! this subroutine, if the snoAlbedoParam switch is set to '2-band', the visible and
  ...
  !!
  !! If the snoAlbedoParam switch is set to '2-band', the value of ALSNO in the first
  ...
  !! If the snoAlbedoParam switch is set to '4-band', a new parameterization for the snow albedo and
  ...
  subroutine snowAlbedoTransmiss (ALVSSN, ALIRSN, ALVSSC, ALIRSC, ALBSNO, & ! Formerly SNOWALBA
    ...
    !     * JUN 22/13 - J.COLE.     ADD CODE FOR "snoAlbedoParam" OPTION,
    ...
    real, intent(in)    :: BCSN  (ILG)  !< Black carbon mixing ratio in snow \f$[kg m^{-2} s^{-1}]\f$
  ...
  6100 format('0AT (I,J) = (',I3,',',I3,'),ALVSSN,ALIRSN = ',2F10.5)
    ...
    if (snoAlbedoParam == '2-band') then
      ...
      call errorHandler('snowAlbedoTransmiss', 0) 
