...
subroutine waterBudgetDriver (THLIQ, THICE, TBAR, TCAN, RCAN, SNCAN, & ! Formerly CLASSW
                              ...
                              DELZW, ZBOTW, ISAND, IGDR, DLON, DLAT, &
                              IWF, ILG, IL1, IL2, N, &
                              JL, IG, IGP1, IGP2, &
                              NLANDCS, NLANDGS, NLANDC, NLANDG, NLANDI, &
                              RB, RC, RCS, FRAINC, FSNOWC, FRAICS, FSNOCS, & 
                              LAIPAIRatio, LAISPAISRatio, FSNOW, maxSNO, &
                              SNOLIM, stddevTopography, fracSnowParam, TCSNOW) 
  !  
  !     * Dec 05/25 - L. WANG.    Add Calculation of TCSNOW through function 
  !                               snowThermalProp (in snowProcesses).
  ...
  !
  use classicParams, only : DELT, TFREZ, HCPW, HCPSND, SPHW, SPHICE, RHOW, SNOWCAP_LAND
  use snowProcesses, only : snowAging, snowThermalProp, fsnowSL12
  ...
  !     * INTEGER CONSTANTS.
  !
  character(7), intent(in) :: fracSnowParam  !< Switch to model snow cover fraction using the default or SL12 scheme
  integer, intent(in) :: IWF   !< Flag governing lateral soil water flow calculations
  integer, intent(in) :: ILG, IL1, IL2, JL, N
  ...
  real, intent(inout) :: TCSNOW (ILG)  !< Thermal conductivity of Snowpack 
  ...
  real, intent(inout) :: FSNOW(ILG)  !< Simulated fractional snow coverage [ ]
  real, intent(inout) :: maxSNO(ILG) !< Effective maximum snow mass used in recession curve \f$[kg m^{-2}]\f$
  real, intent(in) :: stddevTopography(ILG)  !< Standard deviation of elevation in grid-cell [m]
  real, intent(in) :: SNOLIM(ILG)  !< Limiting snow depth below which coverage is < 100% \f$[m] (z_{s,lim})\f$
  !
  !     * DIAGNOSTIC ARRAYS.
  !
  real, intent(out)   :: PCFC(ILG)  !< Frozen precipitation intercepted by vegetation \f$[kg m^{-2} s^{-1}]\f$
  real, intent(out)   :: PCLC(ILG)  !< Liquid precipitation intercepted by vegetation \f$[kg m^{-2} s^{-1}]\f$
  real, intent(out)   :: PCPN(ILG)  !< Precipitation incident on snow pack \f$[kg m^{-2} s^{-1}]\f$
  real, intent(out)   :: PCPG(ILG)  !< Precipitation incident on ground \f$[kg m^{-2} s^{-1}]\f$
  ...
  real, intent(out)   :: QFN (ILG)  !< Sublimation from snow pack \f$[kg m^{-2} s^{-1}]\f$
  real, intent(out)   :: QFG (ILG)  !< Evaporation from ground \f$[kg m^{-2} s^{-1}]\f$
  real, intent(inout) :: HMFC(ILG)  !< Diagnosed energy associated with phase change of water on vegetation \f$[W m^{-2}]\f$
  real, intent(inout) :: HMFN(ILG)  !< Diagnosed energy associated with phase change of water in snow pack \f$[W m^{-2}]\f$
  real, intent(inout) :: HTCC(ILG)  !< Diagnosed internal energy change of vegetation canopy due to 
                                    !! conduction and/or change in mass \f$[W m^{-2}]\f$
  real, intent(inout) :: HTCS(ILG)  !< Diagnosed internal energy change of snow pack due to conduction 
  ...
  real, intent(inout) :: ROVG(ILG)  !< Liquid/frozen water runoff from vegetation to ground surface \f$[kg m^{-2} s^{-1}]\f$
  real, intent(inout) :: WTRS(ILG)  !< Diagnosed residual water transferred into or out of the snow pack \f$[kg m^{-2} s^{-1}]\f$
  real, intent(inout) :: WTRG(ILG)  !< Diagnosed residual water transferred into or out of the soil \f$[kg m^{-2} s^{-1}]\f$
  real, intent(inout) :: OVRFLW(ILG) !< Overland flow from top of soil column \f$[m or kg m^{-2} s^{-1}]\f$
  real, intent(inout) :: SUBFLW(ILG) !< Interflow from sides of soil column \f$[kg m^{-2} s^{-1}]\f$
  real, intent(inout) :: BASFLW(ILG) !< Base flow from bottom of soil column \f$[m or kg m^{-2} s^{-1}]\f$
  ...
  real, intent(out) :: HMFG (ILG,IG) !< Diagnosed energy associated with phase change of water in soil layers \f$[W m^{-2}]\f$
  ...
  !
  real(r8), intent(inout) :: TBARC(ILG,IG)  !< Subarea temperatures of soil layers [C]
  real(r8), intent(inout) :: TBARG(ILG,IG)  !< Subarea temperatures of soil layers [C]
  real(r8), intent(inout) :: TBARCS(ILG,IG) !< Subarea temperatures of soil layers [C]
  real(r8), intent(inout) :: TBARGS(ILG,IG) !< Subarea temperatures of soil layers [C]
  !
  ...
  !
  real, intent(inout) :: TPONDC(ILG) !< Subarea temperature of surface ponded water [C]
  real, intent(inout) :: TPONDG(ILG) !< Subarea temperature of surface ponded water [C]
  real, intent(inout) :: TPNDCS(ILG) !< Subarea temperature of surface ponded water [C]
  real, intent(inout) :: TPNDGS(ILG) !< Subarea temperature of surface ponded water [C]
  !
  real, intent(inout) :: EVAPC (ILG)  !< Evaporation from vegetation over ground \f$[m s^{-1}]\f$
  real, intent(inout) :: EVAPCG(ILG)  !< Evaporation from ground under vegetation \f$[m s^{-1}]\f$
  ...
  real, intent(inout) :: EVPCSG(ILG)  !< Evaporation from snow under vegetation \f$[m s^{-1}]\f$
  real, intent(inout) :: EVAPGS(ILG)  !< Evaporation from snow on bare ground \f$[m s^{-1}]\f$
  real, intent(inout) :: QFREZC(ILG)  !< Heat sink to be used for freezing water on ground under canopy \f$[W m^{-2}]\f$
  real, intent(inout) :: QFREZG(ILG)  !< Heat sink to be used for freezing water on bare ground \f$[W m^{-2}]\f$
  real, intent(inout) :: QMELTC(ILG)  !< Heat to be used for melting snow under canopy \f$[W m^{-2}]\f$
  real, intent(inout) :: QMELTG(ILG)  !< Heat to be used for melting snow on bare ground \f$[W m^{-2}]\f$
  real, intent(inout) :: RAICAN(ILG)  !< Intercepted liquid water stored on canopy over ground \f$[kg m^{-2}]\f$
  real, intent(inout) :: SNOCAN(ILG)  !< Intercepted frozen water stored on canopy over ground \f$[kg m^{-2}]\f$
  real, intent(inout) :: RAICNS(ILG)  !< Intercepted liquid water stored on canopy over snow \f$[kg m^{-2}]\f$
  real, intent(inout) :: SNOCNS(ILG)  !< Intercepted frozen water stored on canopy over snow \f$[kg m^{-2}]\f$
  ...
  real, intent(inout) :: TCANO (ILG)  !< Temperature of canopy over ground [K]
  real, intent(inout) :: TCANS (ILG)  !< Temperature of canopy over snow [K]
  real, intent(inout) :: CHCAP (ILG)  !< Heat capacity of canopy over bare ground \f$[J m^{-2} K^{-1}] \f$
  real, intent(inout) :: CHCAPS(ILG)  !< Heat capacity of canopy over snow \f$[J m^{-2} K^{-1}] \f$
  ...
  real, intent(inout) :: GZEROC(ILG)  !< Subarea heat flux at soil surface \f$[W m^{-2}]\f$
  real, intent(inout) :: GZEROG(ILG)  !< Subarea heat flux at soil surface \f$[W m^{-2}]\f$
  real, intent(inout) :: GZROCS(ILG)  !< Subarea heat flux at soil surface \f$[W m^{-2}]\f$
  real, intent(inout) :: GZROGS(ILG)  !< Subarea heat flux at soil surface \f$[W m^{-2}]\f$
  !
  ...
  !
  real, intent(inout) :: TSNOCS(ILG)  !< Temperature of snow pack under vegetation [K]
  real, intent(inout) :: TSNOGS(ILG)  !< Temperature of snow pack in bare areas [K]
  real, intent(inout) :: WSNOCS(ILG)  !< Liquid water content of snow pack under vegetation \f$[kg m^{-2}]\f$
  real, intent(inout) :: WSNOGS(ILG)  !< Liquid water content of snow pack in bare areas \f$[kg m^{-2}]\f$
  ...
  !
  real, intent(in) :: DLON(ILG)        !< Grid longitude [degrees]
  real, intent(in) :: DLAT(ILG)         !< Grid latitude [degrees]
  !
  ...
  !
  real :: ZMAT(ILG,IGP2,IGP1)   !< depth of each soil layer J (including the dummy soil layer below the lowest layer) that is
                                !! filled by water from level K in the water movement matrix WMOVE [m]
  real :: WMOVE(ILG,IGP2)       !< Water movement matrix \f$[m^3 m^{-2}]\f$
  real :: TMOVE (ILG,IGP2)      !< Temperature of water in WMOVE [C]
  !
  real :: THLIQX(ILG,IGP1)     !< Volumetric liquid water content of soil layer \f$[m^3 m^{-3}]\f$
  real :: THICEX(ILG,IGP1)     !< Volumetric ice content of soil layer \f$[m^3 m^{-3}]\f$
  ...
  real :: FDT   (ILG,IGP1)     !< Water flow at soil layer interfaces during current time step [m]
  ...
  real :: ZRMDR (ILG,IGP1)      !< depth that has not been affected by infiltration for each soil layer [m]
  real :: FDUMMY(ILG,IGP1), TDUMMY(ILG,IGP1)
  !
  ...
  real :: FMAX  (ILG), TUSED (ILG), &
          RDUMMY(ILG), WEXCES(ILG), FDTBND(ILG)
  real :: WADD  (ILG)       !< Water movement accounting variable \f$[m^3 m^{-2}]\f$
  real :: TADD  (ILG), WADJ  (ILG)
  real :: TIMPND(ILG)       !< Time to pond [s]
  real :: DZF   (ILG), &
  ...
  integer :: IGRD (ILG)    !< Flag to indicate whether calculations in subroutine waterFlowNonInfiltrate are to be done 
  ...
  integer :: LZF(ILG)      !< index of the soil layer in which the wetting front occurs        
  integer :: IZERO (ILG)
  integer :: IFIND (ILG)  !< Flag indicating the depth of the wetting front has been found 
  integer :: ITER  (ILG)  !< Flag to indicate if the saturated infiltration calculations are occurring for that tile. 0 if not.
  integer :: NEND  (ILG)  !< Maximum number of iterations in waterInfiltrateSat
  integer :: ISIMP (ILG)  !< Flag for whether water movement occurred (values < 0), used by waterInfiltrateSat
  integer :: ICONT (ILG)
  !
  ...
  if (NLANDCS > 0) then
    ...
    call waterFlowInfiltrate(1, THLQCS, THICCS, TBRWCS, BASFLW, TBASFL, RUNFCS, & ! Formerly GRINFL
                             ...
                             ZERO, WEXCES, WADD, TADD, WADJ, TIMPND, &
    ...
    call checkWaterBudget(1, PCPR, EVPICS, RUNFCS, WLSTCS, RAICNS, SNOCNS, & ! Formerly CHKWAT
                          ...
                          DELZW, ISAND, DLON, DLAT, IG, ILG, IL1, IL2, JL, N)
  ...
  do I = IL1,IL2
    ...
    if ((FC(I) + FCS(I)) > 0.) then
      ...
      ! Check for wild canopy values
      if (TCAN(I) < 173.16 .or. TCAN(I) > 373.16) then
        write(6,6625) I,JL,TCAN(I)
  ...
  do I = IL1,IL2
    if (ZSNOCS(I) > 0. .or. ZSNOGS(I) > 0. .or. &
      ...
      TCSNOW(I) = snowThermalProp(RHOSNO(I))

      SNOPRE(I) = MAX(0., SNO(I))
      FSNOPRE(I) = MAX(0., FSNOW(I))
      SNO(I) = ZSNOW(I) * RHOSNO(I)
      if (SNO(I) < 0.0) SNO(I) = 0.0
      !
      !           * LIMIT SNOW MASS TO A MAXIMUM OF SNOWCAP_LAND (currently 10 m) TO 
      !           * AVOID SNOW PILING UP AT EDGES OF GLACIERS AT HIGH ELEVATIONS.
      !           * THIS IS TARGETED AS OVERLAND RUNOFF.
      !
      if (ZSNOW(I) > SNOWCAP_LAND) then
        SNOROF = (ZSNOW(I) - SNOWCAP_LAND) * RHOSNO(I)
    ...
    !
    ! Check for impossible snow temperatures
    if (TSNOW(I) < 0.0) then 
      write(6,6626) I,JL,TSNOW(I)
      6626 format('0AT (I,J) = (',I3,',',I3,'),TSNOW = ',F10.5)
      call errorHandler('waterBudgetDriver2', - 3)
    end if 
    if ((TSNOW(I) - TFREZ) > 1.0E-3) then
      write(6,6626) I,JL,TSNOW(I)
  ...
  do J = 1,IG ! loop 700
    do I = IL1,IL2
      ...
      !              THLIQ(I,J)=THFC(I,J)
      !          END IF
      if ( (J == 1) .and. (TBAR(I,1) < 173.16 .or. TBAR(I,1) > 373.16) ) then
        write(6,*) 'Longitude,Latitude:', DLON(I), DLAT(I)
        write(6,6600) I,JL,TBAR(I,1)
  ...
  !! growth index, and fsnowSL12 is called to update FSNOW !!
  call classGrowthIndex(GROWTH, TBAR, TA, FC, FCS, ILG, IG, IL1, IL2, JL) ! Formerly CGROW
  !
  if (fracSnowParam == 'SL12') then
     ! when the SL12 scheme is active call it here, because it requires SNO/FSNOW from 
     ! the previous time step, which are assigned to SNOPRE/FSNOPRE before they are updated above
     call fsnowSL12(SNO, FSNOW,  RHOSNO, SNOLIM, maxSNO, SNOPRE, FSNOPRE, stddevTopography, ISAND, ILG, IG, IL1, IL2)
  end if   
  !
  return
