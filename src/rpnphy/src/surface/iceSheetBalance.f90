...
subroutine iceSheetBalance (TBAR, TPOND, ZPOND, TSNOW, RHOSNO, ZSNOW, HCPSNO, & ! Formerly ICEBAL
  ...
  !     * WORK FIELDS.
  !
  real, intent(inout) :: ZMAT  (ILG,IGP2,IGP1)    !< represents the depth of each ice layer J that is occupied by ice from level K
                                                  !! in the layer movement matrix WMOVE after the layer adjustments are complete [m]
  real, intent(inout) :: TMOVE (ILG,IGP2)         !< Temperature of ice in WMOVE [C]
  real, intent(inout) :: WMOVE (ILG,IGP2)         !< Ice movement matrix \f$[m^3 m^{-2}]\f$
  real, intent(inout) :: ZRMDR (ILG,IGP1)         !< available depth matrix ZRMDR for ice layer thicknesses [m]
  ...
  !!
  !
  !     * LIMIT SNOW PACK TO SNOWCAP_ICE (10 m) .IF SNOW DENSITY EXCEEDS
  ...
  do I = IL1,IL2
    if (FI(I) > 0. .and. ISAND(I,1) == - 4) then
      ...
      HTCS(I) = HTCS(I) - FI(I) * (TSNOW(I) + TFREZ) * HCPSNO(I) * &
                ZSNOW(I) / DELT
      if ((ZSNOW(I)) > SNOWCAP_ICE) then
        SNOCONV = (ZSNOW(I) - SNOWCAP_ICE) * RHOSNO(I)
