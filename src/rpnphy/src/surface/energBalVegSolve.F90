...
subroutine energBalVegSolve (ISNOW, FI, & ! Formerly TSOLVC
                             ...
                             RA, RB, RAGINV, RBINV, TVRTAC, TPOTG, RESID, &
                             ...
                             GLEAFMAS, NGLEAFMAS, Ncycle_on, VCMAX0, NcycleConnect_on, &
                             CEVAPC, RSOILC, AVE_SM_FUNC)
  ...
  real, intent(inout) :: CDH   (ILG) !< Surface drag coefficient for heat [ ]
  ...
  real, intent(inout) :: FVAP  (ILG) !< Product of surface-air humidity gradient, drag coefficient
  ...
  real, intent(out) :: ANVEG(ILG,ICTEM), RMLVEG(ILG,ICTEM)
  real :: CFLUXV_IN(ILG)
  ...
  !     * Local VARIABLES.
  !
  real :: qswnvg(ilg)   !< Visible shortwave radiation on the ground , \f$W/m^2\f$
  real :: QSWNIG        !< NIR on the ground , \f$W/m^2\f$
  real :: QSWNIC        !< Absorbed NIR part of shortwave radiation, \f$W/m^2\f$
  real :: HFREZ, HCONV, &
          RCONV, HCOOL, HMELT, SCONV, HWARM, WCAN, DQ0DT, &
          DRDT0, QEVAPT, BOWEN, DCFLUX, DXEVAP, TCANT, &
  ...
  !! first case, the maximum number of iterations ITERMX is set to 50, and in the second case it is set to 12.
  ...
  if (ctem_on) then
    ...
    call photosynCanopyConduct(AILCG, FCANC, TCAN, CO2CONC, & ! In ! Formerly PHTSYN3
                               ...
                               DAYL, DAYL_MAX, peatlandType, NcycleConnect_on, & ! In 
                               VCMAX0, CO2I1, CO2I2, & ! In/Out
                               RCPHTSYN, ANVEG, RMLVEG, AVE_SM_FUNC) ! Out
    !
  ...
  call  mossPht(il1, il2, iday, qswnvg, thliq, co2conc, tgnd, zsnow, & ! In
                pressg, Cmossmas, dmoss, mossPresent, Nmossmas, & ! In
                DAYL, DAYL_MAX, redcoeff_vcmaxMoss, Ncycle_on, & ! In 
