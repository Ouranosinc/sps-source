...
contains
  ...
  subroutine turbation (il1, il2, zbotw, isand, &
                        actlyr, spinfast, peatlandType, & ! In
                        litter, soilC) ! In/Out
    ...
    real, allocatable, dimension(:) :: rvect            !< Righthand side of Crank-Nicholson solver
    ...
    real, allocatable, dimension(:) :: effectiveD_sc    !< effective diffusion coefficient for soil C in that soil layer
    real, allocatable, dimension(:) :: effectiveD_lt    !< effective diffusion coefficient for litter in that soil layer
    ...
    real :: bturblyr                                    !< depth to which bioturbation is vigorous and diffusion coefficient is not reduced [m]
    real :: kbturblyr                                   !< depth of deepest layer where bioturbation occurs [m]
    ...
    do i = il1,il2                                       
      if (peatlandType(i) == 'None') then ! turbation only occurs in mineral soils (so not in peatlands)
        ...
        if (botlyr > 0) then
          ...
          allocate(rvect(botlyr))
          allocate(soilcinter(botlyr))
          allocate(littinter(botlyr))
          allocate(depthinter(botlyr))
          allocate(effectiveD_lt(botlyr))
          allocate(effectiveD_sc(botlyr))
          ...
          do j = 1,iccp1     !> We assume that the LUC pools do not migrate from the first soil layer so
            ...
            if (psoilc > zero) then !> and only do turbation for PFTs with some soil C. (assume soil C is always ~> lit).
              ...
              !! Because soil C can have spinfast applied, but litter won't, we need to treat them differently here.
              if (actlyr(i) <= 1.) then ! Cryoturbation is dominant
                do l = 1,botlyr
                  if (depthinter(l) <= actlyr(i)) then ! shallow so vigorous cryoturb.
                    effectiveD_sc(l) = diffus * real(spinfast)
                    effectiveD_lt(l) = diffus ! no spinfast allowed
                    ...
                    effectiveD_lt(l) = diffus * (1. - (depthinter(l) - actlyr(i)) &
                                    / ((kterm - 1.) * actlyr(i))) 
                    effectiveD_sc(l) = effectiveD_lt(l) * real(spinfast)  ! soil C is the same but can have spinfast applied.
                    turblyrbot = l  ! deepest layer in which turbation occurs
                ...
                do l = 1,botlyr
                  if (depthinter(l) <= bturblyr) then ! shallow so vigorous bioturb.
                    ...
                    effectiveD_lt(l) = diffus * (1. - (depthinter(l) - bturblyr) &
                                    / (kbturblyr - bturblyr)) 
                    effectiveD_sc(l) = effectiveD_lt(l) * real(spinfast) ! soil C is the same but can have spinfast applied.
                    turblyrbot = l  ! deepest layer in which turbation occurs
              ...
              ! upper boundary condition (common to all pools)
              avect(1) = 0.0 ! not defined in this element
              bvect(1) = 1.0
              cvect(1) = 0.0
              rvect(1) = 0.

              ! bottom boundary condition (common to all pools)
              avect(botlyr) = 0.
              bvect(botlyr) = 1.
              cvect(botlyr) = 0. ! not defined in this element
              rvect(botlyr) = 0.

              !> First treat the soil C:
              ! soil layers
              do l = 2,botlyr - 1

                dzm = depthinter(l) - depthinter(l - 1)

                termr = effectiveD_sc(l) * deltat / dzm ** 2
                avect(l) = -termr
                bvect(l) = 2. * (1. + termr)
                cvect(l) = -termr
                rvect(l) = termr * soilcinter(l - 1) + 2. * (1. - termr) &
                              * soilcinter(l) +  termr * soilcinter(l + 1)
              end do

              !> Once set up, call tridiagonal solver for the soil C mass:
              call tridiag(avect, bvect, cvect, rvect, soilcinter)


              !> Now do the same for the litter: 

              ! litter layers
              do l = 2,botlyr - 1
                dzm = depthinter(l) - depthinter(l - 1)
                termr = effectiveD_lt(l) * deltat / dzm ** 2
                avect(l) = -termr
                bvect(l) = 2. * (1. + termr)
                cvect(l) = -termr
                rvect(l) = termr * littinter(l - 1) + 2. * (1. - termr) &
                              * littinter(l) +  termr * littinter(l + 1)

              end do

              !> Next call tridiagonal solver for the litter mass:
              call tridiag(avect, bvect, cvect, rvect, littinter)
          ...
          deallocate(effectiveD_sc)
          deallocate(effectiveD_lt)
