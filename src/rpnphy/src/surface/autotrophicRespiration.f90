...
contains
  ...
  subroutine mainres (fcan, fct, stemmass, rootmass, il1, il2, ilg, & ! In
    ...
    real, intent(in) :: tracerRootMass(:,:)  !< Tracer mass in the root for each of the CTEM pfts, 14C: \f$ng ^{14}C/m^2\f$
    real, intent(out) :: rmsTracer(ilg,icc)  !< Tracer maintenance respiration for stem for the CTEM pfts (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    real, intent(out) :: rmrTracer(ilg,icc)  !< Tracer maintenance respiration for root for the CTEM pfts both (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
      ...
      do j = 1,icc ! loop 210
        if (fcan(i,j) > 0.) then
          ...
          if (useTracer /= 'None') then
            ...
            ! include here the conversion from  ng 14C/m2.day -> amol 14CO2/m2.sec
            rmsTracer(i,j) = tracerStemMass(i,j) * livstmfr(i,j) * q10funcStem &
                              * (bsrtstem(sort(j)) / ldoy) * 963.62
  ...
  !> @author Vivek Arora and Joe Melton
  subroutine growthRespiration(il1,il2,sort,useTracer,nppveg,rgveg, &
                                tracerNPP,tracerRG)
    ...
    integer, intent(in) :: il1        !< ilg position
    integer, intent(in) :: il2        !< pft
    ...
    real, intent(out) :: rgveg(:,:)                !< PFT level growth respiration (\f$\mu mol CO_2 m^{-2} s^{-1}\f$)
    real, intent(in), optional :: tracerNPP(:,:)   !< tracer NPP for individual pfts, (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    real, intent(out), optional :: tracerRG(:,:)   !< Tracer growth respiration (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    ...
    do i = il1,il2
      do j = 1, icc 
        if (nppveg(i,j) > zero) then
          ...
          if (useTracer /= 'None') tracerRG(i,j) = grescoef(sort(j)) * tracerNPP(i,j)
        else
          rgveg(i,j) = 0.0
          if (useTracer /= 'None') tracerRG(i,j) = 0.
        end if
      end do
    end do
