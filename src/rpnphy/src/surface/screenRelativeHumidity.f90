...
!
subroutine screenRelativeHumidity (SRH, STDA, ST, SQ, PRESSG, FMASK, ILG, IL1, IL2) ! Formerly SCREENRH
  !
  !     * DEC 16, 2014 - V.FORTIN.  REMOVE MERGE IN CALCULATION OF FRACW.
  !     * APR 30, 2009 - M.LAZARE.
  !
  use classicParams, only : EPS1, EPS2, T1S, RW1, RW2, RW3, RI1, RI2, RI3, TFREZ
  ...
  real, intent(out)   :: STDA(ILG)  ! screen level dew point temperatres (K)
  !
  !     * INPUT FIELDS.
  !
  ...
  real, parameter :: a = 237.7 !< parameter for dew point temperature calculation
  real, parameter :: b = 17.27 !< parameter for dew point temperature calculation
  real :: fac_A  !< surrogate factor for dew point temperature calculation  
  real :: temp_C !< temperature in Celcius as need for dew point temperature calculation
  real :: SDP    !< Surface dew point temperature   
  ...
  do IL = IL1,IL2
    if (FMASK(IL) > 0.) then
      ...
      SRH(IL) = MIN(MAX((SQ(IL) * (1. + QSW * FACTE)) &
                / (QSW * (1. + SQ(IL) * FACTE)),0.001),1.)

      temp_C = ST(IL) - TFREZ
      fac_A = log(SRH(IL)) + (b * temp_C) / (a + temp_C)
      SDP = TFREZ + a * fac_A / (b - fac_A)
      STDA(IL) = min(SDP, ST(IL))
