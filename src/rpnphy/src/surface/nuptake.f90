...
subroutine nuptake(il1, il2, sort, fcancmx, zbotw, delzw, & ! In
  ...
  real,intent(in),dimension(ilg)     :: nppmosstep      !< moss NPP (kgC/m2/day)
  ...
  real,intent(out),dimension(ilg)     :: ndemandmoss_npp        !< plant nitrogen demand for moss based on NPP, \f$g N m^{-2} day^{-1}\f$
  ...
  !> If moss present then calculate their N demand based upon their NPP
  do i = il1, il2
   if (mossPresent(i) /= 'None') then 
      if (mossPresent(i) == 'Sphagnum') then 
         ndemandmoss_npp(i) = nppmosstep(i) * 1000. / c2n_Mossmin(1)
      else if (mossPresent(i) == 'Feather') then
         ndemandmoss_npp(i) = nppmosstep(i) * 1000. / c2n_Mossmin(2)
      else 
         print*,'Unknown moss type in nuptake!', mossPresent(i)
