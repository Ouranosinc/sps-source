...
subroutine calcLandSurfParams (FC, FG, FCS, FGS, PAICAN, PAICNS, FSVF, FSVFS, & ! Formerly APREP
  ...
  !!
  !! In the same loop the roughness length for peatlands and moss covered uplands is also calculated assuming
  !! a natural log of the roughness length of the moss surface (zolnmoss)
  !!
    ...
    if (FG(I) > 0.) then
      if (ISAND(I,1) /= - 4) then
        ZOMLNG(I) = ((FG(I) - FCANMX(I,ICP1) * (1.0 - FSNOW(I))) * ZOLNG + &
                    ...
                    / FG(I)
        ! Find the roughness length for peatlands/moss covered uplands.
        if (mossPresent(i) == 'Sphagnum') then 
          ZOMLNG(I) = ((FG(I) - FCANMX(I,ICP1) * (1.0 - FSNOW(I))) &
                      * zolnmoss(1) + FCANMX(I,ICP1) * (1.0 - FSNOW(I)) * &
                      ZOLN(I,ICP1)) / FG(I)
        else if (mossPresent(i) == 'Feather') then 
          ZOMLNG(I) = ((FG(I) - FCANMX(I,ICP1) * (1.0 - FSNOW(I))) &
                      * zolnmoss(2) + FCANMX(I,ICP1) * (1.0 - FSNOW(I)) * &
                      ZOLN(I,ICP1)) / FG(I)
        end if
      else
        ZOMLNG(I) = ZOLNI
  ...
  do I = IL1,IL2 ! loop 350
    ...
    ! Removed the ctem_on if-block below; otherwise the CLASS-only run crashes.
    ! The aggregated canopy mass CMAI is calculated the same way in both ctem-on
    ! and CLASS-only configurations -- 12/2025
