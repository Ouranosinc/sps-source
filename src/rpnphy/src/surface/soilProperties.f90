...
subroutine soilProperties (THPOR, THLRET, THLMIN, BI, PSISAT, GRKSAT, & ! Formerly CLASSB
                           ...
                           ISAND, IGDR, NL, NM, IL1, IL2, IM, IG, peatlandType, &
                           mossPresent, KsatScalaron)
  !
  ...
  use classicParams,  only : thpmoss, thrmoss, thmmoss, bmoss, psismoss, &
                             ...
                             ALWV, ALWN, ALDV, ALDN, &
                             MIN_KSAT_FRAC, ADD_2_DEPTH, EXP_DECAY_PARM
  ...
  !
  integer, intent(out) :: ISAND (NL,NM,IG) !< Sand content flag
  integer, intent(out) :: IGDR  (NL,NM) !< Index of soil layer in which bedrock is encountered
  !
  !     * INPUT ARRAYS.
  !
  character(8), intent(in) :: peatlandType(nl,nm) !< Peatland type (else 'None')
  character(8), intent(in) :: mossPresent(nl,nm)   !< Type of moss present (else 'None')
  ...
  do M = 1,IM ! loop 200
    do I = IL1,IL2
      do J = 1,IG ! loop 150
        if (ISAND(I,M,1) == -4) then ! ice sheet
          DELZW(I,M,J) = DELZ(J)
          ISAND(I,M,J) = -4
      ...
      if (SAND(I,M,1) >= - 3.5) then
        ISOCI = NINT(SOCI(I,M))
        ! This sets the soil colour index of uninitialized land cells with surface bedrock to 16, which was chosen for rocks.
        if (ISAND(I,M,1) == -3 .and. ISOCI == 0) ISOCI = 16
        ALGWV(I,M) = ALWV(ISOCI)
        ALGWN(I,M) = ALWN(ISOCI)
        ALGDV(I,M) = ALDV(ISOCI)
        ALGDN(I,M) = ALDN(ISOCI)
      else
        ALGWV(I,M) = 0.0
        ALGWN(I,M) = 0.0
        ALGDV(I,M) = 0.0
  ...
  do J = 1,IG
    do M = 1,IM
      do I = IL1,IL2
        ...
        ! formulated for peatlandType. If other systems are considered (such as tropical peatlands), this scheme 
        ! will need to be at least checked over carefully so stop the user now.
        if (peatlandType(i,m) /= 'None' .and. peatlandType(i,m) /= 'Bog' .and. peatlandType(i,m) /= 'Fen') then                                          
          print*,'Error: soilProperties - not yet setup for peatland type',peatlandType(i,m)
          call errorHandler('soilProperties', -2)
        end if 

        ! Another couple checks, ISAND, moss, and peatland options should correspond.
        ! ISAND for peat is -2 and peatlands need to be at least 30 cm deep
        if (ISAND(I,M,J) > -2 .and. peatlandType(i,m) /= 'None' .and. ZBOTW(I,M,J) <= 0.3) then 
          print*,'Error: soilProperties - peatland type:',peatlandType(i,m),' but ISAND /= -2',ISAND(I,M,J)
          call errorHandler('soilProperties', -3)
        else if (j == 1 .and. ISAND(I,M,J) /= -2 .and. mossPresent(i,m) /= 'None') then 
          print*,'Error: soilProperties - moss is present:',mossPresent(i,m),' but ISAND /= -2',ISAND(I,M,J)
          call errorHandler('soilProperties', -4)
        end if 

        ! First we check for the special cases that determine how the soil is setup:
        ...
        ! Organics:
        !
        ! There are several options here when dealing with organic soils
        ! It can be an organic/peat soils (but not treated as a peatland),
        ! a peatland (using our full peatland scheme), or an upland with 
        ! moss cover. We now go through each option and setup in turn.
        !
        ! Organic soils/peat (but no explicit peat representation or moss
        !  cover). This simply sets the physical representation as peat but 
        ! does not consider any of the biogeochemistry.      
        else if (ISAND(I,M,J) == -2 .and. peatlandType(i,m) == 'None' .and. &
                mossPresent(i,m) == 'None') then ! Organic soils / Peat 
          ! This sets up the soils following Letts et al. /cite Letts2000-pg 
          if (j == 1) then
            k = 1
          ...
          ! based on the newly defined values, find thfc, thlw, and psiwlt:
          THFC(I,M,J) = THLRET(I,M,J)
          THLW(I,M,J) = THLMIN(I,M,J)
          PSIWLT(I,M,J) = PSISAT(I,M,J) * (THLMIN(I,M,J) / &
                          THPOR(I,M,J)) ** ( - BI(I,M,J))

        ! Next is explicit peatland simulation. This presently includes a moss
        ! representation as our peatlands are high-latitude (not tropical)
        ! The moss representation is assumed to be Sphagnum and the peatlands are bogs or fens. 
        ! Note: this setup assumes that the soil has 10 cm layers in the top 1 meter.
        ! We could add some logic to make sure it sets up correctly (COMBAK) - JM Nov 2023.
        ...
        !
        else if (ISAND(I,M,J) == -2 .and. &
                (peatlandType(i,m) == 'Bog' .or. peatlandType(i,m) == 'Fen') .and. &
                mossPresent(i,m) == 'Sphagnum') then  

          ! Then set the volumetric heat capacity and thermal conductivity. These
          ! are overwritten for the moss layer (below)
          HCPS(I,M,J) = HCPOM
          TCS(I,M,J) = TCOM
          if (j == 1) then ! First layer is Sphagnum moss                            
            thpor(i,m,j)  = thpmoss(1)
            thlret(i,m,j) = thrmoss(1)
            thlmin(i,m,j) = thmmoss(1)
            bi(i,m,j)     = bmoss(1)
            psisat(i,m,j) = psismoss(1)
            grksat(i,m,j) = grksmoss(1)
            hcps(i,m,j) = hcpmoss(1)
            tcs(i,m,j) = tcom
          ...
          ! First set the thlrat, which is the same for each layer
          thlrat(i,m,j) = 0.5 ** (1.0 / (2.0 * bi(i,m,j) + 3.0))          
          THFC(I,M,J) = THLRET(I,M,J)
          THLW(I,M,J) = THLMIN(I,M,J)
          PSIWLT(I,M,J) = PSISAT(I,M,J) * (THLMIN(I,M,J) / &
        ...
        ! Next is upland moss simulation. For absolute clarity,
        ! we retain the requirement for ISAND to be -2. Note that 
        ! upland moss simulation only affects the top layer. The rest 
        ! is treated as mineral soil.
        else if (ISAND(I,M,J) == -2 .and. peatlandType(i,m) == 'None' .and. &
                mossPresent(i,m) == 'Feather') then 
          if (j == 1) then ! First layer is Feather moss                            
              thpor(i,m,j)  = thpmoss(2)
              thlret(i,m,j) = thrmoss(2)
              thlmin(i,m,j) = thmmoss(2)
              bi(i,m,j)     = bmoss(2)
              psisat(i,m,j) = psismoss(2)
              grksat(i,m,j) = grksmoss(2)
              hcps(i,m,j) = hcpmoss(2)
              tcs(i,m,j) = tcom
              thlrat(i,m,j) = 0.5 ** (1.0 / (2.0 * bi(i,m,j) + 3.0))
              THFC(I,M,J) = THLRET(I,M,J)
              THLW(I,M,J) = THLMIN(I,M,J)
            ...
            ! While this is not a peatland, the feather moss is present on an organic soil.
            ! These situations can occur where there is not the high water table 
            ! expected in a peatland, but the soils are still considered organic. 
            ! Here we set it up using the peat soil characteristics, but respiration etc.
            ! does not function like it does when the model is explicitly dealing with
            ! a peatland.
            if (j == 2) then ! Next treated as fibric peat
              thpor(i,m,j)  = thporg(1)
              thlret(i,m,j) = throrg(1)
              thlmin(i,m,j) = thmorg(1)
              bi(i,m,j)     = borg(1)
              ...
              thpor(i,m,j)  = thporg(3)
                ...
                mossPresent(i,m) == 'Sphagnum') then ! Organic soils with Sphagnum moss

          print*,'Error: soilProperties - Invalid model setup. Peatland type: ',peatlandType(i,m),' and moss type:',mossPresent(i,m)
          call errorHandler('soilProperties', -3)

        ! Lastly, we have mineral soils:
          ...
          if (KsatScalaron) then
            ! Exponential distribution Ksat with a scalar of MIN_KSAT_FRAC below about 1m
            ! x =  max(MIN_KSAT_FRAC,exp(EXP_DECAY_PARM(y+ADD_2_DEPTH)))
            SCALAR = MAX(MIN_KSAT_FRAC,EXP((((ZBOTW(I,M,J)-(DELZW(I,M,J)*0.5))*(-1.0))+ADD_2_DEPTH)*EXP_DECAY_PARM))
          else
            SCALAR = 1.0
          end if
