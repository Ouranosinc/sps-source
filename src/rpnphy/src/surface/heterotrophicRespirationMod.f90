...
contains
  ...
  subroutine heterotrophicRespiration (il1, il2, ilg, peatlandType, mossPresent, fcancmx, & ! In
    ...
    use classicParams,         only : icc, ignd, zero, tanhq10, &
                                      ...
                                      tfrez, iccp2, iccp1, bsratesc_peat, &
                                      bsratelt_moss, bsratesc_moss, c_switch
    ...
    real, intent(in) :: tracerMossLitrMass(:)   !< Tracer mass in moss litter, 14C: \f$ng ^{14}C/m^2\f$
    ...
    real, intent(in) :: upMossSoilC(:,:)   !< Moss soil C mass (non-peat), \f$kg C/m^2\f$
    ...
    real, intent(out) :: litresmoss(ilg,ignd)    !< moss litter respiration (\f$\mu mol CO_2 m^{-2} s^{-1}\f$)
    real, intent(out) :: socres_peat(ilg)   !< heterotrophic repsiration from peat soil (\f$\mu mol CO_2 m^{-2} s^{-1}\f$)
    real, intent(out) :: socres_moss(ilg,ignd)   !< heterotrophic repsiration from moss-derived soil C (\f$\mu mol CO_2 m^{-2} s^{-1}\f$)
    ...
    real, intent(out) :: ltResTracer(ilg,iccp2,ignd)  !< Tracer litter respiration over the given unvegetated sub-area, 14C: \f$amol ^{14}CO2 m^{-2} s^{-1}\f$
    real, intent(out) :: sCResTracer(ilg,iccp2,ignd)  !< Tracer soil C respiration over the given unvegetated sub-area, 14C: \f$amol ^{14}CO2 m^{-2} s^{-1}\f$
    real, intent(out) :: litResMossTracer(ilg)    !< Tracer moss litter respiration, 14C: \f$amol ^{14}CO2 m^{-2} s^{-1}\f$
    real, intent(out) :: soCResPeatTracer(ilg)    !< Tracer heterotrophic repsiration from peat soil, 14C: \f$amol ^{14}CO2 m^{-2} s^{-1}\f$
    ...
    do j = 1,ignd
      do i = il1,il2
        if (isand(i,j) /= -3 .and. isand(i,j) /= -4) then ! for mineral or peat soils
          ! FLAG ROSE: Here perhaps we should have upland moss behave like peatlands for the first layer. JM
          if (peatlandType(i) == 'None') then ! mineral soils/ no moss only 
          ...
          if (psi(i,j) >= 10000.0) then ! very dry 
            ...
            ! Now we distinguish the litter response of mineral soils from that of peat soils at high 
            ! soil moistures:
            if (peatlandType(i) == 'None') then ! mineral soils 
            ...
            else ! peatlands ! FLAG Setting ltrmoscl at surface the same as across all depths for now. JM
              !FLAG ROSE: Again here maybe we should have upland moss be similar to peatlands for first layer. JM
            ...
            if (peatlandType(i) == 'None') then !mineral soils 
              ...
              ltrmoscl(i,j) = 0.01  !FLAG Keeping all depths the same for now.  JM
        ...
        if (turbationON) then
          !> If turbation is on, reduce the respiration at depth due to unresolved depth-dependent processes including
          ...
          reduceatdepth(i,j) = exp(-zbotw(i,j) / r_depthredu)
        else
          reduceatdepth(i,j) = 1.0
        end if
  ...
  !! of peat drainage so all LUC in a grid cell must occur on the upland tiles.
    ...
    do i = il1,il2
      ...
      do j = iccp1, iccp2 
        do k = 1,ignd
          ...
          if ((peatlandType(i) == 'None') .and. (fg(i) > zero .or. j == iccp2)) then
            ...
            if (useTracer /= 'None') ltResTracer(i,j,k) = ltrmoscl(i,k) * tracerLitrMass(i,j,k) &
            ...
            if (useTracer /= 'None') sCResTracer(i,j,k) = socmoscl(i,k) * tracerSoilCMass(i,j,k) &
      ...
      !! if no peatlands, then called to set outputs to 0.
      call soilCResPeat(il1, il2, ilg, peatlandType, & ! In
      ...
      ! Sphagnum moss creates peat. If tropical peatlands are added, this will need to
      ! be adapted.
      if (mossPresent(i) == 'Sphagnum' .and. peatlandType(i) /= 'None') then 
        ...
        !! used in this calculation as peatlands are assumed to not turbate and hence
        !! all moss litter is added to, and kept in, the first layer.
        !! Convert from kg c/kg c.year to u-mol co2/kg c.s using 2.64
        litresmoss(i,1) =  ltrmoscl(i,1) * litrmsmoss(i,1) * bsratelt_moss(1) * 2.64 * q10funcLitr(i,1)
          
        ! FLAG litResMossTracer not yet dealt with!
      else if (mossPresent(i) == 'Sphagnum' .and. peatlandType(i) == 'None') then ! is a upland but also has Sphagnum moss.
        print*,'Sphagnum moss specified, but not a peatland, does not work'
        call errorHandler('heterotrophicRespiration', -2)
      end if ! moss type

      ! Feather moss does not generate peat so is treated differently.
      if (mossPresent(i) == 'Feather' .and. peatlandType(i) == 'None') then

        do k = 1,ignd
          !> Moss litter respiration. Feather moss have the possibility of the C
          !! being stored in layers below the surface (due to turbation). We still only
          !! retain the respiration as a single value so sum across layers.
          ! Convert from kg c/kg c.year to u-mol co2/kg c.s using 2.64
            litresmoss(i,k) = ltrmoscl(i,k) * litrmsmoss(i,k) * bsratelt_moss(2) &
                              * 2.64 * q10funcLitr(i,k) * reduceatdepth(i,k)

          !> Moss-derived soil C respiration. Similar to the litter above.
            socres_moss(i,k) = socmoscl(i,k) * upMossSoilC(i,k) * bsratesc_moss(2) &
                                * 2.64 * q10funcSoilC(i,k) * reduceatdepth(i,k)
        end do 
      else if (mossPresent(i) == 'Feather' .and. peatlandType(i) /= 'None') then ! is a peatland but also has feather moss.
        print*,'feather moss specified, but also a peatland does not work'
        call errorHandler('heterotrophicRespiration', -3)
      end if 
      
      ! Vegetated areas on both mineral and peatland tiles. 
      !
      ! Note/FLAG: upland C in peatlands has to be dealt with. Vascular vegetation could be added to the peat
      ! C pool in addition to C originating from moss.
      do j = 1,icc
        do k= 1,ignd
  ...
  subroutine  soilCResPeat (il1, il2, ilg, peatlandType, & ! In
                          isand, peatdep, wtable, tbar, & ! In
                          zbotw, delzw, useTracer, & ! In
                          socres_peat, resoxic, resanoxic, & ! Out
                          soCResPeatTracer) ! Out
    ...
    real, intent(out) :: soCResPeatTracer(ilg)           !< Tracer heterotrophic repsiration from peat soil (\f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    ...
    ! real :: tracerCso (ilg)      !< Tracer carbon mass in the oxic compartment (14C: \f$ng ^{14}C m^{-2}\f$)
    ! real :: tracerCsa (ilg)      !< Tracer carbon mass in the anxic compartment (14C: \f$ng ^{14}C m^{-2}\f$)
    ...
    do i = il1,il2
      if (peatlandType(i) /= 'None') then
        ...
        if (peatlandType(i) == 'Bog') then ! bogs                                ! Rose: else if fens... add else for peatlandType = 3 (?)
          if (ewtable(i) < 0.0) then ! flooded
        ...
        else if (peatlandType(i) == 'Fen') then ! fens
          if (ewtable(i) < 0.0) then ! flooded
  ...
  subroutine updatePoolsHetResp (il1, il2, ilg, fcancmx, ltresveg, scresveg, & ! In
    ...
    real, intent(in) :: litresmoss(ilg,ignd)    !< moss litter respiration (\f$\mu mol CO_2 m^{-2} s^{-1}\f$)
    real, intent(in) :: socres_peat(ilg)   !< heterotrophic repsiration from peat soil (\f$\mu mol CO_2 m^{-2} s^{-1}\f$)
    real, intent(in) :: socres_moss(ilg,ignd)   !< heterotrophic repsiration from moss-derived soil C (\f$\mu mol CO_2 m^{-2} s^{-1}\f$)
    ...
    real, intent(in) :: ltResTracer(ilg,iccp2,ignd)  !< Tracer litter respiration for each pft + bare fraction (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    real, intent(in) :: sCResTracer(ilg,iccp2,ignd)  !< Tracer soil carbon respiration for the given sub-area (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    real, intent(in) :: litResMossTracer(ilg)    !< Tracer moss litter respiration (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    real, intent(in) :: soCResPeatTracer(ilg)    !< Tracer heterotrophic repsiration from peat soil (14C: \f$amol ^{14}CO_2 m^{-2} s^{-1}\f$)
    ...
    real, intent(inout) :: litrmsmoss(ilg,ignd)       !< Moss litter C (kgC/m2)
    real, intent(inout) :: upMossSoilC(ilg,ignd)      !< Moss-derived soil C (kgC/m2)
    ...
    real, intent(out) :: ltrestepmoss(ilg,ignd)  !< litter respiration from moss (kgC/m2/timestep)
    real, intent(out) :: humstepmoss(ilg,ignd)   !< moss humification (transfer from litter to SOM) (kgC/m2/timestep)
    ...
    real :: humicfacmosstemp !< Temp variable to store parameter value
    real :: ldoy
    ...
    !! If it is a peatland or moss covered, we also add litresmoss and socres_peat to the grid sum but 
    !! no bareground values as we assume peatlands/moss tiles have no bareground (and thus no 
    ...
    ! Adjust for peatland and moss contributions and also calculate moss litterfall.    
    do i = il1,il2

      ! First we add the peat soil carbon respiration to any from the vegetation 
      ! calculated above. The peat value is not calculated per layer so can simply add.
      ! We also increment the total soil respiration due to the peat contribution.
      if (peatlandType(i) /= 'None') then 
        socres(i) = socres(i) + socres_peat(i) 
        soilresp(i) = soilresp(i) + socres_peat(i) 
      end if 

      ! Now add any contributions from the moss
      if (mossPresent(i) /= 'None') then ! moss covered

        do k = 1,ignd

          litres(i) = litres(i) + litresmoss(i,k) ! add the moss litter, which is assumed to cover whole tile.
          ...
          ! This is used to update the moss litter pool later.
          ltrestepmoss(i,k) = litresmoss(i,k) * (1.0 / 963.62) * deltat   ! kgC/m2/dt

          ! Soil respiration at the tile level is similarly updated with the contribution 
          ! from moss litter and moss-derived soil C respiration.
          soilresp(i) = soilresp(i) + litresmoss(i,k) + socres_moss(i,k)

        end do 

        ! Find the moss litter fall.
        litrfallmoss(i) = mossLitrFall(Cmossmas(i), mossPresent(i), ldoy) 

      else ! no moss
        litrfallmoss(i) = 0.
        ltrestepmoss(i,:) = 0.  ! set to zero across all depths
      end if
    end do 
        ...
        do k = 1,ignd
          ...
          if (useTracer /= 'None') then
            if (j < iccp1) then ! vegetated
              ...
              if (peatlandType(i) == 'None') then ! non-peatlands
                tracerLitrMass(i,j,k) = tracerLitrMass(i,j,k) - (ltResTracer(i,j,k) * deltat / 963.62 &
                                                               * (1.0 + humicfac_bg))
            ...
            if (litrmass(i,j,k) < zero .or. tracerLitrMass(i,j,k) < zero) tracerLitrMass(i,j,k) = 0.0
            if (soilcmas(i,j,k) < zero .or. tracerSoilCMass(i,j,k) < zero) tracerSoilCMass(i,j,k) = 0.0
    ...
    do i = il1,il2 ! loop 470
      ...
      if (mossPresent(i) /= 'None') then
        ...
        if (mossPresent(i) == 'Sphagnum') then 
          humicfacmosstemp = humicfacmoss(1)
        else if (mossPresent(i) == 'Feather') then 
          humicfacmosstemp = humicfacmoss(2)
        end if 
        
        !> Update the moss litter and peat soil C pools based on their respiration rates
        do k = 1,ignd 
            humstepmoss(i,k) = humicfacmosstemp * ltrestepmoss(i,k)    ! kgC/m2/dt
            humiftrs(i)  = humiftrs(i) + humstepmoss(i,k) * (963.62 / deltat)! umol/m2/s 
          if (k == 1) then 
            ! Moss litter fall only goes into the first layer  
            litrmsmoss(i,k) = litrmsmoss(i,k) + litrfallmoss(i) - ltrestepmoss(i,k) - humstepmoss(i,k)     ! kg/m2
          else ! lower layers only have respiration or humification losses
            litrmsmoss(i,k) = litrmsmoss(i,k) - ltrestepmoss(i,k) - humstepmoss(i,k)     ! kg/m2
          end if 
        
        end do 
      end if

      ! Update the pools for soil moss-derived C stocks.
      ! The peatSoilC pool gains C due to humification and respiration. Presently, peatlands
      ! need to have Sphagnum moss and uplands (if moss covered) need to have Feather, so
      ! specify that and fail if user tries invalid combo.
      if (mossPresent(i) == 'Sphagnum' .and. peatlandType(i) /= 'None') then
         
        ! change the units of socres_peat so it is in kgC/m2/timestep.
        peatSoilC(i)  = peatSoilC(i)  + real(spinfast) * (sum(humstepmoss(i,:)) &
                                                       - socres_peat(i) * (1.0 / 963.62) * deltat)

      else if (mossPresent(i) == 'Feather' .and. peatlandType(i) == 'None') then ! feather moss covered uplands

        do k = 1,ignd 
          ! update the upMossSoilC for the C lost to respiration and gained from humification
          upMossSoilC(i,k) = upMossSoilC(i,k) + real(spinfast) * (humstepmoss(i,k) - socres_moss(i,k)) 
        end do 

      else if (mossPresent(i) /= 'Sphagnum' .and. peatlandType(i) /= 'None') then
        print*,'Peatland without sphagnum moss is not allowed at present'
        call errorHandler('updatePoolsHetResp',-1)
      else if (mossPresent(i) == 'Sphagnum' .and. peatlandType(i) == 'None') then
        print*,'Upland with moss has to be feather at present'
        call errorHandler('updatePoolsHetResp',-2)        
      end if
