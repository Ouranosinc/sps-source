...
subroutine radiationDriver (FC, FG, FCS, FGS, ALVSCN, ALIRCN, & ! Formerly CLASSA
                            ...
                            fracSnowParam, snoAlbedoParam, ALVSCTM, ALIRCTM, mossPresent, &
                            DSL, LAIPAIRatio, LAISPAISRatio) 

  !     * Nov  7/23 - J.Melton    Remove ipeatland and replace with mossPresent
  ...
  !     *                           IN SNOW.
  !     *                         - NEW "snoAlbedoParamA" OPTION, BASED ON
  !     *                           4-BAND SOLAR.
  ...
  use snowProcesses, only : fsnowDefault
  use classicParams,     only : DELT, PI
  ...
  real, intent(in) :: REFSNO(ILG)  !< Snow grain size (for snoAlbedoParam='4-band' option)  [m]
  real, intent(in) :: BCSNO(ILG)   !< Black carbon mixing ratio (for snoAlbedoParam='4-band' option)  \f$[kg m^{-3}]\f$
  ...
  select case (fracSnowParam)
  case ('Default') ! use the standard scheme
    call fsnowDefault(SNO, FSNOW, ZSNOW, WSNOW, RHOSNO, SNOLIM, ILG, IL1, IL2)
  case ('SL12') 
    ! The SL12 scheme is called at the end of waterbudgetdriver. Here, fsnow is adjusted 
    ! to keep zsnow in the snow covered fraction greater than or equal to 0.1 m.
    ! 
    do I = IL1,IL2
       if (SNO(I) > 0.0 .and. FSNOW(I) > 0.0) then
          ZSNOW(I) = SNO(I) / RHOSNO(I)  / FSNOW(I)
          if (ZSNOW(I) < (SNOLIM(I) - 0.00001)) then
             FSNOW(I) = MAX(0.,MIN(1.,SNO(I) / RHOSNO(I) / SNOLIM(I)))
             ZSNOW(I) = SNOLIM(I)
          end if
  ...
  case default
     print * , 'Unknown fracSnowParam ', fracSnowParam
  end select     
  ...
  call calcLandSurfParams(FC, FG, FCS, FGS, PAICAN, PAICNS, FSVF, FSVFS, & ! Formerly called APREP
                          ...
                          mossPresent, LAIPAIRatio, LAISPAISRatio)
  !
  !     * BARE SOIL ALBEDOS.
  !
  ...
  call snowAlbedoTransmiss(ALVSSN, ALIRSN, ALVSSC, ALIRSC, ALBSNO, & ! Formerly SNOALBA
                           ...
                           ALVSG, ALIRG, &
                           ILG, IG, IL1, IL2, JL, IALS, NBS, snoAlbedoParam)
  ...
  if (snoAlbedoParam == '2-band') then
    ...
    call errorHandler('radiationDriver', 3) 
