...
contains
  ...
  subroutine balcar (gleafmas, gleafmas_ns, gleafmas_s, stemmass, &
    ...
    real, intent(in) :: litrmsmoss(ilg,ignd)    !< moss litter C (kgC/m2)
    real, intent(in) :: plitrmsmoss(ilg)   !< moss litter C at the previous time step (kgC/m2)
    real, intent(in) :: ltrestepmoss(ilg,ignd)  !< litter respiration from moss (kgC/m2/timestep)
    real, intent(in) :: humicmosstep(ilg,ignd)  !< moss humification (kgC/m2/timestep)
    ...
    real :: scresveg_temp
    real :: litrestemp
    real :: diff1,diff2,diff3,tolerance2,tolerance3
    !
    ! Set precision dependent tolerance.
    !
    if (precision(tolrance) .gt. 6) then
      ! 64-bit
      tolerance2=tolrance
      tolerance3=tolrance
    else
      ...
      tolerance2=tolrance*2.  ! for tolrance=0.00025 - FLAG: tolrance increased to 0.0005, so this may not be needed.
      tolerance3=0.05        ! see if this works for -7 crash. Vivek. Tolerance needs bumping up with fire.
    endif
    !>
  ...
  than our tolerance of ',f12.6,' for root')
    ...
    do j = 1,icc ! loop 250
      do i = il1,il2 ! loop 260
        ...
        if ((abs(diff1 - diff2)) > tolerance3) then
          write(6,2003)i,j,abs(diff1 - diff2),tolerance3
2003      format('at (i) = (',i3,'),pft = ',i2,', ',f12.6,' is greater &
   than our tolerance of ',f12.6,' for litter')
    ...
    do i = il1,il2 ! loop 280
      if (peatlandType(i) == 'None') then ! Over the non-peatland/moss covered areas. 
        littempor = 0.
        litrestemp = 0.
        humtrstemp = 0.
        do k = 1,ignd
    ...
    do j = 1,icc ! loop 300
      do i = il1,il2 ! loop 310
        if (peatlandType(i) == 'None') then ! Over the non-peatland regions       
          soiltempor = 0.
          scresveg_temp = 0.
          humtrstemp = 0.
          do k = 1,ignd
...
2014      format('at (i) = (',i3,'),pft = ',i2,', ',f12.6,' is greater &
  than our tolerance of ',f12.6,' for soil c')
    ...
    do i = il1,il2 ! loop 400
      if (mossPresent(i) /= 'None') then ! moss covered only          
...
2008      format('at (i) = (',i3,'),',f12.6,' is greater' &
        ...
        humicmosstemp = 0.
        do k = 1,ignd
          mosstempor = mosstempor + litrmsmoss(i,k)
          ltresmosstemp = ltresmosstemp + ltrestepmoss(i,k)
          humicmosstemp = humicmosstemp + humicmosstep(i,k)
        end do 
        diff1 = mosstempor - plitrmsmoss(i)
        diff2 = litrfallmoss(i) - ltresmosstemp - humicmosstemp
        if ((abs(diff1 - diff2)) > tolrance) then
          write(6,3001)'total litrmsmoss(',i,') = ',mosstempor
          write(6,3001)'plitrmsmoss(',i,') = ',plitrmsmoss(i)
          write(6,3001)'litrfallmoss(',i,') = ',litrfallmoss(i)
          write(6,3001)'total ltrestepmoss(',i,') = ',ltresmosstemp
          write(6,3001)'total humicmosstep(',i,') = ',humicmosstemp
...
2009      format('at (i) = (',i3,'),',f12.6,' is greater &
 than our tolerance of ',f12.6,' for moss litter')
    ...
    do i = il1,il2 ! loop 350
      ...
      if ((abs(diff1 - diff2)) > (tolerance3 )) then ! FLAG, Vivek, 32 bit with fire
...
3001    format(a9,i2,a2,f14.9)
        write(6,2005)i,abs(diff1 - diff2),tolerance3
2005    format('at (i) = (',i3,'),',f12.6,' is greater than our tolerance of ',f12.6,' for vegetation biomass')
    ...
    do i = il1,il2 ! loop 380
      ...
      diff2 = (litrfall(i) - litres(i) - humiftrs(i) - galtcels(i) &
              + repro_cost_g(i)) * &
              (deltat/963.62)
      if ((abs(diff1 - diff2)) > (tolerance3 )) then    ! 32 bit with fire, Vivek
        ! and biomass can go a bit over the tolerance
        ...
        write(6,2006 )i,abs(diff1 - diff2),tolerance3
...
2007    format('at (i) = (',i3,'),',f12.6,' is greater &
  ...
  subroutine prepBalanceC (il1, il2, ilg, fcancmx, glealtrm, glfltrdt, & ! In
    ...
    real, intent(in) :: upMossSoilC(:,:) !< Soil C from upland mosses (kgC/m2)
    real, intent(in) :: Cmossmas(:) !< C in moss biomass, \f$kg C/m^2\f$
    real, intent(in) :: litrmsmoss(:,:) !< moss litter mass, \f$kg C/m^2\f$
    ...
    !
    !> Now add bareground, moss covered, or peatland values to the grid-average.
    !! If a peatland/moss covered, we assume no bareground and add the moss values
    !! instead. So here we update the vegetation biomass, the litterfall, and the
    !! grid average litter and soil C values.
    do i = il1,il2 ! loop 1020

      if (peatlandType(i) == 'None') then ! uplands

        ! Mineral soil bare ground litter and soil masses are added.       
        do k  = 1,ignd                                                                
        ...
        ! Peatlands and uplands with moss cover treat their soil C derived from moss differently.
        ! The peatlands track it in the peatSoilC pool while the uplands have the upMossSoilC pool.
          gavgscms(i) = gavgscms(i) + peatSoilC(i)

      end if 

      if (mossPresent(i) /= 'None') then ! moss cover
        vgbiomas(i) = vgbiomas(i) + Cmossmas(i)
        litrfall(i) = litrfall(i) + litrfallmoss(i) * (963.62 / deltat)! umolCO2/m2/s
        do k = 1, ignd
          gavgltms(i) = gavgltms(i) + litrmsmoss(i,k)
          gavgscms(i) = gavgscms(i) + upMossSoilC(i,k)
        end do 
      end if 
