SUBROUTINE DIASURFZ(UDIAG,VDIAG,TDIAG,QDIAG,N, VMOD,VDIR,  &
...
!                    H,UE,FTEMP,FVAP,ZU,ZT,LAT,FG,IL1,IL2,JL)
  ...
  use sfc_options , only: sl_Lmin_soil

  IMPLICIT NONE
  INTEGER N,JL,IL1,IL2
  REAL ZTG(N),ZUG(N)
  REAL UDIAG(N),VDIAG(N),TDIAG(N),QDIAG(N),ZUA(N),ZTA(N)
  REAL TA(N),QA(N),UA(N),VA(N),VMOD(N),VDIR(N)
...
!print*,'tmp_ZTG:',tmp_ZTG
  ...
  err = sl_sfclayer(tmp_TA, tmp_QA, VMOD   , VDIR   , tmp_ZUA, tmp_ZTA,            &
                    ...
                    tdiaglim=TDIAGLIM_FALSE, L_min=sl_Lmin_soil)
