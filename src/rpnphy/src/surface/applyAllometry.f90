...
contains
  ...
  subroutine allometry (gleafmas, gleafmas_ns, gleafmas_s, bleafmas, & ! In  ! Formerly bio2str
                        ...
                        zbotw, soildpth, fcancmx, peatlandType, maxAnnualActLyr, & ! In
    ...
    real, intent(out) :: rmatctem(ilg,icc,ignd)  !< fraction of live roots in each soil layer for each of ctem's 9 pfts
    real, intent(out) :: slai(ilg,icc)           !< storage or imaginary lai for phenology purposes
    real, intent(out) :: cmasvegc(ilg,ican)      !< total canopy mass for each of the 4 class pfts. recall that class requires canopy
                                                 !! mass as an input, and this is now provided by ctem. kg/m2.
    real, intent(out) :: ailcg(ilg,icc)          !< green lai for ctem's 9 pfts
    real, intent(out) :: ailcb(ilg,icc)          !< brown lai for ctem's 9 pfts. for now we assume only grasses can have brown leaves.
    real, intent(out) :: ailc(ilg,ican)          !< lai to be used by class
    real, intent(out) :: paic(ilg,ican)          !< plant area index for class' 4 pfts. this is the sum of leaf area index and stem area index.
    real, intent(out) :: rmatc(ilg,ican,ignd)    !< fraction of live roots in each soil layer for each of the class' 4 pfts
    real, intent(out) :: alvisc(ilg,ican)        !< albedo for 4 class pfts simulated by ctem, visible
    real, intent(out) :: alnirc(ilg,ican)        !< albedo for 4 class pfts simulated by ctem, near ir
    real, intent(out) :: slaic(ilg,ican)         !< storage lai. this will be used as min. lai that class sees
                                                 !! so that it doesn't blow up in its stomatal conductance calculations.
    real, intent(out) :: veghght(ilg,icc)        !< vegetation height (meters)
    ...
    do j = 1,icc ! loop 150
      do i = il1,il2 ! loop 160
        ...
        sla(j) = calcSLA(peatlandType(i),sort(j))
