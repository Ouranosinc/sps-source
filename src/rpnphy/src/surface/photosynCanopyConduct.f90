...
subroutine photosynCanopyConduct (AILCG, FCANC, TCAN, CO2CONC, PRESSG, FC, & !In ! Formerly PHTSYN3
                                  ...
                                  VCMAX0_OUT, CO2I1, CO2I2, & ! In/Out
                                  RC, AN_VEG, RML_VEG, AVE_SM_FUNC) ! Out

  use classicParams, only : KN, TUP, TLOW, alpha_phtsyn, omega_phtsyn, &
                            ISC4, MM, BB, VPD0, SN, SMSCALE, VMAX, VMAX_peat_redu, &
  ...
  integer, intent(in) :: IG       !< NO. OF SOIL LAYERS
  integer, intent(in) :: ICC      !< NO. OF CTEM's PFTs
  ...
  real, intent(in) :: FCANC(ILG,ICC) !< FRACTIONAL COVERAGE OF CTEM's PFTs
  ...
  real, intent(in) :: DAYL_MAX(ILG)      !< MAXIMUM DAYLENGTH FOR THAT LOCATION (hours)
  real, intent(in) :: DAYL(ILG)          !< DAYLENGTH FOR THAT LOCATION (hours)
  real, intent(in) :: THLIQ(ILG,IG)    !< LIQUID MOIS. CONTENT OF SOIL LAYERS
  ...
  real, intent(inout) :: VCMAX0_OUT(ILG,ICC)  !< Max. N-dependent photosynthetic rate at the top of canopy (\f$(mol CO_2 m^{-2} s^{-1}\f$)
  ...
  real, intent(out) :: AVE_SM_FUNC(ILG,ICC)  !< Soil mositure stress function, (unitless) FOR EACH PFT
  ...
  real, DIMENSION(ILG,ICC)  :: USEAILCG, VMAXC, JE3, TOT_RMAT, VMUNS1, VMUNS2, VMUNS3, &
                                            VMUNS, FPAR, JC, JC1, JC2, JC3, JE, JE1, JE2, JS, A_VEG, &
                                           RC_VEG, GCTU, GCMIN, GCMAX, VPD_TERM, CO2LS, GC, VM, CO2I, PREV_CO2I, VCMAX0
  ...
  do I = IL1,IL2
    ...
    EA     = MIN(QA(I) * PRESSG(I) / (0.622 + 0.378 * QA(I)),EASAT)
    RH(I)  = EA / EASAT
    VPD(I) = EASAT - EA
    VPD(I) = MAX(0.0,VPD(I))
  end do ! loop 420
    ...
    do M = K1,K2
      do I = IL1,IL2
        if (FCANC(I,M) > ZERO) then
          ...
          if (Ncycle_on .and. NcycleConnect_on) then
            if (FPAR(I,M) < ZERO) then
              NGLEAFMAS0(I,M) = 0.0
            else
              NGLEAFMAS0(I,M) = NGLEAFMAS(I,M) / FPAR(I,M)
            end if
 
            if (NGLEAFMAS0(I,M) < ZERO) NGLEAFMAS0(I,M) = 0.0
            if (GLEAFMAS(I,M) /= 0.0 .and. AILCG(I,M) > 0.0 .and. NGLEAFMAS0(I,M) > 0.0) then
               ...
               if ((REDCOEFF_VCMAX(I,M) > 0.0) .and. (REDCOEFF_VCMAX(I,M) <= 1.0)) VCMAX0(I,M) = VCMAX0(I,M) * REDCOEFF_VCMAX(I,M)
          ...
          ! Peatlands have the Vmax value scaled for some PFTs. Note this is independent of moss cover.
          if (peatlandType(i) /= 'None') then ! Peatland
            VCMAX0(I,M) = VCMAX0(I,M) * vmax_peat_redu(sort(m))
          end if 
          ...
          select case (ctempfts(m))

          case ('NdlDcdTr', 'BdlDCoTr', 'BdlEvgSh', 'BdlDCoSh') !deciduous trees or shrubs

	          vcmax0(i,m) = vcmax0(i,m) * (dayl(i) / dayl_max(i)) ** 2

          case ('NdlEvgTr', 'CNEvgTr', 'INEvgTr', 'BdlEvgTr', 'BdlDDrTr', 'CropC3', & !evergreens,crops,and grasses
          ...
          case default 
	          ...
	          call errorHandler('PHTSYN', - 4)

          end select  

          VCMAX0_OUT(i,m) = vcmax0(i,m)
...
999 CONTINUE ! Anchor for a GO TO statement that needs to be removed
  ...
  do I = IL1,IL2
    ...
    if (Ncycle_on .and. NcycleConnect_on) N_EFFECT(I) = 1.0
    VCMAX0_OUT(i,:) = VCMAX0(i,:) * N_EFFECT(I)
  ...
  do J = 1,ICC
    do I = IL1,IL2
      ...
      if( RML_VEG(I,J) < 0.0 .or. (A_VEG(I,J) * 1.0E+06) < 0.0 .or. RML_VEG(I,J) > 1000 .or. (A_VEG(I,J) * 1.0E+06) > 1000) then
        write(6, * )'unreasonable rates in photosynthesis'
        write(6, * )'leaf respiration = ',RML_VEG(I,J)
        write(6, * )'gross photosyntehsis = ',(A_VEG(I,J) * 1.0E+06)
        call errorHandler('PHTSYN', - 100)
      end if
