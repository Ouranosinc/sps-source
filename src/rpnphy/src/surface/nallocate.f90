...
subroutine nallocate(il1, il2, fcancmx, lfstatus, sort, ailcg,  & ! In
  ...
  use classicParams, only : ilg, ignd, icc, iccp1, ntolerance, ctempfts, &
                            ...
                            Fmax_leaf_N, Fmax_stem_N, Fmax_root_N, r_bnf_s, &
                            b_bnf, max_frac_bnf, C_cost_bnf_s, r_bnf_moss, C_cost_mossBNF
  ...
  real,intent(in),dimension(ilg)     :: ndemandmoss_npp      !< plant nitrogen demand for moss based on NPP, \f$g N m^{-2} day^{-1}\f$
  ...
  character(8),intent(in),dimension(ilg) :: mossPresent      !< Type of moss present (else 'None')
  real,intent(in),dimension(ilg)     :: depToMoss            !< Deposition to the moss, \f$g N m^{-2} day^{-1}\f$
  ...
  real,intent(out), dimension(ilg)    :: bnf_moss            !< moss-associated biological nitrogen fixation, \f$g N m^{-2} day^{-1}\f$
  ...
  real,dimension(ilg) :: nstressMoss           !< Moss nitrogen stress ()
 ...
 ! Symbiotic BNF:
    do j = 1,icc
       ...
       do i = il1,il2
         ...
         case ('NdlEvgTr','CNEvgTr','INEvgTr','NdlDcdTr','BdlEvgTr','BdlDCoTr',&
               'BdlDDrTr','GrassC3 ','GrassC4 ','Sedge   ','BdlEvgSh','BdlDCoSh')  
           if (ntchrveg(i,j) > 0.0 .and. isand(i,1) /= -4 .and. isand(i,1) /= -3) then
             if ((tbar(i,1)-273.16)<44.83 .and. (tbar(i,1)-273.16)>1.41) then
               bnf_nat(i,j) = max(0.,(r_bnf_s(sort(j)) * (nstress(i,j) + b_bnf(sort(j))))&
                                     * max(0.,(44.83-(tbar(i,1)-273.16))/(44.83-32.71)*&
                                     (((tbar(i,1)-273.16)-1.41)/(32.71-1.41))** &
                                     ((32.71-1.41)/(44.83-32.71))))
               if(C_cost_bnf_s > 0.) bnf_nat(i,j) = min(bnf_nat(i,j),max_frac_bnf(sort(j))*&
                                                       1000*rootmass_ns(i,j)/C_cost_bnf_s)
             else
               bnf_nat(i,j) = 0.
             end if
             bnf_ant(i,j) = 0.
         ...
         case ('CropC3  ','CropC4  ')
           if (ntchrveg(i,j) > 0.0 .and. isand(i,1) /= -4 .and. isand(i,1) /= -3) then
             ...
             if ((tbar(i,1)-273.16)<44.83 .and. (tbar(i,1)-273.16)>1.41) then
               ...
               if(C_cost_bnf_s > 0.) bnf_ant(i,j) = min(bnf_ant(i,j),max_frac_bnf(sort(j))*&
                                                       1000*rootmass_ns(i,j)/C_cost_bnf_s)
             else
               bnf_ant(i,j) = 0.
             end if
   ...
   !> If moss are present, calclate their N stress and then their BNF from their
   !! symbiotic cyanobacteria
   do i = il1, il2
      if (mossPresent(i) /= 'None') then 
         if (ndemandmoss_npp(i) > 0.) then 
            nstressMoss(i) = max(0.,min((ndemandmoss_npp(i) - (depToMoss(i)))/ &
                                     ndemandmoss_npp(i),1.))
         end if 
         ! FLAG! ROSE: This is just the vascular relationship thrown in to 
         ! have something that compiles. Replace with a moss-specific one!
         if (mossPresent(i) == 'Sphagnum') then
            if ((tbar(i,1)-273.16)<44.83 .and. (tbar(i,1)-273.16)>1.41) then
               bnf_moss(i) = max(0.,(r_bnf_moss(1) * (nstressMoss(i)))&
                                     * max(0.,(44.83-(tbar(i,1)-273.16))/(44.83-32.71)*&
                                     (((tbar(i,1)-273.16)-1.41)/(32.71-1.41))** &
                                     ((32.71-1.41)/(44.83-32.71))))
            end if 
            ...
            if ((tbar(i,1)-273.16)<44.83 .and. (tbar(i,1)-273.16)>1.41) then
               bnf_moss(i) =  max(0.,(r_bnf_moss(2) * nstressMoss(i))&
                                     * max(0.,(44.83-(tbar(i,1)-273.16))/(44.83-32.71)*&
                                     (((tbar(i,1)-273.16)-1.41)/(32.71-1.41))** &
                                     ((32.71-1.41)/(44.83-32.71))))
            end if 
         else 
            print*,'Unknown moss type in nallocate!', mossPresent(i)
            call errorHandler('nallocate', -3)      
         end if 
         ! Same here, this is just something to make sure it compiles!
         if(C_cost_mossBNF > 0.) bnf_moss(i) = max(0., min(bnf_moss(i),0.1*1000.*Cmossmas(i)/C_cost_mossBNF))

         ! Now we add the bnf_moss to the moss N pool
         Nmossmas(i) = Nmossmas(i) + bnf_moss(i)
  ...
  do j = 1,icc ! loop 120
     ...
     do i = il1,il2 ! loop 130
        ...
        if (fcancmx(i,j) > 0.0) then
          ...
          if (ndemandveg_wp_npp(i,j) > 0.0) then
             ...
             case ('NdlEvgTr','CNEvgTr','INEvgTr','BdlEvgTr','CropC3  ','CropC4  ','BdlEvgSh')
                nallocveg_l(i,j) = (nuptakeveg_nh4(i,j) + nuptakeveg_no3(i,j) + bnf_nat(i,j) + bnf_ant(i,j)) &
                                   * (ndemandveg_l_npp(i,j) / ndemandveg_wp_npp(i,j))
                nallocveg_s(i,j) = (nuptakeveg_nh4(i,j) + nuptakeveg_no3(i,j) + bnf_nat(i,j) + bnf_ant(i,j)) &
                                   * (ndemandveg_s_npp(i,j) / ndemandveg_wp_npp(i,j))
     ...
     if (tltrleaf(i,j) > 0.0 .and. & !lfstatus(i,j) == 3 .and. ! leaf status is almost never 3 for PFT4 - this needs to be looked at
        ...
        case ('NdlEvgTr','CNEvgTr','INEvgTr','BdlEvgTr','CropC3  ','CropC4  ',&
  ...
  do j = 1,icc
     ...
     do i = il1,il2
       ...
       case ('NdlEvgTr','CNEvgTr','INEvgTr','NdlDcdTr','BdlEvgTr','BdlDCoTr','BdlDDrTr',&
         ...
         !! Preventing negative ngleafmas_ns(i,j) when nntchveg_l(i,j) < nleafns2sveg(i,j)
         if (ngleafmas_ns(i,j) + nntchveg_l(i,j) < nleafns2sveg(i,j)) then
            !write(6,*)'warning nleafns2sveg(i,j) exceeds avaliable leaf mass at i = ',i,' for pft = ',j
            nleafns2sveg(i,j) = max(0.0, ngleafmas_ns(i,j) + nntchveg_l(i,j))
            temp_ns(i,j) = nntchveg_l(i,j)
            temp_s(i,j) = 0.0
         end if

         !! Preventing negative ngleafmas_ns(i,j) when nntchveg_l(i,j) > avaliable leaf pool
         if (ngleafmas_ns(i,j) + nntchveg_l(i,j) < 0.0) then
            !write(6,*)'warning nntchveg_l(i,j) exceeds avaliable leaf mass at i = ',i,' for pft = ',j
            if(ngleafmas_s(i,j) > abs(ngleafmas_ns(i,j) + nntchveg_l(i,j))) then
               temp_s(i,j) = ngleafmas_ns(i,j) + nntchveg_l(i,j)
               temp_ns(i,j) = nntchveg_l(i,j) - temp_s(i,j)
            else
               temp_s(i,j) = -ngleafmas_s(i,j)
               temp_ns(i,j) = -ngleafmas_ns(i,j)
               nntchveg_l(i,j) = temp_s(i,j) + temp_ns(i,j)
            end if
         end if
      ...
      if (ngleafmas_ns(i,j) < 0.0 .and. abs(ngleafmas_ns(i,j)) < 1.E-05) then
          ngleafmas_ns(i,j) = 0.0
          ngleafmas(i,j) = ngleafmas_s(i,j)
       end if
       if (ngleafmas_s(i,j) < 0.0 .and. abs(ngleafmas_s(i,j)) < 1.E-05) then
          ngleafmas_s(i,j) = 0.0
          ngleafmas(i,j) = ngleafmas_ns(i,j)
       end if

       if (ngleafmas_ns(i,j) < 0.0 .or. ngleafmas_s(i,j) < 0.0 .or. &
           ngleafmas(i,j) < 0.0) then
           write(6,*)'ngleafmas lt zero in nallocate.f90 at i=',i,' for pft=',j
           !write(6,*)'ngleafmas_ns = ',ngleafmas_ns(i,j)
           !write(6,*)'ngleafmas_s  = ',ngleafmas_s(i,j)
           !write(6,*)'ngleafmas    = ',ngleafmas(i,j)
       ...
       if ((ngleafmas_in(i,j) - ngleafmas(i,j) + nallocveg_l(i,j) &
            ...
            write(6,*)'imbalance in ngleafmas pool in nallocate subroutine at (i)=',i,'pft=',j
            call errorHandler('nallocate', - 9)
       end if
       if ((ngleafmas_ns_in(i,j) - ngleafmas_ns(i,j) + temp_ns(i,j) &
            - nleafns2sveg(i,j)) > ntolerance) then
            write(6,*)'ngleafmas_ns_in = ',ngleafmas_ns_in(i,j)
            write(6,*)'ngleafmas_ns = ',ngleafmas_ns(i,j)
            write(6,*)'temp_ns = ',temp_ns(i,j)
            write(6,*)'nleafns2sveg = ',nleafns2sveg(i,j)
            write(6,*)'imbalance in ngleafmas_ns pool in nallocate subroutine at (i)=',i,'pft=',j
            call errorHandler('nallocate', - 10)
       end if
       if ((ngleafmas_s_in(i,j) - ngleafmas_s(i,j) + temp_s(i,j) &
            + nleafns2sveg(i,j)) > ntolerance) then
            write(6,*)'ngleafmas_s_in = ',ngleafmas_s_in(i,j)
            write(6,*)'ngleafmas_s = ',ngleafmas_s(i,j)
            write(6,*)'temp_s = ',temp_s(i,j)
            write(6,*)'nleafns2sveg = ',nleafns2sveg(i,j)
            write(6,*)'imbalance in ngleafmas_s pool in nallocate subroutine at (i)=',i,'pft=',j
  ...
  do j = 1,icc
     do i = il1,il2
      ...
      !! Preventing negative nstemmass_ns(i,j) when nntchveg_s(i,j) < nstemns2sveg(i,j)
      if ((nstemmass_ns(i,j) + nntchveg_s(i,j)) < nstemns2sveg(i,j)) then
         !write(6,*)'warning nstemns2sveg(i,j) exceeds avaliable stem mass at i = ',i,' for pft = ',j
          nstemns2sveg(i,j) = max(0.0, nstemmass_ns(i,j) + nntchveg_s(i,j))
          temp_ns(i,j) = nntchveg_s(i,j)
          temp_s(i,j) = 0.0
      end if

      !! Preventing negative nstemmass_ns(i,j) when nntchveg_s(i,j) > avaliable stem pool
      if (nstemmass_ns(i,j) + nntchveg_s(i,j) < 0.0) then
         !write(6,*)'warning nntchveg_s(i,j) exceeds avaliable stem mass at i = ',i,' for pft = ',j
         if(nstemmass_s(i,j) > abs(nstemmass_ns(i,j) + nntchveg_s(i,j))) then
            temp_s(i,j) = nstemmass_ns(i,j) + nntchveg_s(i,j)
            temp_ns(i,j) = nntchveg_s(i,j) - temp_s(i,j)
           else
            temp_s(i,j) = -nstemmass_s(i,j)
            temp_ns(i,j) = -nstemmass_ns(i,j)
            nntchveg_s(i,j) = temp_s(i,j) + temp_ns(i,j)
         end if
      end if
       ...
       if (nstemmass_ns(i,j) < 0.0 .and. abs(nstemmass_ns(i,j)) < 1.E-05) then
          nstemmass_ns(i,j) = 0.0
          nstemmass(i,j) = nstemmass_s(i,j)
       end if
       if (nstemmass_s(i,j) < 0.0 .and. abs(nstemmass_s(i,j)) < 1.E-05) then
          nstemmass_s(i,j) = 0.0
          nstemmass(i,j) = nstemmass_ns(i,j)
       end if

       if (nstemmass_ns(i,j) < 0.0 .or. nstemmass_s(i,j) < 0.0 .or. nstemmass(i,j) < 0.0) then
           write(6,*)'nstemmass_ns lt zero in nallocate.f90 at i=',i,' for pft=',j
           ! write(6,*)'nstemmass_ns = ',nstemmass_ns(i,j)
           ! write(6,*)'nstemmass_s  = ',nstemmass_s(i,j)
           ! write(6,*)'nstemmass    = ',nstemmass(i,j)
       ...
       if ((nstemmass_in(i,j) - nstemmass(i,j) + nallocveg_s(i,j) &
            ...
            write(6,*)'imbalance in nstemmass pool in nallocate subroutine at (i)=',i,'pft=',j
            call errorHandler('nallocate', - 13)
       end if
       if ((nstemmass_ns_in(i,j) - nstemmass_ns(i,j) + temp_ns(i,j) &
            - nstemns2sveg(i,j)) > ntolerance) then
            write(6,*)'nstemmass_ns_in = ',nstemmass_ns_in(i,j)
            write(6,*)'nstemmass_ns = ',nstemmass_ns(i,j)
            write(6,*)'temp_ns = ',temp_ns(i,j)
            write(6,*)'nstemns2sveg = ',nstemns2sveg(i,j)
            write(6,*)'imbalance in nstemmass_ns pool in nallocate subroutine at (i)=',i,'pft=',j
            call errorHandler('nallocate', - 10)
       end if
       if ((nstemmass_s_in(i,j) - nstemmass_s(i,j) + temp_s(i,j) &
            + nstemns2sveg(i,j)) > ntolerance) then
            write(6,*)'nstemmass_s_in = ',nstemmass_s_in(i,j)
            write(6,*)'nstemmass_s = ',nstemmass_s(i,j)
            write(6,*)'temp_s = ',temp_s(i,j)
            write(6,*)'nstemns2sveg = ',nstemns2sveg(i,j)
            write(6,*)'imbalance in nstemmass_s pool in nallocate subroutine at (i)=',i,'pft=',j
            call errorHandler('nallocate', - 11)
       end if
  ...
  do j = 1,icc
     ...
     do i = il1,il2
      ...
      if ((nrootmass_ns(i,j) + nntchveg_r(i,j)) < nrootns2sveg(i,j)) then
          !write(6,*)'warning nrootns2sveg(i,j) exceeds avaliable root mass at i = ',i,' for pft = ',j
          nrootns2sveg(i,j) = max(0.0, nrootmass_ns(i,j) + nntchveg_r(i,j))
          temp_ns(i,j) = nntchveg_r(i,j)
          temp_s(i,j) = 0.0
      end if

      !! Preventing negative ntoormass_ns(i,j) when nntchveg_r(i,j) > avaliable root pool
      if (nrootmass_ns(i,j) + nntchveg_r(i,j) < 0.0) then
         !write(6,*)'warning nntchveg_r(i,j) exceeds avaliable root mass at i = ',i,' for pft = ',j
         if(nrootmass_s(i,j) > abs(nrootmass_ns(i,j) + nntchveg_r(i,j))) then
            temp_s(i,j) = nrootmass_ns(i,j) + nntchveg_r(i,j)
            temp_ns(i,j) = nntchveg_r(i,j) - temp_s(i,j)
           else
            temp_s(i,j) = -nrootmass_s(i,j)
            temp_ns(i,j) = -nrootmass_ns(i,j)
            nntchveg_r(i,j) = temp_s(i,j) + temp_ns(i,j)
         end if
      end if
      ...
      if (nrootmass_ns(i,j) < 0.0 .and. abs(nrootmass_ns(i,j)) < 1.E-05) then
         nrootmass_ns(i,j) = 0.0
         nrootmass(i,j) = nrootmass_s(i,j)
      end if
      if (nrootmass_s(i,j) < 0.0 .and. abs(nrootmass_s(i,j)) < 1.E-05) then
         nrootmass_s(i,j) = 0.0
      ...
      if (nrootmass_ns(i,j) < 0.0 .or. nrootmass_s(i,j) < 0.0 .or. &
         ...
         write(6,*)'nrootmass lt zero in nallocate.f90 at i=',i,' for pft=',j
         ! write(6,*)'nrootmass_ns(i,j)= ',nrootmass_ns(i,j)
         ! write(6,*)'nrootmass_s(i,j) = ',nrootmass_s(i,j)
         ! write(6,*)'nrootmass(i,j)   = ',nrootmass(i,j)
     ...
     if ((nrootmass_in(i,j) - nrootmass(i,j) + nallocveg_r(i,j) &
         - nre_allocveg_r2l(i,j) + nresorpedveg_r(i,j) &
          ...
          write(6,*)'imbalance in nrootmass pool in nallocate.f90 subroutine at (i)=',i,'pft=',j
          call errorHandler('nallocate', - 15)
     end if
     if ((nrootmass_ns_in(i,j) - nrootmass_ns(i,j) + temp_ns(i,j) &
            - nrootns2sveg(i,j)) > ntolerance) then
            write(6,*)'nrootmass_ns_in = ',nrootmass_ns_in(i,j)
            write(6,*)'nrootmass_ns = ',nrootmass_ns(i,j)
            write(6,*)'temp_ns = ',temp_ns(i,j)
            write(6,*)'nrootns2sveg = ',nrootns2sveg(i,j)
            write(6,*)'imbalance in nrootmass_ns pool in nallocate subroutine at (i)=',i,'pft=',j
            call errorHandler('nallocate', - 10)
     end if
     if ((nrootmass_s_in(i,j) - nrootmass_s(i,j) + temp_s(i,j) &
            + nrootns2sveg(i,j)) > ntolerance) then
            write(6,*)'nrootmass_s_in = ',nrootmass_s_in(i,j)
            write(6,*)'nrootmass_s = ',nrootmass_s(i,j)
            write(6,*)'temp_s = ',temp_s(i,j)
            write(6,*)'nrootns2sveg = ',nrootns2sveg(i,j)
            write(6,*)'imbalance in nrootmass_s pool in nallocate subroutine at (i)=',i,'pft=',j
            call errorHandler('nallocate', - 11)
     end if
