...
btopo=""
intopo=""
dircfg=configurations/"${MODEL_NAME}_cfgs"
cfg="0:0"
binMPIext=Abs
dryrun=0
...
myexpname=${PWD##*/}
run_basedir_name=RUNMOD
\rm -rf RUNMOD
mkdir -p ${run_basedir_name}
...
set_tsk_cfg() {
   ...
   ATMMOD=${BINMOD}/mainspsdm
   export UM_EXEC_Abs=${ATMMOD}
   export UM_EXEC_ovbin=${BINMOD}
...
set_tsk() {
   ...
   while [[ $dom0 -le $domN ]] ; do
      ...
      UM_EXEC_Abs_ovbin=$(set_tsk_cfg ${__tsk_file} ${cfgid} ${cfgdir} ${cfgfile1} ${model_sharedir})
      ((dom0 = dom0 + 1 ))
      ((ndoms = ndoms + 1 ))
